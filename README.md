...
```typescript
import { createClient, createClientWithPipeline, createServer } from '@dotdo/rpc'

// Standard client - each method call is a separate request
const api = createClient<MyAPI>({ url: 'https://my-worker.dev' })
const user = await api.getUser('123')
const orders = await api.users.list()

// Client with promise pipelining - chain calls in 1 round trip
const client = createClientWithPipeline<MyAPI>({ url: 'https://my-worker.dev' })
...
**Features:**
- Promise pipelining (Cap'n Proto style) via `createClientWithPipeline`
