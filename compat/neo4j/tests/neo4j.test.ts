...
describe('Neo4j Compat Layer', () => {
  ...
  describe('MATCH', () => {
    beforeEach(async () => {
      await session.run('CREATE (a:Person {name: "Alice", age: 30})')
      await session.run('CREATE (b:Person {name: "Bob", age: 25})')
      await session.run('CREATE (c:Company {name: "Acme"})')
    })
  ...
  describe('DELETE', () => {
    beforeEach(async () => {
      await session.run('CREATE (n:Temp {name: "ToDelete"})')
    })
  ...
  describe('Procedures', () => {
    beforeEach(async () => {
      // Create test graph
      await session.run('CREATE (a:Node {name: "A"})')
      await session.run('CREATE (b:Node {name: "B"})')
      await session.run('CREATE (c:Node {name: "C"})')
      await session.run(`
