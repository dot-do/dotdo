...
function parseCypher(query: string, params: CypherParams = {}): ParsedQuery {
  ...
  // Parse WHERE clause
  const whereMatch = substituted.match(/WHERE\s+(.+?)(?=\s+(?:RETURN|SET|DELETE|ORDER|SKIP|LIMIT|WITH|$))/is)
  const where = whereMatch ? parseWhere(whereMatch[1]!) : undefined

  // Parse RETURN clause
  const returnMatch = substituted.match(/RETURN\s+(.+?)(?=\s+(?:ORDER|SKIP|LIMIT|$))/is)
  const returnClause = returnMatch ? parseReturn(returnMatch[1]!) : undefined

  // Parse SET clause
  const setMatch = substituted.match(/SET\s+(.+?)(?=\s+(?:RETURN|DELETE|ORDER|SKIP|LIMIT|$))/is)
  const set = setMatch ? parseSet(setMatch[1]!) : undefined

  // Parse DELETE clause
  const deleteMatch = substituted.match(/DELETE\s+(.+?)(?=\s+(?:RETURN|ORDER|SKIP|LIMIT|$))/is)
  const deleteClause = deleteMatch ? deleteMatch[1]!.split(',').map(s => s.trim()) : undefined

  // Parse ORDER BY
  const orderMatch = substituted.match(/ORDER\s+BY\s+(.+?)(?=\s+(?:SKIP|LIMIT|$))/is)
...
function parsePatterns(query: string): PatternClause[] {
  ...
  // Find pattern sections (after MATCH, CREATE, MERGE)
  const patternMatches = query.matchAll(/(?:MATCH|CREATE|MERGE)\s+(\([^)]+\)(?:\s*-\[[^\]]*\]->\s*\([^)]+\))*)/gi)
  ...
  for (const match of patternMatches) {
    ...
    patterns.push({ nodes, relationships })
  }
...
class TransactionImpl implements Transaction {
  ...
  private async executeMatch(parsed: ParsedQuery, records: Neo4jRecord[]): Promise<void> {
    ...
    const bindings: Map<string, Node | Edge>[] = [new Map()]
  ...
  private async executeDelete(parsed: ParsedQuery, records: Neo4jRecord[]): Promise<void> {
    // First execute the MATCH part
    await this.executeMatch({ ...parsed, type: 'MATCH' }, records)
