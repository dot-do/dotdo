...
describe('@dotdo/hubspot/deals - Deal Pipeline Operations', () => {
  ...
  // ===========================================================================
  // DEAL PIPELINE VALIDATION (RED PHASE - Tests for unimplemented features)
  // ===========================================================================

  describe('Deal Pipeline Validation', () => {
    it('should reject moving deal to non-existent stage', async () => {
      const deal = await hubspot.crm.deals.basicApi.create({
        properties: { dealname: 'Validation Deal', dealstage: 'appointmentscheduled' },
      })

      await expect(
        hubspot.crm.deals.basicApi.update(deal.id, {
          properties: { dealstage: 'nonexistent_stage_id' },
        })
      ).rejects.toThrow()
    })

    it('should reject invalid stage for specified pipeline', async () => {
      const customPipeline = await hubspot.crm.pipelines.pipelinesApi.create('deals', {
        label: 'Validation Pipeline',
        displayOrder: 99,
        stages: [
          { id: 'vp_stage_1', label: 'VP Stage 1', displayOrder: 0 },
          { id: 'vp_stage_2', label: 'VP Stage 2', displayOrder: 1 },
        ],
      })

      const deal = await hubspot.crm.deals.basicApi.create({
        properties: {
          dealname: 'Cross Pipeline Deal',
          pipeline: customPipeline.id,
          dealstage: 'vp_stage_1',
        },
      })

      // Try to move to a stage from default pipeline while in custom pipeline
      await expect(
        hubspot.crm.deals.basicApi.update(deal.id, {
          properties: { dealstage: 'closedwon' }, // Default pipeline stage
        })
      ).rejects.toThrow()
    })

    it('should validate deal stage exists when creating deal', async () => {
      await expect(
        hubspot.crm.deals.basicApi.create({
          properties: {
            dealname: 'Invalid Stage Deal',
            dealstage: 'completely_invalid_stage',
          },
        })
      ).rejects.toThrow()
    })
  ...
  // ===========================================================================
  // DEAL PROBABILITY WEIGHTING (RED PHASE)
  // ===========================================================================

  describe('Deal Probability Weighting', () => {
    it('should calculate weighted pipeline value', async () => {
      // Create pipeline with probability metadata
      const pipeline = await hubspot.crm.pipelines.pipelinesApi.create('deals', {
        label: 'Weighted Pipeline',
        displayOrder: 100,
        stages: [
          { id: 'wp_10', label: '10%', displayOrder: 0, metadata: { probability: '0.1' } },
          { id: 'wp_50', label: '50%', displayOrder: 1, metadata: { probability: '0.5' } },
          { id: 'wp_90', label: '90%', displayOrder: 2, metadata: { probability: '0.9' } },
        ],
      })

      await hubspot.crm.deals.basicApi.create({
        properties: { dealname: 'Deal A', dealstage: 'wp_10', pipeline: pipeline.id, amount: '10000' },
      })
      await hubspot.crm.deals.basicApi.create({
        properties: { dealname: 'Deal B', dealstage: 'wp_50', pipeline: pipeline.id, amount: '20000' },
      })
      await hubspot.crm.deals.basicApi.create({
        properties: { dealname: 'Deal C', dealstage: 'wp_90', pipeline: pipeline.id, amount: '30000' },
      })

      // Expected: 10000*0.1 + 20000*0.5 + 30000*0.9 = 1000 + 10000 + 27000 = 38000
      const metrics = await hubspot.crm.pipelines.pipelinesApi.getWeightedValue('deals', pipeline.id)

      expect(metrics.weightedValue).toBe(38000)
      expect(metrics.totalValue).toBe(60000)
    })

    it('should get stage probability from metadata', async () => {
      const stage = await hubspot.crm.pipelines.pipelineStagesApi.getById(
        'deals',
        'default',
        'qualifiedtobuy'
      ...
      expect(stage.metadata?.probability).toBeDefined()
      expect(Number(stage.metadata?.probability)).toBeGreaterThanOrEqual(0)
      expect(Number(stage.metadata?.probability)).toBeLessThanOrEqual(1)
    })
  })

  // ===========================================================================
  // DEAL VELOCITY TRACKING (RED PHASE)
  // ===========================================================================

  describe('Deal Velocity Tracking', () => {
    it('should calculate average deal velocity (days to close)', async () => {
      // Create and close several deals with different durations
      const deal1 = await hubspot.crm.deals.basicApi.create({
        properties: { dealname: 'Velocity Deal 1', dealstage: 'appointmentscheduled' },
      })
      await hubspot.crm.deals.basicApi.update(deal1.id, {
        properties: { dealstage: 'closedwon' },
      })

      const velocity = await hubspot.crm.pipelines.pipelinesApi.getDealVelocity('deals', 'default')

      expect(velocity.averageDaysToClose).toBeDefined()
      expect(velocity.averageDaysToClose).toBeGreaterThanOrEqual(0)
      expect(velocity.dealsClosed).toBeGreaterThanOrEqual(1)
    })

    it('should track time spent in each stage', async () => {
      const deal = await hubspot.crm.deals.basicApi.create({
        properties: { dealname: 'Stage Time Deal', dealstage: 'appointmentscheduled' },
      })
      ...
      const stageHistory = await hubspot.crm.deals.basicApi.getStageHistory(deal.id)

      expect(stageHistory).toBeDefined()
      expect(stageHistory.length).toBeGreaterThanOrEqual(2)
      expect(stageHistory[0].stage).toBe('appointmentscheduled')
      expect(stageHistory[0].timeInStageMs).toBeGreaterThanOrEqual(50)
    })

    it('should calculate stage conversion rates', async () => {
      const conversionRates = await hubspot.crm.pipelines.pipelinesApi.getConversionRates(
        'deals',
        'default'
      )

      expect(conversionRates).toBeDefined()
      expect(Array.isArray(conversionRates.stages)).toBe(true)
      // Each stage should have entryCount, exitCount, and conversionRate
      for (const stage of conversionRates.stages) {
        expect(stage.stageId).toBeDefined()
        expect(stage.entryCount).toBeGreaterThanOrEqual(0)
  ...
  // ===========================================================================
  // WIN/LOSS ANALYSIS (RED PHASE)
  // ===========================================================================

  describe('Win/Loss Analysis', () => {
    it('should track closed lost reason', async () => {
      const deal = await hubspot.crm.deals.basicApi.create({
        properties: {
          dealname: 'Lost Deal Analysis',
          dealstage: 'qualifiedtobuy',
          amount: '25000',
        },
      })
      ...
      await hubspot.crm.deals.basicApi.update(deal.id, {
        properties: {
          ...
          closed_lost_reason: 'Price too high',
          hs_closed_amount: '0',
        },
      })

      const retrieved = await hubspot.crm.deals.basicApi.getById(deal.id, {
        properties: ['closed_lost_reason', 'dealstage'],
      })

      expect(retrieved.properties.closed_lost_reason).toBe('Price too high')
    })

    it('should calculate win rate by pipeline', async () => {
      const winRate = await hubspot.crm.pipelines.pipelinesApi.getWinRate('deals', 'default')

      expect(winRate).toBeDefined()
      expect(winRate.winRate).toBeGreaterThanOrEqual(0)
      expect(winRate.winRate).toBeLessThanOrEqual(100)
      expect(winRate.wonCount).toBeGreaterThanOrEqual(0)
      expect(winRate.lostCount).toBeGreaterThanOrEqual(0)
      expect(winRate.totalClosed).toBe(winRate.wonCount + winRate.lostCount)
    })

    it('should get loss reasons distribution', async () => {
      const lossReasons = await hubspot.crm.pipelines.pipelinesApi.getLossReasons('deals', 'default')

      expect(lossReasons).toBeDefined()
      expect(Array.isArray(lossReasons.reasons)).toBe(true)
      // Each reason should have count and percentage
      for (const reason of lossReasons.reasons) {
  ...
  // ===========================================================================
  // DEAL FORECASTING (RED PHASE)
  // ===========================================================================

  describe('Deal Forecasting', () => {
    it('should forecast revenue for period', async () => {
      const forecast = await hubspot.crm.deals.searchApi.getForecast({
        pipelineId: 'default',
      ...
      expect(forecast.dealCount).toBeGreaterThanOrEqual(0)
    })

    it('should forecast by owner', async () => {
      await hubspot.crm.deals.basicApi.create({
        properties: {
          dealname: 'Owner Forecast Deal',
          dealstage: 'qualifiedtobuy',
          amount: '50000',
          hubspot_owner_id: 'owner-123',
          closedate: '2025-06-15T00:00:00.000Z',
        },
      })

      const ownerForecast = await hubspot.crm.deals.searchApi.getForecastByOwner({
        pipelineId: 'default',
        startDate: '2025-01-01',
        endDate: '2025-12-31',
      })

      expect(ownerForecast).toBeDefined()
      expect(Array.isArray(ownerForecast.owners)).toBe(true)
    })

    it('should forecast by close date month', async () => {
      const monthlyForecast = await hubspot.crm.deals.searchApi.getMonthlyForecast({
        pipelineId: 'default',
        year: 2025,
      })

      expect(monthlyForecast).toBeDefined()
      expect(monthlyForecast.months).toHaveLength(12)
      for (const month of monthlyForecast.months) {
        ...
        expect(month.weightedValue).toBeGreaterThanOrEqual(0)
        expect(month.dealCount).toBeGreaterThanOrEqual(0)
      }
    })
  })

  // ===========================================================================
  // PIPELINE ROTATOR (RED PHASE)
  // ===========================================================================

  describe('Pipeline Rotator', () => {
    it('should configure deal rotation rules', async () => {
      const rotationConfig = await hubspot.crm.pipelines.pipelinesApi.configureRotation('deals', 'default', {
        enabled: true,
        method: 'round_robin',
        ownerIds: ['owner-1', 'owner-2', 'owner-3'],
        triggerStage: 'appointmentscheduled',
      })

      expect(rotationConfig).toBeDefined()
      expect(rotationConfig.enabled).toBe(true)
      expect(rotationConfig.method).toBe('round_robin')
      expect(rotationConfig.ownerIds).toHaveLength(3)
    })

    it('should automatically assign deal owner on stage entry', async () => {
      // Configure rotation first
      await hubspot.crm.pipelines.pipelinesApi.configureRotation('deals', 'default', {
        enabled: true,
        method: 'round_robin',
      ...
      // Create deal without owner - should auto-assign
      const deal = await hubspot.crm.deals.basicApi.create({
        properties: {
          dealname: 'Rotation Test Deal',
          dealstage: 'appointmentscheduled',
        },
      })

      expect(deal.properties.hubspot_owner_id).toBeDefined()
      expect(['owner-a', 'owner-b']).toContain(deal.properties.hubspot_owner_id)
    })

    it('should support weighted rotation', async () => {
      const rotationConfig = await hubspot.crm.pipelines.pipelinesApi.configureRotation('deals', 'default', {
        enabled: true,
        method: 'weighted',
        weights: {
          'owner-1': 50, // 50% of deals
          'owner-2': 30, // 30% of deals
          'owner-3': 20, // 20% of deals
        },
        triggerStage: 'appointmentscheduled',
      })

      expect(rotationConfig.method).toBe('weighted')
      expect(rotationConfig.weights).toBeDefined()
    })
  })

  // ===========================================================================
  // DEAL STALE DETECTION (RED PHASE)
  // ===========================================================================

  describe('Deal Stale Detection', () => {
    it('should identify stale deals by days in stage', async () => {
      // This would find deals stuck in a stage for too long
      const staleDeals = await hubspot.crm.deals.searchApi.findStaleDeals({
        pipelineId: 'default',
        daysInStage: 30,
      })

      expect(staleDeals).toBeDefined()
      expect(Array.isArray(staleDeals.results)).toBe(true)
      // Each stale deal should have staleness info
      for (const deal of staleDeals.results) {
        expect(deal.properties.dealname).toBeDefined()
        expect(deal.staleDays).toBeGreaterThanOrEqual(30)
      }
    })

    it('should configure stale deal alerts', async () => {
      const alertConfig = await hubspot.crm.pipelines.pipelinesApi.configureStaleAlerts('deals', 'default', {
        enabled: true,
        thresholdDays: {
          appointmentscheduled: 7,
          qualifiedtobuy: 14,
          presentationscheduled: 10,
          decisionmakerboughtin: 21,
        },
        notifyOwner: true,
        notifyManager: true,
      })
      ...
      expect(alertConfig.thresholdDays).toBeDefined()
    })

    it('should get pipeline health score', async () => {
      const healthScore = await hubspot.crm.pipelines.pipelinesApi.getHealthScore('deals', 'default')
      ...
      expect(healthScore.factors.staleDealsPercentage).toBeGreaterThanOrEqual(0)
      expect(healthScore.factors.averageVelocity).toBeGreaterThanOrEqual(0)
      expect(healthScore.factors.winRate).toBeGreaterThanOrEqual(0)
    })
  })
})
