/**
 * RED Phase Tests: Automation Triggers
 *
...
describe('Automation Triggers', () => {
  ...
  describe('WebhookTrigger', () => {
    ...
    it('should support HMAC signature verification', async () => {
      const trigger = new WebhookTrigger({
        path: '/webhook/github',
        method: 'POST',
        authentication: {
      ...
      // We'll test that the trigger validates HMAC signatures
      // Implementation will compute and verify the signature
      expect(trigger.config.authentication?.type).toBe('hmac')
    })
    ...
    it('should respond with custom response', async () => {
      ...
      expect(result.responseData).toEqual({ acknowledged: true })
    })

    it('should support form data parsing', async () => {
      const trigger = new WebhookTrigger({
        path: '/webhook/form',
        method: 'POST',
      })
      ...
      const request = new Request('http://localhost/webhook/form', {
        method: 'POST',
        body: formData,
      })
  ...
  describe('CronTrigger', () => {
    ...
    it('should emit trigger event at scheduled time', async () => {
      const trigger = new CronTrigger({
    ...
    it('should include execution metadata', async () => {
      ...
      expect(events[0].metadata?.scheduledTime).toBeDefined()
      expect(events[0].metadata?.expression).toBe('* * * * *')
    ...
    it('should not emit events when stopped', async () => {
      const trigger = new CronTrigger({
        expression: '* * * * *',
      })
  ...
  describe('PollingTrigger', () => {
    ...
    it('should handle poll errors gracefully', async () => {
      ...
      trigger.onTrigger((event) => events.push(event))
      trigger.onError((error) => errors.push(error))
      trigger.start()
      ...
      // Third poll succeeds (continues after error)
      await vi.advanceTimersByTimeAsync(60 * 1000)
    ...
    it('should support exponential backoff on errors', async () => {
      let pollCount = 0
      ...
      trigger.onError(() => {}) // Suppress errors
      trigger.start()
      ...
      // After 1 minute (first backoff)
      await vi.advanceTimersByTimeAsync(60 * 1000)
      expect(pollCount).toBe(2)

      // After 2 minutes (second backoff with factor 2)
    ...
    it('should persist state for deduplication', async () => {
      const state = new Map<string, unknown>()

      const trigger = new PollingTrigger({
        interval: '1m',
        pollFunction: async () => ({ data: [{ id: 'item-1' }] }),
        deduplicationKey: (item) => item.id,
        stateStorage: {
    ...
    it('should support watermark-based polling', async () => {
      let lastWatermark: string | undefined
      const events: TriggerEvent[] = []

      const trigger = new PollingTrigger({
        interval: '1m',
        pollFunction: async (watermark) => {
      ...
      // First poll with no watermark
      await vi.advanceTimersByTimeAsync(100)
      expect(lastWatermark).toBeUndefined()

      // Second poll with watermark from first
      await vi.advanceTimersByTimeAsync(60 * 1000)
      expect(lastWatermark).toBe('2024-01-15T10:00:00Z')
