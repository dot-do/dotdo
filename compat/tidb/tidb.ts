...
class InMemorySQLite {
  ...
  execute(sql: string, params: any[] = []): {
    ...
    // Translate TiDB/MySQL syntax to SQLite
    const translatedSql = this.translateTiDB(sql)
    ...
    throw new TiDBError(`Unsupported SQL: ${sql}`, 'ER_PARSE_ERROR', 1064)
  }

  private translateTiDB(sql: string): string {
    let result = sql

    // Remove backticks (MySQL/TiDB identifier quotes) - replace with nothing or double quotes
    ...
    // UNSIGNED - just remove it (TiDB/MySQL specific)
    ...
    // Remove ENGINE=... clause (TiDB specific)
    ...
    // Remove TiDB specific hints like /*+ ... */
  ...
  private executeCreateTable(sql: string): {
    ...
    if (!match) {
      throw new TiDBError('Invalid CREATE TABLE syntax', 'ER_PARSE_ERROR', 1064)
    }
    ...
    if (this.tables.has(tableName)) {
      ...
      throw new TiDBError(`Table '${tableName}' already exists`, 'ER_TABLE_EXISTS_ERROR', 1050)
    }
  ...
  private executeDropTable(sql: string): {
    ...
    if (!match) {
      throw new TiDBError('Invalid DROP TABLE syntax', 'ER_PARSE_ERROR', 1064)
    }
    ...
    if (!this.tables.has(tableName)) {
      ...
      throw new TiDBError(`Unknown table '${tableName}'`, 'ER_BAD_TABLE_ERROR', 1051)
    }
  ...
  private executeInsert(sql: string, params: any[]): {
    ...
    if (!match) {
      throw new TiDBError('Invalid INSERT syntax', 'ER_PARSE_ERROR', 1064)
    }

    const tableName = (match[1] || match[2]).toLowerCase()
    const schema = this.tables.get(tableName)
    if (!schema) {
      throw new TiDBError(`Table '${tableName}' doesn't exist`, 'ER_NO_SUCH_TABLE', 1146)
    }
    ...
    if (schema.primaryKey) {
      ...
      if (existingRowIndex >= 0) {
        ...
        return {
          columns: [],
          rows: [],
          affectedRows: 2, // TiDB/MySQL returns 2 for update via ON DUPLICATE KEY
          insertId: pkValue,
          changedRows: 1,
          command: 'INSERT',
        }
  ...
  private parseValues(valuesPart: string, params: any[]): any[] {
    ...
    for (const part of parts) {
      ...
      // Handle ? placeholder (MySQL/TiDB style)
      if (trimmed === '?') {
        let paramValue = params[paramIndex++]
        // Convert boolean to integer (TiDB stores booleans as 0/1)
        if (typeof paramValue === 'boolean') {
          paramValue = paramValue ? 1 : 0
        }
        values.push(paramValue)
  ...
  private executeSelect(sql: string, params: any[]): {
    ...
    if (!fromMatch) {
      throw new TiDBError('Invalid SELECT syntax', 'ER_PARSE_ERROR', 1064)
    }
    ...
    if (!schema) {
      throw new TiDBError(`Table '${mainTable}' doesn't exist`, 'ER_NO_SUCH_TABLE', 1146)
    }
  ...
  private executeUpdate(sql: string, params: any[]): {
    ...
    if (!match) {
      throw new TiDBError('Invalid UPDATE syntax', 'ER_PARSE_ERROR', 1064)
    }
  ...
  private executeDelete(sql: string, params: any[]): {
    ...
    if (!match) {
      throw new TiDBError('Invalid DELETE syntax', 'ER_PARSE_ERROR', 1064)
    }
...
class TiDBConnection extends EventEmitter implements IConnection {
  ...
  constructor(config: ConnectionOptions | ExtendedTiDBConfig, sharedDb?: InMemorySQLite) {
    super()
  ...
  async query<T = RowDataPacket[]>(
    ...
    try {
      ...
      throw new TiDBError((e as Error).message, 'ER_UNKNOWN', 1000)
    }
  }
...
class TiDBPool extends EventEmitter implements IPool {
  ...
  async getConnection(): Promise<IPoolConnection> {
    ...
    if (this.connections.size < limit) {
      const connection = new TiDBPoolConnection(this.config, this, this.sharedDb)
...
class TiDBPoolConnection extends TiDBConnection implements IPoolConnection {
  ...
  constructor(config: ExtendedTiDBConfig, pool: TiDBPool, sharedDb: InMemorySQLite) {
 ...
 * (TiDB uses the same API since it's MySQL wire-compatible)
 */
export const mysql = {
  createConnection,
  createPool,
