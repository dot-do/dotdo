...
describe('n8n Workflow Integration', () => {
    ...
    const workflow = new Workflow({
      ...
      nodes: [
        ...
        },
        {
          ...
          parameters: {
            assignments: {
              values: [{ name: 'processed', value: true, type: 'boolean' }],
            },
          },
        },
      ],
  ...
  it('should propagate trigger context to workflow nodes', async () => {
    ...
    // Code node that captures context
    const workflow = new Workflow({
      name: 'Context Test',
      nodes: [
        {
        ...
        },
        {
          ...
          position: [200, 0],
          parameters: {
            code: `return items.map(item => ({ json: { ...item.json, hasContext: true } }))`,
          },
        },
      ],
      connections: {
...
describe('Trigger Statistics', () => {
  it('should track n8n trigger statistics through TriggerEngine', async () => {
    ...
    // Execute a few times
    const request = new Request('https://api.dotdo.dev/stats', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({}),
    })
...
describe('Event Bus Integration', () => {
  ...
  it('should allow subscribing to specific node triggers', async () => {
    ...
    const request = new Request('https://api.dotdo.dev/subscribe', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
