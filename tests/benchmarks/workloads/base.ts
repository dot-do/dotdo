...
export abstract class BaseWorkload implements Workload {
  abstract readonly config: WorkloadConfig
  ...
  *operations(keyCount: number): Generator<WorkloadOperation> {
  ...
  protected getKey(keyCount: number, pattern: AccessPattern, seqIndex: number): string {
    switch (pattern) {
      case 'sequential':
