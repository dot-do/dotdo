...
/**
 * In-memory test fetch for OpenAI integration tests.
...
describe('OpenAI Compat Layer - Real Integration', () => {
  ...
  describe('Chat Completions', () => {
    it('creates chat completion with messages', async () => {
      ...
      const { OpenAI } = await import('../../../compat/openai/index')
      const client = new OpenAI({ apiKey: 'sk-xxx', fetch: testFetch.createFetch() })
  ...
  describe('Error Handling Compatibility', () => {
    it('throws OpenAIError for API errors', async () => {
      testFetch.queueResponse(false, 401, {
        error: {
      ...
      const { OpenAI } = await import('../../../compat/openai/index')
      const client = new OpenAI({ apiKey: 'invalid', fetch: testFetch.createFetch() })
    ...
    it('handles rate limit errors', async () => {
      testFetch.queueResponse(false, 429, {
        error: {
