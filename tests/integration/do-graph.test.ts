...
describe('GraphEngine Node Operations', () => {
  ...
  describe('Node Creation and Retrieval', () => {
    ...
    it('handles node deletion with edge cleanup', async () => {
      ...
      // Add edge
      await engine.createEdge('placed', customer.id, order.id, {})
...
describe('GraphEngine Edge and Traversal Operations', () => {
  ...
  describe('Edge Creation and Traversal', () => {
    it('creates edges and enables traversal', async () => {
      ...
      // Create relationship
      const edge = await engine.createEdge('memberOf', user.id, team.id, {
        role: 'engineer',
        since: '2024-01-01',
      })
    ...
    it('supports bidirectional traversal', async () => {
      ...
      await engine.createEdge('memberOf', alice.id, team.id, {})
      await engine.createEdge('memberOf', bob.id, team.id, {})
    ...
    it('filters traversal by edge type', async () => {
      ...
      await engine.createEdge('memberOf', alice.id, team.id, {})
      await engine.createEdge('contributes', alice.id, project.id, {})
      await engine.createEdge('follows', alice.id, bob.id, {})
  ...
  describe('Graph Analytics', () => {
    it('computes shortest path between nodes', async () => {
      ...
      await engine.createEdge('follows', alice.id, bob.id, {})
      await engine.createEdge('follows', bob.id, carol.id, {})
      await engine.createEdge('follows', carol.id, dave.id, {})
      // Alice -> Dave (direct path)
      await engine.createEdge('follows', alice.id, dave.id, {})

      const path = await engine.shortestPath(alice.id, dave.id)

      expect(path).toBeDefined()
      expect(path!.length).toBe(2) // Alice -> Dave directly
    })
    ...
    it('finds all paths between nodes within depth', async () => {
      ...
      await engine.createEdge('connects', a.id, b.id, {})
      await engine.createEdge('connects', b.id, d.id, {})
      await engine.createEdge('connects', a.id, c.id, {})
      await engine.createEdge('connects', c.id, d.id, {})
      await engine.createEdge('connects', a.id, d.id, {})
    ...
    it('detects cycles during traversal', async () => {
      ...
      await engine.createEdge('dependsOn', taskA.id, taskB.id, {})
      await engine.createEdge('dependsOn', taskB.id, taskC.id, {})
      await engine.createEdge('dependsOn', taskC.id, taskA.id, {}) // Creates cycle
...
describe('Typed Nodes Graph Operations', () => {
  ...
  describe('Type-Based Operations', () => {
    ...
    it('queries nodes by label', async () => {
      ...
      const customers = await engine.findNodes({ label: 'Customer' })
    ...
    it('queries edges by type', async () => {
      ...
      await engine.createEdge('follows', alice.id, bob.id, {})
      await engine.createEdge('authored', alice.id, post.id, {})
      await engine.createEdge('liked', bob.id, post.id, {})

      // Query only 'follows' relationships
      const followEdges = await engine.findEdges({ type: 'follows' })
