...
describe('Workflow Triggers Agent Handoff', () => {
  ...
  describe('Workflow Error Handling During Handoff', () => {
    it('handles agent timeout gracefully', async () => {
      const handoffTimeout = 30000 // 30 seconds
      let timedOut = false
      let fallbackExecuted = false

      // Simulate timeout check
      const handoffStarted = Date.now()
      const handoffDeadline = handoffStarted + handoffTimeout

      // Simulate timeout (for testing, we check if deadline passed)
      const checkTimeout = () => {
        if (Date.now() > handoffDeadline - 29900) { // Simulate near-timeout
          timedOut = true
          return true
        }
        return false
...
describe('Handoff Graph Tracking', () => {
  describe('Graph-Based Handoff Chain', () => {
    it('tracks handoffs as graph relationships', async () => {
      ...
      // Add agents as nodes
      engine.addNode({ id: 'Agent/intake', label: 'Agent', data: { name: 'Intake Agent' } })
      engine.addNode({ id: 'Agent/support', label: 'Agent', data: { name: 'Support Agent' } })
      engine.addNode({ id: 'Agent/billing', label: 'Agent', data: { name: 'Billing Agent' } })

      // Track handoffs as edges
      engine.addEdge({
        source: 'Agent/intake',
        target: 'Agent/support',
        label: 'handedOffTo',
        data: { handoffId: 'h-1', timestamp: new Date().toISOString(), reason: 'routing' },
      })
      engine.addEdge({
        source: 'Agent/support',
        target: 'Agent/billing',
        label: 'handedOffTo',
        data: { handoffId: 'h-2', timestamp: new Date().toISOString(), reason: 'specialization' },
      })

      // Query handoff chain
      const result = engine.traverse('Agent/intake', {
        direction: 'outgoing',
        maxDepth: 5,
        edgeFilter: (e) => e.label === 'handedOffTo',
      })
    ...
    it('detects circular handoff (ping-pong)', async () => {
      ...
      // Agents
      engine.addNode({ id: 'Agent/a', label: 'Agent' })
      engine.addNode({ id: 'Agent/b', label: 'Agent' })

      // Circular handoffs
      engine.addEdge({ source: 'Agent/a', target: 'Agent/b', label: 'handedOffTo' })
      engine.addEdge({ source: 'Agent/b', target: 'Agent/a', label: 'handedOffTo' }) // Circular!

      // Check for cycle
      const result = engine.traverse('Agent/a', { direction: 'outgoing', maxDepth: 10 })
    ...
    it('computes handoff analytics from graph', async () => {
      ...
      agents.forEach(a => engine.addNode({ id: `Agent/${a}`, label: 'Agent', data: { name: a } }))

      // Multiple handoff chains
      engine.addEdge({ source: 'Agent/intake', target: 'Agent/support', label: 'handedOffTo' })
      engine.addEdge({ source: 'Agent/intake', target: 'Agent/billing', label: 'handedOffTo' })
      engine.addEdge({ source: 'Agent/support', target: 'Agent/shipping', label: 'handedOffTo' })

      // Compute analytics
      const handoffEdges = engine.findEdges({ label: 'handedOffTo' })
      const stats = engine.getStats()

      // Intake agent has most outgoing handoffs
      const intakeOutgoing = handoffEdges.filter(e => e.source === 'Agent/intake')
