...
export function runWithChildContext<T>(
  ...
  const child = createObservabilityContext({
    correlationId: parent?.correlationId ?? updates.correlationId,
    traceContext: updates.traceContext ?? parent?.traceContext,
    activeSpan: updates.activeSpan ?? parent?.activeSpan,
    baggage: new Map([
