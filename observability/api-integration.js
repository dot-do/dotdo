...
import { createTracer, SpanKind, SpanStatusCode, extractTraceContext, injectTraceContext, } from './tracing';
import { createObservabilityContext, extractContextFromHeaders, runWithContext, generateCorrelationId, CORRELATION_ID_HEADER, } from './context';
/**
 ...
 */
export function createAPIMetrics(meter) {
    return {
 ...
 * ```
 */
export function createAPIObservabilityMiddleware(config = {}) {
    const { serviceName = 'dotdo-api', logLevel = LogLevel.INFO, enableTracing = true, enableMetrics = true, enableLogging = true, excludePaths = DEFAULT_EXCLUDE_PATHS, excludeHeaders = DEFAULT_EXCLUDE_HEADERS, } = config;
    ...
    return async (c, next) => {
        ...
        // Log request start
        if (enableLogging) {
            const childContext = { correlationId };
            if (span) {
        ...
        try {
            ...
            // Log response
            if (enableLogging) {
                const responseChildContext = { correlationId };
                if (span) {
 ...
 */
export function createRouteTracer(c, config = {}) {
 ...
 */
export function metricsEndpoint(meter) {
