 ...
 *
 * Tests for pusher-js API compatibility backed by DO storage:
...
import { PusherError, AuthError, SubscriptionError, isPrivateChannel, isPresenceChannel } from './types'
import { Pusher, createPusher, _clearAll } from './pusher'
...
describe('Integration', () => {
  ...
  it('should work with live updates scenario', async () => {
    // Server (simulated)
    const server = new Pusher('app-key')
    await new Promise(r => setTimeout(r, 50))
    ...
    // Server publishes updates (via send_event)
    server.send_event('stock-update', { symbol: 'AAPL', price: 150.5 }, 'updates')
    await new Promise(r => setTimeout(r, 50))

    // Note: In real implementation, server would push to all clients
    // For testing, we manually deliver
