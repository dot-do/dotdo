...
describe('Sandbox Regex Bypass Vulnerabilities (do-94il)', () => {
  ...
  beforeEach(() => {
    context = createTestContext('sandbox-regex-bypass-test')
  })
  ...
  describe('Braceless Loop Bypass', () => {
    ...
    it('should block braceless nested loops (critical vulnerability)', async () => {
      ...
      // Use 100M iterations (10000 * 10000) to ensure timeout
      // 1M iterations completes too fast on modern hardware
      const result = await sandbox.execute(`
        let x = 0
      ...
      // AFTER FIX: Should timeout (100M iterations)
      expect(result.success).toBe(false)
      expect(result.error).toMatch(/timeout|CPU time limit/i)
    })
