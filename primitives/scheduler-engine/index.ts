 ...
 *
 * Scheduler Engine Primitive - Enterprise-grade job scheduling for the dotdo platform.
 *
 * Provides cron-based scheduling, interval jobs, one-time execution, retries with
 * backoff, timeouts, priority queuing, and timezone-aware scheduling. Built for
 * Durable Objects with support for concurrent job limits and overlap prevention.
 *
 * ## Features
 *
 * - **Cron expressions** with full 5-field syntax support
 * - **Interval scheduling** for recurring tasks
 * - **One-time jobs** for scheduled future execution
 * - **Retry with backoff** for resilient job execution
 * - **Timeout handling** with automatic cancellation
 * - **Priority queuing** for job ordering
 * - **Concurrency limits** to prevent resource exhaustion
 * - **Overlap prevention** to avoid duplicate runs
 * - **Timezone support** for global deployments
 *
 * @example Basic Job Scheduling
 * ```typescript
 * import { SchedulerEngine } from 'dotdo/primitives/scheduler-engine'
 *
 * const scheduler = new SchedulerEngine({ concurrency: 5 })
 *
 * // Schedule a cron job (every day at 9am)
 * scheduler.schedule({
 *   id: 'daily-report',
 *   name: 'Daily Report',
 *   schedule: { cron: '0 9 * * *' },
 ...
 * })
 *
 * // Schedule an interval job (every 5 minutes)
 * scheduler.schedule({
 *   id: 'health-check',
 *   name: 'Health Check',
 *   schedule: { interval: 5 * 60 * 1000 },
 *   handler: async () => await checkServices(),
 * })
 *
 * scheduler.start()
 * ```
 ...
 *   schedule: { at: new Date('2024-12-25T00:00:00Z') },
 *   handler: async () => await launchCampaign(),
 * })
 * ```
 *
 * @example Job with Retry and Timeout
 * ```typescript
 * scheduler.schedule({
 *   id: 'sync-data',
 ...
 * ```typescript
 * // High priority jobs run first when scheduler ticks
 * scheduler.schedule({
 *   id: 'critical-alerts',
 *   name: 'Critical Alerts',
 *   schedule: { interval: 60000 },
 *   options: { priority: 10 },
 *   handler: checkCriticalAlerts,
 * })
 *
 * scheduler.schedule({
 *   id: 'analytics',
 ...
 * ```typescript
 * // Pause a job
 * scheduler.pause('sync-data')
 *
 * // Resume a job
 * scheduler.resume('sync-data')
 *
 * // Manual trigger
 * const execution = await scheduler.run('daily-report')
 *
 * // Check status
 * const status = scheduler.getStatus('daily-report')
 * console.log(status.nextRun, status.lastRun, status.paused)
 *
 * // Get execution history
 * const history = scheduler.getHistory('daily-report')
 * ```
 *
