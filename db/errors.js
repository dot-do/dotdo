 ...
 */
export function getHttpStatusForCode(code) {
...
export class DbValidationError extends DatabaseError {
    ...
    static forField(field, constraint, value) {
        return new DbValidationError(`Validation failed: ${field} ${constraint}`, { field, constraint, value });
    }
}
/**
...
export class DbNotFoundError extends DatabaseError {
    ...
    static forResource(resourceType, resourceId) {
        return new DbNotFoundError(`${resourceType} with id ${resourceId} not found`, { resourceType, resourceId });
    }
}
/**
...
export class TransactionError extends DotdoError {
     ...
     */
    static rollbackFailed(originalError, rollbackError) {
        return new TransactionError(`Transaction rollback failed: ${rollbackError.message}`, rollbackError, {
     ...
     */
    static nestedFailed(savepointName, error) {
        return new TransactionError(`Nested transaction '${savepointName}' failed: ${error.message}`, error, { savepointName });
    }
}
/**
 ...
 */
export function serializeDotdoError(error, options = {}) {
 ...
 */
export function serializeUnknownAsDotdoError(error, options = {}) {
    if (error instanceof DotdoError) {
...
export function deserializeDotdoError(serialized) {
    ...
    const error = new DotdoError(serialized.code ?? ErrorCode.INTERNAL_ERROR, serialized.message, {
        ...(serialized.details !== undefined && { details: serialized.details }),
    });
    if (serialized.stack) {
        error.stack = serialized.stack;
 ...
 */
export function isSerializedDotdoError(value) {
