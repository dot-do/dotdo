...
import { buildWhereClause, buildOrderByClause, buildPaginationClause } from './query';
import { applyCursorPagination, encodeCursor, decodeCursor } from './pagination';
import { MigrationRunner, coreMigrations } from './migrations';
    ...
    async initialize() {
      ...
      `);
            ...
            if (result.errors.length > 0) {
                ...
                if (error) {
                    throw new Error(`Migration failed: ${error.name} (version ${error.version}): ${error.error}`);
                }
            }
        }
 ...
 */
export function createSQLiteThingsStore(adapter) {
    ...
    return {
        async create(data) {
            ...
            await sql
                .prepare('INSERT INTO things (id, type, data, created_at, updated_at) VALUES (?, ?, ?, ?, ?)')
 ...
 */
export function createSQLiteEventsStore(adapter) {
    const sql = adapter.getSql();
    ...
    const defaultDurabilityConfig = { retries: 3, backoff: 'exponential' };
    return {
        async emit(data) {
            ...
            await sql
                .prepare('INSERT INTO events (id, type, payload, timestamp, source, correlation_id) VALUES (?, ?, ?, ?, ?, ?)')
                .bind(event.$id, event.type, payloadJson, event.$timestamp, event.source || null, event.correlationId || null)
                .run();
            // Notify subscribers
        ...
        async get(id) {
            const row = await sql
                .prepare('SELECT id, type, payload, timestamp, source, correlation_id FROM events WHERE id = ?')
                .bind(id)
                .first();
            if (!row)
                return null;
        ...
        async getStorageUsage() {
            ...
            const sampleResult = await sql
                ...
                .bind()
                .first();
            const avgPayloadSize = sampleResult?.['avg_size'] ?? 100;
        ...
        async addToDeadLetterQueue(entry) {
           ...
           VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`)
                .bind(id, entry.event.$id, entry.event.type, JSON.stringify(entry.event.payload), entry.event.$timestamp, entry.event.source || null, entry.event.correlationId || null, entry.attempts, entry.lastError, entry.handlerIndex ?? null, timestamp)
                .run();
        },
        async getDeadLetterQueue() {
            const result = await sql
                .prepare(`SELECT id, event_id, event_type, event_payload, event_timestamp, event_source, event_correlation_id, attempts, last_error, handler_index, timestamp
           ...
           ORDER BY timestamp DESC`)
            ...
            return result.results.map((row) => {
                ...
                if (row['event_correlation_id']) {
                    event.correlationId = row['event_correlation_id'];
                }
                return {
                    event,
                    attempts: row['attempts'],
                    lastError: row['last_error'],
                    handlerIndex: row['handler_index'],
                    timestamp: row['timestamp']
                };
            });
        },
        async queryDeadLetterQueue(options) {
        ...
        async getDLQStats() {
            ...
            const stats = {
                ...
                uniqueEvents: uniqueRow?.['unique_count'] || 0
            };
            if (oldestRow?.['oldest'] != null) {
                stats.oldestEntry = oldestRow['oldest'];
            }
            if (newestRow?.['newest'] != null) {
                stats.newestEntry = newestRow['newest'];
            }
            return stats;
        },
        async getDLQEntry(eventId) {
            const row = await sql
                .prepare(`SELECT id, event_id, event_type, event_payload, event_timestamp, event_source, event_correlation_id, attempts, last_error, handler_index, timestamp
        ...
        async cleanupDeadLetterQueue(options) {
            const result = {
                removed: 0,
                removedByType: {}
            };
            ...
            for (const row of countResult.results) {
                result.removedByType[row['event_type']] = row['count'];
                result.removed += row['count'];
            }
        ...
        async addValidationFailure(failure) {
            ...
            const timestamp = Date.now();
            await sql
                .prepare(`INSERT INTO validation_failures (id, type, payload, error, details, timestamp)
           VALUES (?, ?, ?, ?, ?, ?)`)
                .bind(id, failure.type, JSON.stringify(failure.payload), failure.error, failure.details ? JSON.stringify(failure.details) : null, timestamp)
                .run();
        },
        ...
        async setEventRetryStatus(eventId, status) {
            ...
            await sql
                .prepare(`INSERT OR REPLACE INTO event_retry_status (event_id, attempts, succeeded, last_attempt, errors)
           VALUES (?, ?, ?, ?, ?)`)
                .bind(eventId, status.attempts, status.succeeded ? 1 : 0, status.lastAttempt, status.errors ? JSON.stringify(status.errors) : null)
                .run();
        },
        ...
        async recordRetryAttempt(eventType, succeeded, retryCount) {
            ...
            if (existing) {
             ...
             WHERE event_type = ?`)
                ...
                // Insert new entry
                await sql
                    .prepare(`INSERT INTO retry_metrics (event_type, total_events, total_retries, successes)
             VALUES (?, 1, ?, ?)`)
 ...
 */
export function createSQLiteRelationshipsStore(adapter) {
    const sql = adapter.getSql();
    return {
        async add(rel) {
            // Check for duplicate
            const existing = await sql
                .prepare('SELECT 1 FROM relationships WHERE subject = ? AND predicate = ? AND object = ?')
            ...
            await sql
                .prepare('INSERT INTO relationships (subject, predicate, object, created_at) VALUES (?, ?, ?, ?)')
                .bind(relationship.subject, relationship.predicate, relationship.object, relationship.$createdAt)
                .run();
            return relationship;
        },
        async remove(rel) {
        ...
        async findWithCursor(options = {}) {
            ...
            const getRelId = (rel) => `${rel.subject}:${rel.predicate}:${rel.object}`;
            rels.sort((a, b) => {
            ...
            return applyCursorPagination(rels, { cursor, limit, direction }, '$createdAt', 'desc', getRelId, (rel) => rel.$createdAt);
        },
