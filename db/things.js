...
import { validateThingInput, validateThingUpdate, validateId, validateIds, validateListOptions, validateBulkUpdateItems, createValidationContext, } from './validation';
 ...
 * @since 1.0.0
 */
export function createThingsStoreWithAdapter(adapter) {
    return {
        async create(data) {
        ...
        async get(id) {
            // Validate ID (do-c8s8)
            validateId(id, '$id');
            const thing = await adapter.get(`${THINGS_PREFIX}${id}`);
            return thing ?? null;
        },
        async update(id, data) {
            ...
            const existing = await adapter.get(`${THINGS_PREFIX}${id}`);
            if (!existing) {
        ...
        async list(options = {}) {
            ...
            const result = await adapter.list({ prefix: THINGS_PREFIX, includeValues: true });
            let items = Array.from(result.entries.values()).filter((t) => t !== undefined);
            if (type) {
 ...
 * ```
 */
export function createThingsStoreWithContext(adapter, validationContext) {
    ...
    return {
        ...
        async bulkUpdate(items) {
            ...
            const validatedItems = ctx.validateBulkUpdateItems(items);
            // Get all existing items first
            const keys = validatedItems.map(({ id }) => `${THINGS_PREFIX}${id}`);
            const existingMap = await adapter.getMany(keys);
            // Validate all items exist
 ...
 * @removal v4.0.0
 */
export function createThingsStore() {
    // Runtime deprecation warning
    if (typeof console !== 'undefined' && console.warn) {
        console.warn('[DEPRECATION] createThingsStore() is deprecated and will be removed in v4.0.0. ' +
    ...
    return {
        ...
        async listWithCursor(options = {}) {
            ...
            return applyCursorPagination(results, options, '$createdAt', 'desc', (item) => item.$id, (item) => item.$createdAt);
        },
        async bulkCreate(items) {
            if (items.length === 0) {
                return [];
