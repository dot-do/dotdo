...
## Known Code Locations Using Blocked Statements

The following files use `BEGIN`/`COMMIT`/`ROLLBACK` and will fail in production. Each has a warning comment referencing this documentation:

| File | Method | Status |
|------|--------|--------|
| `db/sqlite.ts` | `SQLiteAdapter.transaction()` | Warning comment added |
| `gitx/src/do/object-store.ts` | `batchPut()` | Warning comment added |
| `fsx/storage/sqlite.ts` | `beginTransaction()` | Warning comment added |
| `fsx/do/module.ts` | `beginTransaction()` | Warning comment added |
| `primitives/packages/digital-objects/src/ns.ts` | Various methods | **Submodule - needs separate fix** |

### Submodule Notes

The `primitives/` directory is a git submodule. Transaction-related changes must be made in the primitives repository separately. The affected methods in `primitives/packages/digital-objects/src/ns.ts`:
- `bulkCreate()` (line ~1037)
- `bulkUpdate()` (line ~1081)
- `bulkDelete()` (line ~1121)
- `bulkUpsert()` (line ~1152)
...
- `do-stc2d.3` - Document transaction limitations and add code comments
