...
describe('PayloadAdapterTestHarness', () => {
  describe('harness creation', () => {
    ...
    it('should create isolated instances per call', async () => {
      ...
      await (harness1.adapter as any).create({ collection: 'posts', data: { title: 'Harness 1' } })

      // Harness 2 should not see harness 1's data
      const result = await (harness2.adapter as any).find({ collection: 'posts' })
  ...
  describe('CRUD operation tracking', () => {
    it('should track create operations for assertions', async () => {
      const harness = createPayloadAdapterHarness()
      await (harness.adapter as any).create({ collection: 'posts', data: { title: 'Test' } })
    ...
    it('should track find operations with query details', async () => {
      const harness = createPayloadAdapterHarness()
      await (harness.adapter as any).find({ collection: 'posts', where: { status: 'published' } })
    ...
    it('should track findOne operations', async () => {
      const harness = createPayloadAdapterHarness()
      await (harness.adapter as any).findOne({ collection: 'posts', id: 'post-123' })
    ...
    it('should track update operations', async () => {
      ...
      await (harness.adapter as any).update({
        collection: 'posts',
        id: '1',
        data: { title: 'Updated' },
      })
    ...
    it('should track delete operations', async () => {
      ...
      await (harness.adapter as any).delete({ collection: 'posts', id: '1' })
    ...
    it('should track count operations', async () => {
      const harness = createPayloadAdapterHarness()
      await (harness.adapter as any).count({ collection: 'posts' })
  ...
  describe('seed data', () => {
    it('should allow seeding initial data', async () => {
      ...
      const result = await (harness.adapter as any).findOne({ collection: 'posts', id: '1' })
    ...
    it('should seed multiple collections', async () => {
      ...
      const posts = await (harness.adapter as any).find({ collection: 'posts' })
      const users = await (harness.adapter as any).find({ collection: 'users' })
      const categories = await (harness.adapter as any).find({ collection: 'categories' })
  ...
  describe('reset functionality', () => {
    ...
    it('should clear all data on reset', async () => {
      ...
      const result = await (harness.adapter as any).find({ collection: 'posts' })
      expect(result.docs).toHaveLength(0)
    })
    ...
    it('should restore seed data on reset if specified', async () => {
      ...
      await (harness.adapter as any).create({ collection: 'posts', data: { title: 'New Post' } })
      harness.reset()
  ...
  describe('underlying store access', () => {
    ...
    it('should allow direct manipulation via stores', async () => {
      ...
      harness.things.create({
        $id: 'https://test.do/posts/direct-1',
        $type: 'https://test.do/posts',
        name: 'Direct Insert',
      })

      // Should be visible via things store
      const result = harness.things.get('https://test.do/posts/direct-1')
    ...
    it('should sync store changes with adapter view', async () => {
      ...
      // Update via things store
      const thingId = `${harness.config.namespace ?? 'https://test.do'}/posts/1`
      harness.things.update(thingId, { name: 'Updated via Store' })

      // Things store should see updated data
      const result = harness.things.get(thingId)
      expect(result?.name).toBe('Updated via Store')
    })
  })
  ...
  describe('adapter interface compliance', () => {
    it('should implement find with pagination', async () => {
      ...
      const result = await (harness.adapter as any).find({
        collection: 'posts',
        limit: 2,
        page: 1,
      })
    ...
    it('should implement findOne returning single document', async () => {
      ...
      const result = await (harness.adapter as any).findOne({
        collection: 'posts',
        id: '1',
      })
    ...
    it('should implement create returning created document', async () => {
      ...
      const result = await (harness.adapter as any).create({
        collection: 'posts',
    ...
    it('should implement update returning updated document', async () => {
      ...
      const result = await (harness.adapter as any).update({
        collection: 'posts',
        id: '1',
    ...
    it('should implement delete returning deleted document', async () => {
      ...
      const result = await (harness.adapter as any).delete({
        collection: 'posts',
        id: '1',
      })
      ...
      // Verify deleted
      const findResult = await (harness.adapter as any).findOne({ collection: 'posts', id: '1' })
    ...
    it('should implement count returning total count', async () => {
      ...
      const result = await (harness.adapter as any).count({ collection: 'posts' })
