...
import { DuplicateOrganizationError } from './errors'
...
/**
 * Error thrown when a cycle is detected in the organization hierarchy.
 * This prevents infinite loops during ancestor/descendant traversal.
 */
export class CycleDetectedError extends Error {
  constructor(
    message: string,
    public readonly cycleNodeId?: string
  ) {
    super(message)
...
export { DuplicateOrganizationError }
...
export interface OrganizationStore {
  ...
  getAncestors(orgId: string, maxDepth?: number): Promise<Organization[]>
 ...
 *
 * Uses a slug lock mechanism to prevent TOCTOU race conditions on create/update.
 * The lock ensures atomicity of the check-then-create/update operation.
 */
export class OrganizationStoreImpl implements OrganizationStore {
  /**
   * Static lock map to track slugs currently being created.
   * Key: slug, Value: Promise that resolves when operation completes.
   *
   * This prevents TOCTOU race conditions by ensuring only one create
   * operation for a given slug can proceed at a time.
   */
  private static slugLocks = new Map<string, Promise<void>>()

  constructor(private graph: GraphEngine) {}

  /**
   * Acquire a lock for a slug, waiting for any pending operation to complete.
   * Returns a release function that must be called when the operation is done.
   */
  private async acquireSlugLock(slug: string): Promise<() => void> {
    // Wait for any existing operation on this slug to complete
    while (OrganizationStoreImpl.slugLocks.has(slug)) {
      await OrganizationStoreImpl.slugLocks.get(slug)
    }
    ...
    OrganizationStoreImpl.slugLocks.set(slug, lockPromise)

    // Return the release function
    return () => {
      OrganizationStoreImpl.slugLocks.delete(slug)
      release!()
    }
  }
  ...
  async create(data: OrganizationInput): Promise<Organization> {
    ...
    // Acquire slug lock to prevent TOCTOU race condition
    const releaseLock = await this.acquireSlugLock(data.slug)

    try {
      // Check for duplicate slug (under lock)
      const existing = await this.findBySlug(data.slug)
      if (existing) {
        throw new DuplicateOrganizationError(data.slug)
      }
  ...
  async update(id: string, data: Partial<OrganizationInput>): Promise<Organization> {
    ...
    // If slug is being changed, acquire lock and check for uniqueness
    ...
    // No slug change, update directly
    const node = await this.graph.updateNode(id, data)
    return nodeToOrganization(node)
  }
  ...
  async getAncestors(orgId: string, maxDepth = 100): Promise<Organization[]> {
    ...
    while (ancestors.length < maxDepth) {
      ...
      if (visited.has(parent.id)) {
        throw new CycleDetectedError(
          `Organization hierarchy contains cycle: ${parent.id} already visited`,
          parent.id
        )
      }
  ...
  async getDescendants(orgId: string, maxDepth = 100): Promise<Organization[]> {
    const descendants: Organization[] = []
    const visited = new Set<string>([orgId])

    // BFS through children with cycle detection
    const queue = [orgId]
    while (queue.length > 0 && descendants.length < maxDepth) {
      ...
      for (const child of children) {
        ...
        if (visited.has(child.id)) {
          throw new CycleDetectedError(
            `Organization hierarchy contains cycle: ${child.id} already visited`,
            child.id
          )
        }
