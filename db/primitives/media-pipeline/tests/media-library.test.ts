...
describe('MediaLibrary', () => {
  ...
  // =============================================================================
  // CDN Operations Tests
  // =============================================================================

  describe('CDN operations', () => {
    let cdnLibrary: MediaLibrary

    beforeEach(() => {
      cdnLibrary = createMediaLibrary({
        bucket: mockBucket,
        cdn: {
    ...
    it('should generate CDN URL for media', async () => {
      const png = createTestPng()
      const added = await cdnLibrary.add(png, { filename: 'photo.png' })

      const cdnUrl = await cdnLibrary.getCDNUrl(added.id)

      expect(cdnUrl).toBeDefined()
      expect(cdnUrl.url).toContain('https://cdn.example.com')
      expect(cdnUrl.url).toContain('photo.png')
    })

    it('should include transformation parameters for images', async () => {
      const png = createTestPng()
      const added = await cdnLibrary.add(png, { filename: 'photo.png' })
      ...
      expect(cdnUrl.url).toContain('w=300')
      expect(cdnUrl.url).toContain('h=200')
      expect(cdnUrl.url).toContain('fit=cover')
      expect(cdnUrl.url).toContain('q=80')
      expect(cdnUrl.url).toContain('f=webp')
    })

    it('should add disposition parameter', async () => {
      const png = createTestPng()
      const added = await cdnLibrary.add(png, { filename: 'photo.png' })
      ...
      expect(cdnUrl.url).toContain('disposition=attachment')
      expect(cdnUrl.url).toContain('filename=download.png')
    })
    ...
    it('should include cache key when caching is enabled', async () => {
      ...
      expect(cdnUrl.cacheKey).toBeDefined()
      expect(cdnUrl.cacheKey).toContain(added.id)
    })

    it('should generate signed URLs when signing key is configured', async () => {
      const signedLibrary = createMediaLibrary({
        bucket: mockBucket,
        cdn: {
          baseUrl: 'https://cdn.example.com',
      ...
      const png = createTestPng()
      const added = await signedLibrary.add(png, { filename: 'photo.png' })

      const cdnUrl = await signedLibrary.getCDNUrl(added.id, {
        expiresIn: 3600,
      })

      expect(cdnUrl.url).toContain('expires=')
      expect(cdnUrl.url).toContain('sig=')
      expect(cdnUrl.expiresAt).toBeDefined()
    })

    it('should throw for non-existent media', async () => {
      await expect(
        cdnLibrary.getCDNUrl('non-existent-id')
      ).rejects.toThrow(/not found/i)
    })
    ...
    it('should invalidate cache without error', async () => {
      ...
      // Should not throw
      await cdnLibrary.invalidateCache(added.id)
    })
  })

  // =============================================================================
  // Transcoding Operations Tests
  // =============================================================================

  describe('transcoding operations', () => {
    let transcodeLibrary: MediaLibrary

    beforeEach(() => {
      transcodeLibrary = createMediaLibrary({
        bucket: mockBucket,
        transcoding: {
          enabled: true,
          provider: 'worker',
     ...
     */
    function createTestMp4(): Uint8Array {
      // Minimal ftyp box for MP4 detection
      return new Uint8Array([
    ...
    it('should start a transcoding job', async () => {
      const mp4 = createTestMp4()
      const added = await transcodeLibrary.add(mp4, { filename: 'video.mp4' })

      const job = await transcodeLibrary.transcode(added.id, {
        format: 'webm',
        quality: 'high',
      })
      ...
      expect(job.sourceId).toBe(added.id)
      // Job starts in pending but immediately transitions to processing
      expect(['pending', 'processing']).toContain(job.status)
      expect(job.format).toBe('webm')
      expect(job.options.quality).toBe('high')
    })

    it('should get transcode job by ID', async () => {
      const mp4 = createTestMp4()
      const added = await transcodeLibrary.add(mp4, { filename: 'video.mp4' })

      const job = await transcodeLibrary.transcode(added.id)
      const retrieved = await transcodeLibrary.getTranscodeJob(job.id)
    ...
    it('should return null for non-existent job', async () => {
      const result = await transcodeLibrary.getTranscodeJob('non-existent')
      expect(result).toBeNull()
    })

    it('should list transcode jobs', async () => {
      const mp4 = createTestMp4()
      const video1 = await transcodeLibrary.add(mp4, { filename: 'video1.mp4' })
      const video2 = await transcodeLibrary.add(mp4, { filename: 'video2.mp4', overwrite: false })

      await transcodeLibrary.transcode(video1.id)
      await transcodeLibrary.transcode(video2.id)

      const jobs = await transcodeLibrary.listTranscodeJobs()
      expect(jobs.length).toBe(2)
    })

    it('should filter jobs by sourceId', async () => {
      const mp4 = createTestMp4()
      const video1 = await transcodeLibrary.add(mp4, { filename: 'video1.mp4' })
      const video2 = await transcodeLibrary.add(mp4, { filename: 'video2.mp4', overwrite: false })
      ...
      const jobs = await transcodeLibrary.listTranscodeJobs({ sourceId: video1.id })
      expect(jobs.length).toBe(1)
      expect(jobs[0].sourceId).toBe(video1.id)
    })
    ...
    it('should cancel a pending job', async () => {
      ...
      const job = await transcodeLibrary.transcode(added.id)
      await transcodeLibrary.cancelTranscodeJob(job.id)

      const updated = await transcodeLibrary.getTranscodeJob(job.id)
      expect(updated?.status).toBe('cancelled')
    })

    it('should throw when cancelling non-existent job', async () => {
      await expect(
        transcodeLibrary.cancelTranscodeJob('non-existent')
      ).rejects.toThrow(/not found/i)
    })

    it('should throw when transcoding is disabled', async () => {
      const disabledLibrary = createMediaLibrary({
        bucket: mockBucket,
        transcoding: {
          enabled: false,
        },
      ...
      const mp4 = createTestMp4()
      const added = await disabledLibrary.add(mp4, { filename: 'video.mp4' })

      await expect(
        disabledLibrary.transcode(added.id)
      ).rejects.toThrow(/not enabled/i)
    })

    it('should throw when transcoding non-video media', async () => {
      const png = createTestPng()
      const added = await transcodeLibrary.add(png, { filename: 'image.png' })

      await expect(
        transcodeLibrary.transcode(added.id)
      ).rejects.toThrow(/only supported for video\/audio/i)
    })
    ...
    it('should use default format when not specified', async () => {
      ...
      const job = await transcodeLibrary.transcode(added.id)
      expect(job.format).toBe('mp4') // default format from config
    })

    it('should support HLS format with adaptive outputs', async () => {
      const mp4 = createTestMp4()
      const added = await transcodeLibrary.add(mp4, { filename: 'video.mp4' })
      ...
      expect(job.format).toBe('hls')
      // After completion, job would have outputs
      // (simulated in the async processor)
    })

    it('should limit job list results', async () => {
      const mp4 = createTestMp4()
      const video = await transcodeLibrary.add(mp4, { filename: 'video.mp4' })

      await transcodeLibrary.transcode(video.id)
      await transcodeLibrary.transcode(video.id)
      await transcodeLibrary.transcode(video.id)

      const jobs = await transcodeLibrary.listTranscodeJobs({ limit: 2 })
      expect(jobs.length).toBe(2)
    })
  })
})
