...
function createLargePng(): Uint8Array {
  ...
  // Combine
  // PNG signature (8) + IHDR chunk (4 length + 4 type + 13 data + 4 crc) + IDAT chunk (4 length + 4 type + data + 4 crc) + IEND chunk (4 length + 4 type + 4 crc)
  const output = new Uint8Array(
    8 + // signature
    4 + 4 + 13 + 4 + // IHDR: length + type + data + crc
    4 + idatChunk.length + 4 + // IDAT: length + chunk (type+data) + crc
    4 + iendChunk.length + 4 // IEND: length + chunk (type) + crc
  )
  let offset = 0
...
describe('MediaLibrary', () => {
  ...
  describe('search', () => {
    ...
    it('should sort by uploadedAt desc by default', async () => {
      ...
      expect(result.items.length).toBe(3)
      // Latest uploaded should be first
      expect(result.items[0].filename).toBe('doc1.png')
    })
