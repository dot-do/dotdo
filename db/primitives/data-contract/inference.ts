...
interface FieldStats {
  ...
  stringCount: number // Total count of string values (for cardinality ratio)
...
export class SchemaInferrer {
   ...
   * Note: This public method uses values.length for cardinality, which is approximate.
   * For accurate cardinality calculation during inference, use detectStringConstraintsFromStats internally.
   */
  detectStringConstraints(values: string[]): StringConstraints {
    ...
    // For public API, we use values.length as an approximation of total count
  ...
  /**
   * Internal method to detect string constraints using accurate stats
   */
  private detectStringConstraintsFromStats(stats: FieldStats, values: string[]): StringConstraints {
    const result: StringConstraints = {}
    ...
    // Enum detection - check cardinality using accurate total count
    const uniqueCount = stats.stringValues.size
    const totalCount = stats.stringCount
    const cardinalityRatio = totalCount > 0 ? uniqueCount / totalCount : 1

    if (
      uniqueCount <= this.options.enumThreshold &&
      cardinalityRatio <= this.options.enumCardinalityThreshold &&
      uniqueCount > 1 // Don't treat single value as enum
    ) {
      result.enumValues = Array.from(stats.stringValues).sort()
    }

    // Length constraints
    if (this.options.inferLengthConstraints && stats.stringLengths.length > 0) {
      result.minLength = Math.min(...stats.stringLengths)
      result.maxLength = Math.max(...stats.stringLengths)
    }
  ...
  private collectStats(value: unknown, stats: FieldStats): void {
    ...
    switch (type) {
      case 'string':
        ...
        stats.stringCount++ // Track total string occurrences for cardinality ratio
  ...
  private generateStringSchema(stats: FieldStats, isNullable: boolean): JSONSchema {
    ...
    const constraints = this.detectStringConstraintsFromStats(stats, values)
    ...
    // Apply enum - prefer enum over generic patterns like slug
    // Only skip enum for semantic patterns (email, url, uuid, date, etc.)
    const semanticPatterns = ['email', 'url', 'uuid', 'iso-date', 'iso-datetime', 'time', 'ipv4', 'ipv6', 'phone', 'credit-card']
    const isSemanticPattern = constraints.pattern && semanticPatterns.includes(constraints.pattern.type)

    if (constraints.enumValues && !isSemanticPattern) {
  ...
  private countEnums(stats: FieldStats): number {
    ...
    // Check if this field could be an enum using stats-based detection
    if (stats.stringValues.size > 0) {
      const constraints = this.detectStringConstraintsFromStats(stats, Array.from(stats.stringValues))
