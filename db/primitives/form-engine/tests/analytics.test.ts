...
describe('FormAnalytics', () => {
  ...
  describe('Collector Integration', () => {
    it('should create collector for automatic tracking', () => {
      ...
      const events = analytics.getEvents()
      // Only view, start, and submit are events; field interactions are stored separately
      expect(events.length).toBe(3)

      // Verify field interactions were tracked separately
      const emailInteractions = analytics.getFieldInteractions('email')
      expect(emailInteractions.length).toBe(3) // focus, blur, change
    })
