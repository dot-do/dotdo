...
export interface ProblematicField {
  field: string
  issue: 'high_error_rate' | 'high_abandonment' | 'slow'
  value: number
}
...
export class FormAnalytics {
  ...
  getProblematicFields(): ProblematicField[] {
    ...
    for (const [field, rate] of Object.entries(errorRates)) {
      if (rate > 20) {
        problematic.push({ field, issue: 'high_error_rate', value: rate })
      }
    }

    for (const [field, count] of Object.entries(dropOff)) {
      if (count > 3) {
        problematic.push({ field, issue: 'high_abandonment', value: count })
      }
    }

    const avgTime = Object.values(timePerField).reduce((a, b) => a + b, 0) / Object.keys(timePerField).length || 0
    for (const [field, time] of Object.entries(timePerField)) {
      if (time > avgTime * 3) {
        problematic.push({ field, issue: 'slow', value: time })
      }
    }
