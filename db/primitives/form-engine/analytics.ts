...
export class FormAnalytics {
  ...
  getFunnel(): FunnelStep[] {
    ...
    // Get sorted step order for calculating drop-offs
    const stepOrder = Array.from(stepCompletions.keys()).sort()

    // Calculate drop-off for 'start' step (difference between starts and first step or submissions)
    const firstStepCount = stepOrder.length > 0
      ? stepCompletions.get(stepOrder[0])!
      : submissions
    const startDropOff = starts - firstStepCount
    ...
    funnel.push({
      step: 'complete',
      count: completions,
      dropOff: 0, // Final step has no drop-off (nowhere else to go)
