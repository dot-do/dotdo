...
describe('TamperAlertNotifier', () => {
  ...
  describe('sendAlert', () => {
    ...
    it('should handle send failure gracefully', async () => {
      ...
      // The notification channel captures errors internally, so success will be false
      // but the error may be in the delivery result rather than on the top level
      expect(result.success).toBe(false)
      // Check either top-level error or failed channels in delivery result
      const hasError = result.error !== undefined ||
        (result.deliveryResult && result.deliveryResult.failedChannels.length > 0)
