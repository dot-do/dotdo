...
// Import helper functions for temporal nodes
import {
  asOf,
  before,
  temporalBetween,
...
describe('UnifiedAST', () => {
  ...
  describe('TemporalNode', () => {
    describe('AS OF queries', () => {
      it('should represent AS OF timestamp query', () => {
        ...
        expect(node.type).toBe('temporal')
        expect(node.queryType).toBe('AS_OF')
        expect(node.asOfTimestamp).toBe(timestamp)
      })

      it('should create AS OF node with asOf helper', () => {
        const timestamp = new Date('2024-01-15T10:00:00Z')
        const node = asOf(timestamp)

        expect(node.type).toBe('temporal')
        expect(node.queryType).toBe('AS_OF')
        expect(node.asOfTimestamp).toBe(timestamp.getTime())
      })

      it('should create AS OF node with epoch milliseconds', () => {
        const node = asOf(1705312800000)

        expect(node.asOfTimestamp).toBe(1705312800000)
      })

      it('should create AS OF node with table name', () => {
        const node = asOf(1705312800000, 'users')

        expect(node.tableName).toBe('users')
      })
    })

    describe('BEFORE queries', () => {
      it('should represent BEFORE timestamp query', () => {
        const timestamp = new Date('2024-01-15T10:00:00Z').getTime()
        const node: TemporalNode = {
          type: 'temporal',
          queryType: 'BEFORE',
        ...
        expect(node.queryType).toBe('BEFORE')
      })

      it('should create BEFORE node with before helper', () => {
        const timestamp = new Date('2024-01-15T10:00:00Z')
        const node = before(timestamp)
    ...
    describe('BETWEEN queries', () => {
      it('should represent BETWEEN timestamps query', () => {
        ...
        expect(node.queryType).toBe('BETWEEN')
        expect(node.asOfTimestamp).toBe(start)
        expect(node.toTimestamp).toBe(end)
      })

      it('should create BETWEEN node with temporalBetween helper', () => {
        const start = new Date('2024-01-01T00:00:00Z')
        const end = new Date('2024-01-31T23:59:59Z')
        const node = temporalBetween(start, end)

        expect(node.type).toBe('temporal')
        expect(node.queryType).toBe('BETWEEN')
        expect(node.asOfTimestamp).toBe(start.getTime())
        expect(node.toTimestamp).toBe(end.getTime())
      })
    })

    describe('version-based queries', () => {
      it('should represent version-based AS OF query', () => {
        const node: TemporalNode = {
          type: 'temporal',
          queryType: 'AS_OF',
          snapshotId: 12345,
        ...
        expect(node.snapshotId).toBe(12345)
      })

      it('should create version-based node with atVersion helper', () => {
        const node = atVersion(12345)
      ...
      it('should represent VERSIONS_BETWEEN query', () => {
        const node: TemporalNode = {
          type: 'temporal',
        ...
        expect(node.queryType).toBe('VERSIONS_BETWEEN')
        expect(node.fromVersion).toBe(100)
        expect(node.toVersion).toBe(200)
      })

      it('should create VERSIONS_BETWEEN node with versionsBetween helper', () => {
        const node = versionsBetween(100, 200)
    ...
    describe('type guards', () => {
      it('should identify temporal nodes with isTemporal', () => {
        ...
        expect(isTemporal(temporalNode)).toBe(true)
        expect(isTemporal(predicateNode)).toBe(false)
      })
    })

    describe('createTemporal helper', () => {
      it('should create generic temporal node', () => {
        const node = createTemporal('AS_OF', {
          asOfTimestamp: 1705312800000,
          tableName: 'users',
        })
