...
describe('ai-database Integration', () => {
  ...
  describe('Relationship Handling Integration', () => {
     ...
     *
     * IMPLEMENTATION: Uses createRelationshipProvider() from ai-integration
     */
    it('should support relationship traversal via verb actions', async () => {
      // Create a relationship provider
      ...
      // Create relationship using the provider
      await relationships.perform('employs', company.$id, customer.$id)

      // Get related entities (forward direction: company -> employees)
      const employees = await relationships.related(company.$id, 'employs', 'forward')

      expect(employees.length).toBe(1)
      expect(employees[0].id).toBe(customer.$id)

      // Test backward direction: who employs this customer?
      const employers = await relationships.related(customer.$id, 'employs', 'backward')
      expect(employers.length).toBe(1)
      expect(employers[0].id).toBe(company.$id)
    })
  })
  ...
  describe('Events API Integration', () => {
     ...
     *
     * IMPLEMENTATION: Uses createEventEmitter() and wrapWithEvents() from ai-integration
     */
    let doProvider: DigitalObjectsProvider
    let store: DigitalObjectsThingsStore
    ...
    beforeEach(async () => {
      ...
      // Create event emitter and wrap the store
      emitter = createEventEmitter()
      const baseStore = createDigitalObjectsAdapter(doProvider)
      store = wrapWithEvents(baseStore, emitter) as DigitalObjectsThingsStore
    })

    it('should emit events on entity creation', async () => {
      const eventHandler = vi.fn()

      // Subscribe to Customer.created events using the emitter
      const unsubscribe = emitter.on('Customer.created', eventHandler)
    ...
    it('should support custom event emission', async () => {
      // Use the emitter to emit custom events
      const event = await emitter.emit('Customer.signup', { plan: 'enterprise' })
