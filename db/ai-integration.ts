...
/**
 * Create an event emitter for entity lifecycle events
 *
 * @returns An EntityEventEmitter instance
 *
 * @example
 * ```typescript
 ...
 *
 * // Subscribe to events
 * const unsubscribe = emitter.on('Customer.created', (data) => {
 ...
 * ```
 */
export function createEventEmitter(): EntityEventEmitter {
 ...
 *
 * @param store - The ThingsStore to wrap
 * @param emitter - The event emitter to use
 * @returns A wrapped store that emits events
 *
 * @example
 * ```typescript
 * const emitter = createEventEmitter()
 ...
 * ```
 */
export function wrapWithEvents<T extends StorableData = StorableData>(
  store: ThingsStore<T>,
  ...
  return {
    async create(data) {
      ...
      await emitter.emit(`${result.$type}.created`, result)
      return result
    },
    ...
    async update(id, data) {
      ...
      await emitter.emit(`${result.$type}.updated`, result)
      return result
    },

    async delete(id) {
      // Get the entity first to know its type
      const entity = await store.get(id)
      await store.delete(id)
      if (entity) {
        await emitter.emit(`${entity.$type}.deleted`, { $id: id, $type: entity.$type })
      }
    },
...
/**
 * Relationship provider interface for verb-based relationships
 */
export interface RelationshipProvider {
  ...
  /** Get entities related by a verb */
  ...
  relationships(entityId: string): Promise<RelationshipRecord[]>
}

/**
 * Create a relationship provider for verb-based relationships
 *
 * @returns A RelationshipProvider instance
 *
 * @example
 * ```typescript
 ...
 *
 * // Get employees (forward direction)
 * const employees = await relationships.related(companyId, 'employs', 'forward')
 *
 * // Get employer (backward direction)
 * const employers = await relationships.related(customerId, 'employs', 'backward')
 * ```
 */
export function createRelationshipProvider(): RelationshipProvider {
  ...
  return {
    ...
    async related(entityId: string, verb: string, direction: 'forward' | 'backward' | 'both'): Promise<Array<{ id: string }>> {
      ...
      for (const record of records) {
        if (record.verb !== verb) continue

        if ((direction === 'forward' || direction === 'both') && record.fromId === entityId) {
          results.push({ id: record.toId })
        }

        if ((direction === 'backward' || direction === 'both') && record.toId === entityId) {
          results.push({ id: record.fromId })
        }
      }
    ...
    async relationships(entityId: string): Promise<RelationshipRecord[]> {
      return records.filter(r => r.fromId === entityId || r.toId === entityId)
    }
  }
}

/**
 * Extend a digital-objects provider with relationship traversal
 *
 * @param provider - The provider to extend
 * @returns Extended provider with perform/related methods
 */
export function extendWithRelationships<P extends Record<string, unknown>>(
  provider: P
): P & RelationshipProvider {
  const relationshipProvider = createRelationshipProvider()

  return {
    ...provider,
    perform: relationshipProvider.perform,
    related: relationshipProvider.related,
