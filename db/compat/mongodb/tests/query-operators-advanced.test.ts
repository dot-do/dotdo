...
describe('$text Operator (Full-Text Search)', () => {
  ...
  it('should search with phrase in quotes', async () => {
    ...
    // Only content field is indexed, so only documents with "Cloudflare Workers" in content match
    // Document 'Durable Objects' has "Cloudflare Workers platform" in its content
    expect(docs.length).toBe(1)
    expect(docs[0]!.title).toBe('Durable Objects')
  })
...
describe('$expr Operator', () => {
  ...
  it('should use $expr with $multiply', async () => {
    ...
    // price * quantity >= 500
    // 100*5=500, 200*2=400, 50*10=500, 150*3=450
    // Two documents have price*quantity >= 500: price=100 and price=50
    expect(docs.length).toBe(2)
  })
