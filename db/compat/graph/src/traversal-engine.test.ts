...
describe('TraversalEngine', () => {
  ...
  // ==========================================================================
  // EDGE CASE TESTS - RED PHASE (dotdo-pkrnk)
  // ==========================================================================

  describe('Edge Cases - Node as Both Source and Target', () => {
    /**
     * Critical edge case: A node that is both the source of the traversal
     * AND appears as a target node during traversal.
     *
     * Example: alice -> bob -> alice
     * When traversing from alice with depth 2, alice should NOT appear
     * in results because it's the source node.
     */
    describe('Source node reached via traversal', () => {
      it('should exclude source node even when reached via cycle', async () => {
        // Create a simple cycle: A -> B -> A
        const index = new AdjacencyIndex()
        const now = Date.now()
        index.addEdge({ id: 'r1', type: 'follows', from: 'A', to: 'B', createdAt: now })
        index.addEdge({ id: 'r2', type: 'follows', from: 'B', to: 'A', createdAt: now })
        ...
        // A should NOT appear in results - it's the source
        expect(result.ids).not.toContain('A')
        expect(result.ids).toContain('B')
        expect(result.ids).toHaveLength(1)
      })

      it('should handle source node that is target of multiple paths', async () => {
        // Create a graph where source can be reached from multiple paths
        //   A -> B -> C
        //   A -> D -> C -> A
        const index = new AdjacencyIndex()
        const now = Date.now()
        index.addEdge({ id: 'r1', type: 'link', from: 'A', to: 'B', createdAt: now })
        index.addEdge({ id: 'r2', type: 'link', from: 'B', to: 'C', createdAt: now })
        index.addEdge({ id: 'r3', type: 'link', from: 'A', to: 'D', createdAt: now })
        index.addEdge({ id: 'r4', type: 'link', from: 'D', to: 'C', createdAt: now })
        index.addEdge({ id: 'r5', type: 'link', from: 'C', to: 'A', createdAt: now }) // Points back to source
        ...
        expect(result.ids).not.toContain('A')
        // B, C, D should all be present
        expect(result.ids).toContain('B')
        expect(result.ids).toContain('C')
        expect(result.ids).toContain('D')
      })

      it('should correctly handle when multiple sources include nodes that are targets', async () => {
        // Start from [A, B], where B is reachable from A
        // A -> B -> C
        const index = new AdjacencyIndex()
        const now = Date.now()
        index.addEdge({ id: 'r1', type: 'link', from: 'A', to: 'B', createdAt: now })
        ...
        const result = await engine.bfs(['A', 'B'], {
          depth: 1,
          direction: 'out',
        })

        // B is a source, so it shouldn't appear in results
        // A is a source, so it shouldn't appear
        // C should appear (from B)
        // B would appear from A's traversal, but B is in source set
        expect(result.ids).not.toContain('A')
    ...
    describe('Self-loop handling', () => {
      it('should handle self-loops (node pointing to itself)', async () => {
        const index = new AdjacencyIndex()
        const now = Date.now()
        index.addEdge({ id: 'r1', type: 'link', from: 'A', to: 'A', createdAt: now }) // Self-loop
        index.addEdge({ id: 'r2', type: 'link', from: 'A', to: 'B', createdAt: now })
        ...
        // A's self-loop should not add A to results (it's the source)
        expect(result.ids).not.toContain('A')
        expect(result.ids).toContain('B')
        expect(result.ids).toHaveLength(1)
      })

      it('should handle self-loops with depth range including 0', async () => {
        const index = new AdjacencyIndex()
        const now = Date.now()
        index.addEdge({ id: 'r1', type: 'link', from: 'A', to: 'A', createdAt: now })
        ...
        // A should appear once (at depth 0), not twice
        expect(result.ids).toContain('A')
        expect(result.ids.filter(id => id === 'A')).toHaveLength(1)
      })
    })
  })

  describe('Edge Cases - Multi-hop BFS Correctness', () => {
    /**
     * Ensure BFS returns nodes at correct depths and maintains proper order.
     */
    describe('BFS level-order correctness', () => {
      it('should return nodes in BFS order (level by level)', async () => {
        // Tree structure:
        //       root
        //      /    \
        //     L1a   L1b
        ...
        const index = new AdjacencyIndex()
        const now = Date.now()
        index.addEdge({ id: 'r1', type: 'child', from: 'root', to: 'L1a', createdAt: now })
        index.addEdge({ id: 'r2', type: 'child', from: 'root', to: 'L1b', createdAt: now })
        index.addEdge({ id: 'r3', type: 'child', from: 'L1a', to: 'L2a', createdAt: now })
        index.addEdge({ id: 'r4', type: 'child', from: 'L1a', to: 'L2b', createdAt: now })
        index.addEdge({ id: 'r5', type: 'child', from: 'L1b', to: 'L2c', createdAt: now })
        ...
        // Level 1 nodes should have depth 1
        expect(result.depths!.get('L1a')).toBe(1)
        expect(result.depths!.get('L1b')).toBe(1)

        // Level 2 nodes should have depth 2
        expect(result.depths!.get('L2a')).toBe(2)
        expect(result.depths!.get('L2b')).toBe(2)
        expect(result.depths!.get('L2c')).toBe(2)
      })

      it('should assign minimum depth when node is reachable via multiple paths', async () => {
        // Diamond: A -> B -> D, A -> C -> D
        // D is reachable at depth 2 via both paths
        const index = new AdjacencyIndex()
        const now = Date.now()
        index.addEdge({ id: 'r1', type: 'link', from: 'A', to: 'B', createdAt: now })
        index.addEdge({ id: 'r2', type: 'link', from: 'A', to: 'C', createdAt: now })
        index.addEdge({ id: 'r3', type: 'link', from: 'B', to: 'D', createdAt: now })
        index.addEdge({ id: 'r4', type: 'link', from: 'C', to: 'D', createdAt: now })
        ...
        // D should appear at depth 2 (not duplicated)
        expect(result.depths!.get('D')).toBe(2)
        expect(result.ids.filter(id => id === 'D')).toHaveLength(1)
      })

      it('should handle complex multi-path graphs correctly', async () => {
        // Multiple paths of different lengths:
        // A -> B -> C -> D (depth 3)
        // A -> C (shortcut, depth 1)
        const index = new AdjacencyIndex()
        const now = Date.now()
        index.addEdge({ id: 'r1', type: 'link', from: 'A', to: 'B', createdAt: now })
        index.addEdge({ id: 'r2', type: 'link', from: 'B', to: 'C', createdAt: now })
        index.addEdge({ id: 'r3', type: 'link', from: 'C', to: 'D', createdAt: now })
        index.addEdge({ id: 'r4', type: 'link', from: 'A', to: 'C', createdAt: now }) // Shortcut
        ...
        // C should be at depth 1 (shortest path via shortcut)
        expect(result.depths!.get('C')).toBe(1)
        // B should be at depth 1
        expect(result.depths!.get('B')).toBe(1)
        // D should be at depth 2 (via C which is at depth 1)
        expect(result.depths!.get('D')).toBe(2)
      })
    })

    describe('Path tracking in multi-hop', () => {
      it('should track shortest paths correctly', async () => {
        // A -> B -> C
        const index = new AdjacencyIndex()
        const now = Date.now()
        index.addEdge({ id: 'r1', type: 'link', from: 'A', to: 'B', createdAt: now })
      ...
      it('should track BFS-first path in diamond graph', async () => {
        // Diamond: A -> B -> D, A -> C -> D
        // Path to D should be via first discovered route
        const index = new AdjacencyIndex()
        const now = Date.now()
        index.addEdge({ id: 'r1', type: 'link', from: 'A', to: 'B', createdAt: now })
        index.addEdge({ id: 'r2', type: 'link', from: 'A', to: 'C', createdAt: now })
        ...
        // D's path should be one of the valid paths
        const pathToD = result.paths!.get('D')
        expect(pathToD).toBeDefined()
        expect(pathToD![0]).toBe('A')
        expect(pathToD![pathToD!.length - 1]).toBe('D')
        expect(pathToD!.length).toBe(3) // A -> B/C -> D
      })
    })
  })

  describe('Edge Cases - Chaining Correctness', () => {
    /**
     * Test that chaining produces correct results for complex traversal patterns.
     */
    describe('Chain step isolation', () => {
      it('should not carry visited set between chain steps when disabled', async () => {
        // A -> B -> C, B -> A (cycle within)
        const index = new AdjacencyIndex()
        const now = Date.now()
        index.addEdge({ id: 'r1', type: 'step1', from: 'A', to: 'B', createdAt: now })
        index.addEdge({ id: 'r2', type: 'step2', from: 'B', to: 'C', createdAt: now })
        index.addEdge({ id: 'r3', type: 'step2', from: 'B', to: 'A', createdAt: now })

        const engine = createTraversalEngine(index)

        const result = await engine.chain(['A'], [
          { relType: 'step1', direction: 'out' },
          { relType: 'step2', direction: 'out' },
        ], { carryVisitedSet: false })

        // With no carried visited set, A could appear (it's not a source of step2)
        // But the implementation may vary - document the expected behavior
        expect(result.ids).toContain('C')
      })

      it('should carry visited set between chain steps when enabled', async () => {
        const index = new AdjacencyIndex()
        const now = Date.now()
        index.addEdge({ id: 'r1', type: 'step1', from: 'A', to: 'B', createdAt: now })
        index.addEdge({ id: 'r2', type: 'step2', from: 'B', to: 'C', createdAt: now })
        ...
        // A should NOT appear - it was the original source
        expect(result.ids).not.toContain('A')
        expect(result.ids).toContain('C')
      })

      it('should handle empty intermediate results gracefully in chain', async () => {
        const index = new AdjacencyIndex()
        const now = Date.now()
        index.addEdge({ id: 'r1', type: 'step1', from: 'A', to: 'B', createdAt: now })
        // No step2 edges from B

        const engine = createTraversalEngine(index)

        const result = await engine.chain(['A'], [
          { relType: 'step1', direction: 'out' },
          { relType: 'step2', direction: 'out' }, // No edges
          { relType: 'step3', direction: 'out' }, // Would never be reached
        ])
    ...
    describe('Chain with depth ranges', () => {
      it('should apply depth range to each step independently', async () => {
        // Linear: A -> B -> C -> D -> E
        const index = new AdjacencyIndex()
        const now = Date.now()
        index.addEdge({ id: 'r1', type: 'next', from: 'A', to: 'B', createdAt: now })
        index.addEdge({ id: 'r2', type: 'next', from: 'B', to: 'C', createdAt: now })
        index.addEdge({ id: 'r3', type: 'next', from: 'C', to: 'D', createdAt: now })
        index.addEdge({ id: 'r4', type: 'next', from: 'D', to: 'E', createdAt: now })

        const engine = createTraversalEngine(index)

        // Step 1: depth 2 from A -> should reach C
        // Step 2: depth 1 from C -> should reach D
        const result = await engine.chain(['A'], [
          { relType: 'next', direction: 'out', depth: 2 },
          { relType: 'next', direction: 'out', depth: 1 },
        ])

        // From A, depth 2 reaches C
        // From C, depth 1 reaches D
        expect(result.ids).toContain('D')
        expect(result.ids).not.toContain('A')
        expect(result.ids).not.toContain('B')
        expect(result.ids).not.toContain('E')
      })

      it('should handle depth range {min, max} in chain steps', async () => {
        // Linear: A -> B -> C -> D
        const index = new AdjacencyIndex()
        const now = Date.now()
        index.addEdge({ id: 'r1', type: 'next', from: 'A', to: 'B', createdAt: now })
        ...
        // Step 1: depth {1, 2} from A -> should reach B and C
        // Step 2: depth 1 from [B, C] -> should reach C (from B) and D (from C)
        const result = await engine.chain(['A'], [
          { relType: 'next', direction: 'out', depth: { min: 1, max: 2 } },
          { relType: 'next', direction: 'out', depth: 1 },
        ])

        // B at depth 1 -> C at depth 1 of step 2
        // C at depth 2 -> D at depth 1 of step 2
        expect(result.ids).toContain('D')
        // C might be included from B's traversal if not in visited set
      })
    })
  })

  describe('Edge Cases - Direction and Edge Type Combinations', () => {
    describe('Mixed direction traversals', () => {
      it('should handle $expand (both directions) correctly', async () => {
        // A -> B, C -> A
        const index = new AdjacencyIndex()
        const now = Date.now()
        index.addEdge({ id: 'r1', type: 'link', from: 'A', to: 'B', createdAt: now })
        index.addEdge({ id: 'r2', type: 'link', from: 'C', to: 'A', createdAt: now })

        const engine = createTraversalEngine(index)

        const result = await engine.$expand(['A'], 1)
      ...
      it('should handle bidirectional edges (A <-> B)', async () => {
        const index = new AdjacencyIndex()
        const now = Date.now()
        index.addEdge({ id: 'r1', type: 'link', from: 'A', to: 'B', createdAt: now })
        index.addEdge({ id: 'r2', type: 'link', from: 'B', to: 'A', createdAt: now })

        const engine = createTraversalEngine(index)

        // With $expand, B should appear once (not twice)
        const result = await engine.$expand(['A'], 1)

        expect(result.ids).toContain('B')
        expect(result.ids.filter(id => id === 'B')).toHaveLength(1)
      })
    })

    describe('Edge type filtering edge cases', () => {
      it('should return empty when filtering by non-existent edge type', async () => {
        const index = new AdjacencyIndex()
        const now = Date.now()
        index.addEdge({ id: 'r1', type: 'follows', from: 'A', to: 'B', createdAt: now })
        ...
        const result = await engine.bfs(['A'], {
          depth: 1,
          direction: 'out',
          edgeTypes: ['nonexistent'],
        })

        expect(result.ids).toEqual([])
      })

      it('should combine multiple edge types correctly', async () => {
        const index = new AdjacencyIndex()
        const now = Date.now()
        index.addEdge({ id: 'r1', type: 'follows', from: 'A', to: 'B', createdAt: now })
        index.addEdge({ id: 'r2', type: 'likes', from: 'A', to: 'C', createdAt: now })
        index.addEdge({ id: 'r3', type: 'blocks', from: 'A', to: 'D', createdAt: now })
        ...
        expect(result.ids).toContain('B')
        expect(result.ids).toContain('C')
        expect(result.ids).not.toContain('D') // blocks is not in filter
      })
    })
  })

  describe('Edge Cases - Large Graph Handling', () => {
    describe('High branching factor', () => {
      it('should handle nodes with many outgoing edges', async () => {
        const index = new AdjacencyIndex()
        const now = Date.now()

        // Create a hub with 100 spokes
        for (let i = 0; i < 100; i++) {
          index.addEdge({ id: `r${i}`, type: 'link', from: 'hub', to: `spoke-${i}`, createdAt: now })
        }

        const engine = createTraversalEngine(index)

        const result = await engine.bfs(['hub'], {
          depth: 1,
          direction: 'out',
        })
      ...
      it('should apply limit correctly on high branching graphs', async () => {
        const index = new AdjacencyIndex()
        const now = Date.now()
    ...
    describe('Deep graph handling', () => {
      it('should handle very deep graphs within depth limit', async () => {
        const index = new AdjacencyIndex()
        const now = Date.now()

        // Create a chain of 100 nodes
        for (let i = 0; i < 99; i++) {
          index.addEdge({ id: `r${i}`, type: 'next', from: `node-${i}`, to: `node-${i + 1}`, createdAt: now })
        }
        ...
        expect(result.ids).toContain('node-50')
        expect(result.ids).toHaveLength(1)
      })
    })
  })
  ...
  describe('Edge Cases - Empty and Null Handling', () => {
    it('should handle empty start vertices array', async () => {
      const index = new AdjacencyIndex()
      const engine = createTraversalEngine(index)
    ...
    it('should handle non-existent start vertex', async () => {
      const index = new AdjacencyIndex()
      const now = Date.now()
      index.addEdge({ id: 'r1', type: 'link', from: 'A', to: 'B', createdAt: now })
      ...
      const result = await engine.bfs(['nonexistent'], {
        depth: 1,
        direction: 'out',
      })
    ...
    it('should handle isolated nodes (no edges)', async () => {
      const index = new AdjacencyIndex()
      // No edges added - empty graph
      const engine = createTraversalEngine(index)

      const result = await engine.bfs(['isolated'], {
        depth: 1,
        direction: 'out',
      })
