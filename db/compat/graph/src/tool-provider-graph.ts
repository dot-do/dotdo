...
export interface ToolProviderGraph {
  ...
  updateRelationship(id: string, input: UpdateRelationshipInput): Promise<ThingRelationship | null>
...
export function createToolProviderGraph(options: ToolProviderGraphOptions): ToolProviderGraph {
  ...
  return {
     ...
     */
    async updateRelationship(id: string, input: UpdateRelationshipInput): Promise<ThingRelationship | null> {
      const updated = await storage.updateRelationship(id, input.data)

      if (!updated) {
        return null
      }

      return storedRelToThingRel(updated)
    },
...
// All types are already exported via interface/type declarations above
