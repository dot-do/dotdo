...
describe('compat/ Provider Integration', () => {
  ...
  beforeEach(async () => {
    graph = await createToolProviderGraph({ namespace: 'test-compat-integration' })
  })
  ...
  describe('real compat/ provider registration', () => {
    it('creates SendGrid provider from compat/sendgrid', async () => {
      // Import from compat/sendgrid - verifies real compat/ SDK exists
      const { MailService } = await import('../../../../compat/sendgrid')
      expect(MailService).toBeDefined()

      // Create provider Thing with SendGrid metadata
      const sendgridProvider = await graph.create({
        $type: 'Provider',
        data: {
          name: 'SendGrid',
      ...
      expect(sendgridProvider.data.name).toBe('SendGrid')
      expect(sendgridProvider.data.compatModule).toBe('@dotdo/sendgrid')

      // Create tool Things for sendgrid operations
      const sendEmailTool = await graph.create({
        $type: 'Tool',
        data: {
          id: 'sendgrid.send',
          name: 'Send Email',
      ...
      const sendMultipleTool = await graph.create({
        $type: 'Tool',
        data: {
          id: 'sendgrid.sendMultiple',
          name: 'Send Multiple',
      ...
      await graph.createRelationship({ verb: 'providedBy', from: sendEmailTool.$id, to: sendgridProvider.$id })
      await graph.createRelationship({ verb: 'providedBy', from: sendMultipleTool.$id, to: sendgridProvider.$id })

      // Verify tools are linked
      const tools = await graph.getRelationshipsTo(sendgridProvider.$id, 'providedBy')
      expect(tools).toHaveLength(2)
    })

    it('creates Slack provider from compat/slack', async () => {
      // Import from compat/slack - verifies real compat/ SDK exists
      const { WebClient, App } = await import('../../../../compat/slack')
      expect(WebClient).toBeDefined()
      expect(App).toBeDefined()

      // Create provider Thing with Slack metadata
      const slackProvider = await graph.create({
        $type: 'Provider',
        data: {
          name: 'Slack',
      ...
      const postMessageTool = await graph.create({
        $type: 'Tool',
        data: {
      ...
      const conversationsTool = await graph.create({
        $type: 'Tool',
        data: {
      ...
      // Link tools to provider
      await graph.createRelationship({ verb: 'providedBy', from: postMessageTool.$id, to: slackProvider.$id })
      await graph.createRelationship({ verb: 'providedBy', from: conversationsTool.$id, to: slackProvider.$id })

      // Verify provider has tools
      const tools = await graph.getRelationshipsTo(slackProvider.$id, 'providedBy')
      expect(tools).toHaveLength(2)
      expect(slackProvider.data.capabilities).toContain('chat')
    })

    it('creates Stripe provider from compat/stripe', async () => {
      // Import from compat/stripe - verifies real compat/ SDK exists
      const { Stripe, Webhooks } = await import('../../../../compat/stripe')
      expect(Stripe).toBeDefined()
      expect(Webhooks).toBeDefined()

      // Create provider Thing with Stripe metadata
      const stripeProvider = await graph.create({
        $type: 'Provider',
        data: {
          name: 'Stripe',
      ...
      const createCustomerTool = await graph.create({
        $type: 'Tool',
        data: {
      ...
      const createPaymentIntentTool = await graph.create({
        $type: 'Tool',
        data: {
          id: 'stripe.paymentIntents.create',
          name: 'Create Payment Intent',
      ...
      const createSubscriptionTool = await graph.create({
        $type: 'Tool',
        data: {
          id: 'stripe.subscriptions.create',
      ...
      // Link tools to provider
      await graph.createRelationship({ verb: 'providedBy', from: createCustomerTool.$id, to: stripeProvider.$id })
      await graph.createRelationship({ verb: 'providedBy', from: createPaymentIntentTool.$id, to: stripeProvider.$id })
      await graph.createRelationship({ verb: 'providedBy', from: createSubscriptionTool.$id, to: stripeProvider.$id })

      // Verify provider has all tools
      const tools = await graph.getRelationshipsTo(stripeProvider.$id, 'providedBy')
      expect(tools).toHaveLength(3)
    })
  })
  ...
  describe('cross-provider tool organization', () => {
    it('organizes tools by category across providers', async () => {
      ...
      const sendgrid = await graph.create({
        $type: 'Provider',
        data: { name: 'SendGrid', category: 'email' },
      })

      const twilio = await graph.create({
        $type: 'Provider',
        data: { name: 'Twilio', category: 'communications' },
      })

      // Create email tools from different providers
      const sendgridSend = await graph.create({
        $type: 'Tool',
        data: { id: 'sendgrid.send', name: 'SendGrid Send', category: 'communication.email.send' },
      })

      const twilioSend = await graph.create({
        $type: 'Tool',
        data: { id: 'twilio.email.send', name: 'Twilio Send', category: 'communication.email.send' },
      })

      // Link to providers
      await graph.createRelationship({ verb: 'providedBy', from: sendgridSend.$id, to: sendgrid.$id })
      await graph.createRelationship({ verb: 'providedBy', from: twilioSend.$id, to: twilio.$id })

      // Query all email tools by category
      const emailTools = await graph.findByType('Tool', {
        where: { category: 'communication.email.send' },
      })
      ...
      for (const tool of emailTools) {
        const providerRels = await graph.getRelationshipsFrom(tool.$id, 'providedBy')
        expect(providerRels).toHaveLength(1)
      }
    })
    ...
    it('discovers tools by provider capability', async () => {
      ...
      const stripe = await graph.create({
        $type: 'Provider',
        data: {
          name: 'Stripe',
          category: 'payments',
      ...
      const createSubscription = await graph.create({
        $type: 'Tool',
        data: { id: 'stripe.subscriptions.create', name: 'Create Subscription', capability: 'subscriptions' },
      })

      const listSubscriptions = await graph.create({
        $type: 'Tool',
        data: { id: 'stripe.subscriptions.list', name: 'List Subscriptions', capability: 'subscriptions' },
      })

      const createInvoice = await graph.create({
        $type: 'Tool',
        data: { id: 'stripe.invoices.create', name: 'Create Invoice', capability: 'invoices' },
      })

      // Link to provider
      await graph.createRelationship({ verb: 'providedBy', from: createSubscription.$id, to: stripe.$id })
      await graph.createRelationship({ verb: 'providedBy', from: listSubscriptions.$id, to: stripe.$id })
      await graph.createRelationship({ verb: 'providedBy', from: createInvoice.$id, to: stripe.$id })

      // Query tools by capability
      const subscriptionTools = await graph.findByType('Tool', {
        where: { capability: 'subscriptions' },
      })

      expect(subscriptionTools).toHaveLength(2)
      expect(subscriptionTools.map(t => t.data.id)).toContain('stripe.subscriptions.create')
      expect(subscriptionTools.map(t => t.data.id)).toContain('stripe.subscriptions.list')
    })
  })
})
