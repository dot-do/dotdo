/**
 * capnweb RPC client for ClickHouse
 *
 * Provides a remote client that connects to a ClickHouse RPC server
...
/**
 * Create a capnweb RPC client for ClickHouse
 *
 * @param config - Client configuration
 * @returns A ClickHouseClient that communicates with a remote RPC server
 *
 * @example
 * ```typescript
 ...
 * ```
 */
export async function createRpcClient(config: RpcClientConfig): Promise<ClickHouseClient> {
  const {
    url,
    auth,
    timeout = 30000,
  ...
  async function sendRequest<T>(
      ...
      if (error instanceof Error) {
        if (error.name === 'AbortError') {
          if (options?.signal?.aborted) {
            throw new ClickHouseError('Request was cancelled');
          }
    ...
    try {
      ...
      throw new ClickHouseError('Invalid JSON response from RPC server');
    }
    ...
    if (rpcResponse.result === undefined) {
      throw new ClickHouseError('RPC response missing result');
    }
  ...
  async function* sendStreamingRequest<T>(
    ...
    try {
      ...
      if (error instanceof Error) {
        if (error.name === 'AbortError') {
          throw new ClickHouseError('Stream was cancelled');
        }
        throw new ConnectionError(`Failed to connect to RPC server: ${error.message}`);
      }
      throw new ConnectionError('Failed to connect to RPC server');
    ...
    if (!response.body) {
      throw new ClickHouseError('Response body is not available for streaming');
    }
  ...
  function parseStreamChunk<T>(line: string): StreamChunk<T> {
    try {
      ...
      throw new ClickHouseError(`Invalid stream chunk: ${line}`);
    }
  }
  ...
  const client: ClickHouseClient = {
    ...
    async insert<T extends Record<string, unknown>>(
      ...
      await sendRequest<{ success: boolean }>('insert', {
        table,
        data,
        options: options || {},
      });
    ...
    async ping(): Promise<boolean> {
      try {
        const result = await sendRequest<{ ok: boolean }>('ping', {});
        return result.ok;
      } catch {
        return false;
      }
