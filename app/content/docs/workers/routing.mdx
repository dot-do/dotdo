...
// Initialize ring once at module level for performance
const SHARD_COUNT = 16
const ring = createHashRing({ shardCount: SHARD_COUNT, virtualNodes: 150 })
...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    // hashKey creates composite key: 'ns:type:id'
...
All parsing functions return `null` for invalid routes. Internally, invalid URLs are caught and logged:
...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    // Handle invalid routes (no subdomain, apex domain, or invalid URL)
    if (!route) {
...
Routes return `null` in these situations:

| Pattern | Invalid Case | Example |
|---------|--------------|---------|
| Subdomain | No subdomain (apex domain) | `api.dotdo.dev/customers` |
| Subdomain | Hostname doesn't match root domain | `acme.other.com/customers` |
| Path | Empty path | `api.dotdo.dev/` |
| Path-Base | Base path doesn't match | `api.dotdo.dev/acme/customers` (expected `/v1`) |
| Path-Base | Nothing after base path | `api.dotdo.dev/v1/` |
