...
2. **Level authority**: Only the current approver or any previous approver can reject
...
The workflow validates all inputs with comprehensive error messages:

```typescript
// Required string fields must be non-empty
await workflow.request({
  id: '',           // Error: ApprovalRequest id is required
  type: '',         // Error: ApprovalRequest type is required
  title: '',        // Error: ApprovalRequest title is required
  description: '',  // Error: ApprovalRequest description is required
  requestedBy: '',  // Error: ApprovalRequest requestedBy is required
  requestedAt: new Date()
}, approvers)
...
// Amount must be non-negative when provided
await workflow.request({
  ...action,
  amount: -100  // Error: ApprovalRequest amount must be a non-negative number
}, approvers)

// Date must be valid
await workflow.request({
  ...action,
  requestedAt: new Date('invalid')  // Error: ApprovalRequest requestedAt must be a valid date
}, approvers)
...
Deadlines support natural language duration strings:

```typescript
// Minutes
'15 minutes', '30 min', '5m'
...
// Examples
await workflow.request(action, approvers, { deadline: '24 hours' })
await workflow.request(action, approvers, { deadline: '2 days' })
await workflow.request(action, approvers, { deadline: '30m' })
```

## Next Steps

- [Escalation Policies](./escalation) - Add automatic escalation to approvals
- [Review Queues](./queues) - Manage pending approvals in priority queues
- [Notifications](./notifications) - Send approval notifications
