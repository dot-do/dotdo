...
interface DirtyEntry {
  ...
  data: unknown    // The actual data to persist
...
### InMemoryStateManager Adapter

The `DOStorage` class provides an adapter to use `InMemoryStateManager` as a `DirtyTracker`:
...
// The InMemoryDirtyTracker adapter (built into DOStorage)
...
interface LazyCheckpointerOptions {
  ...
  memoryThresholdBytes?: number // Default: 10MB (10 * 1024 * 1024)
  columnarThreshold?: number   // Default: 1000 (for future storage optimization)
  onCheckpoint?: (stats: CheckpointStats) => void  // Callback after each checkpoint
}
```

### SqlStorage Interface

The SQLite storage must implement:
...
This matches the Cloudflare Durable Objects `ctx.storage.sql` interface.
...
The checkpointer writes to a `things` table. The `DOStorage` class may use a slightly different schema with version tracking:

### Standard Schema (LazyCheckpointer)
...
// DOStorage pattern (with version and timestamp):
sql.exec(
...
<Callout type="info">
  Partial checkpoints are safe because entries remain dirty until successfully written. The next checkpoint will retry all failed entries. No data is lost.
</Callout>
