...
<Callout type="warning">
  Fire-and-forget mode provides eventual durability. In rare failure scenarios (Pipeline service unavailable during emit), events may be lost. For mission-critical data, use `waitForPipeline: true` or implement application-level confirmation.
</Callout>
...
// Attempt 3: after 100ms delay
// (retryDelay is constant, not exponential)
```
...
// 2. If DLQ also fails, logged with error context
```

<Callout type="warning">
  If both the main Pipeline and DLQ fail, events are lost. The error is logged to console but not thrown to avoid blocking the application.
</Callout>
...
interface PipelineEmitterConfig {
  ...
  flushInterval?: number      // Default: 1000 (ms) - set to 0 for immediate mode
  batchSize?: number          // Default: 100 - events before forced flush
  maxRetries?: number         // Default: 3 - retry attempts on failure
  retryDelay?: number         // Default: 100 (ms) - delay between retries
  deadLetterQueue?: Pipeline  // Optional: separate Pipeline for failed events
}
```

### Pipeline Interface

The Pipeline binding must implement this interface:
...
The `close()` method:
1. Sets the closed flag to prevent new emits
2. Clears the flush timer
3. Flushes any remaining buffered events
4. Does not wait for in-flight flush promises (they complete in background)
...
<Callout type="info">
  The close() method is designed to not hang even if the Pipeline is slow. In-flight batches complete in the background while close() returns after flushing the buffer.
</Callout>
