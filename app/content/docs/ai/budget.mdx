...
<Callout type="info">
Budget tracking uses abstract cost units, not real currency. One unit equals one AI call at immediate priority.
</Callout>
...
```typescript
interface AIBudget {
  remaining: number  // Available budget units
  spent: number      // Total spent units
  limit: (amount: number) => AI  // Set new limit and reset spent
}
```
...
### Per-Model Pricing (Workers AI)

For real-world cost estimation, each model has tier-based pricing:

| Model | Tier | Input ($/M tokens) | Output ($/M tokens) |
|-------|------|-------------------|---------------------|
| GPT OSS 120B | Premium | $2.50 | $10.00 |
| GPT OSS 20B | Standard | $0.50 | $1.50 |
| Llama 4 Scout | Standard | $0.30 | $1.00 |
| Granite 4.0 Micro | Budget | $0.15 | $0.50 |

TTS and image models have flat per-request pricing.
...
class AIBudgetExceededError extends Error {
  ...
  requested: number  // Cost of attempted operation
}
...
try {
  ...
  if (error instanceof AIBudgetExceededError) {
    ...
    // error.message: "Budget exceeded: requested 1, spent 2, limit 2"
  }
}
```
...
By default, budget is unlimited (uses `Infinity` internally):

```typescript
import { ai } from 'dotdo/ai'

// Default instance has no limit
// Returns Number.MAX_SAFE_INTEGER - spent to avoid Infinity serialization issues
console.log(ai.budget.remaining) // 9007199254740990 (or similar large number)
...
To explicitly configure unlimited budget:

```typescript
const ai = createAI({ budget: { limit: Infinity } })
// or simply omit the budget option
const ai = createAI({})
```
...
- [Batch Processing](./batch) - Cost-effective batch operations
- [Providers](./providers) - Configure provider-specific pricing
