...
export function withBash<TBase extends Constructor<{ $: WorkflowContext }>>(
  ...
  return class WithBash extends Base {
    ...
    /** @internal */ declare _bashCapabilityCache: BashCapability | undefined

    /** @internal */ get bashCapability(): BashCapability {
      if (!this._bashCapabilityCache) {
        ...
        this._bashCapabilityCache = createBashCapability(
          executor,
          fs,
          config.useNativeOps ?? true
        )
      }
      return this._bashCapabilityCache
    }
