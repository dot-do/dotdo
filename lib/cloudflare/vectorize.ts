 ...
 * ```typescript
 * import { createVectorizeClient } from './vectorize'
 *
 ...
 *   { id: 'doc-1', values: newEmbedding, metadata: { updated: true } }
 * ])
 *
 * // Delete by IDs
 * await vectorize.delete(['doc-1', 'doc-2'])
 * ```
 *
 * @example Similarity search with metadata filtering
 * ```typescript
 * const vectorize = createVectorizeClient(env)
 *
 ...
 * ```typescript
 * import { createNamespacedVectorizeClient } from './vectorize'
 *
 * // Create tenant-specific client
 * const tenantVectorize = createNamespacedVectorizeClient(env, 'tenant-123')
 *
 * // All operations scoped to tenant-123 namespace
 * await tenantVectorize.insert([
 *   { id: 'product-1', values: embedding, metadata: { name: 'Widget' } }
 * ])
 *
 * // Or create namespaced client from existing client
 * const baseClient = createVectorizeClient(env)
 * const scopedClient = baseClient.withNamespace('tenant-456')
 * ```
 *
 * @example Semantic search with Workers AI
 * ```typescript
 * import { createSemanticSearchClient } from './vectorize'
 *
 * // Client with AI integration for text-to-vector conversion
 * const search = createSemanticSearchClient(env)
 *
 * // Search with natural language (embedding generated automatically)
 * const results = await search.semanticSearch('How do I configure authentication?', {
 *   topK: 5,
 *   filter: { category: 'documentation' },
 *   model: '@cf/baai/bge-base-en-v1.5' // Optional: custom model
 * })
 *
 * // Results include the generated query embedding
 * console.log('Query embedding dimension:', results.queryEmbedding.length)
 *
 * for (const match of results.matches) {
 *   console.log(`${match.score.toFixed(3)}: ${match.metadata?.title}`)
 * }
 * ```
 *
 * @example Batch operations and retrieval
 * ```typescript
 * const vectorize = createVectorizeClient(env, {
 *   batchSize: 500 // Process in chunks of 500
 * })
 *
 * // Insert large dataset in batches (handled automatically)
 * const vectors = generateEmbeddings(documents) // 10,000 vectors
 * const result = await vectorize.insert(vectors)
 * console.log(`Inserted ${result.count} vectors`)
 *
 * // Retrieve vectors by ID
 * const retrieved = await vectorize.getByIds(['doc-1', 'doc-2', 'doc-3'])
 * for (const v of retrieved) {
 *   console.log(`${v.id}: ${v.values.length} dimensions`)
 * }
 *
 ...
 * console.log(`Dimensions: ${info.config.dimensions}`)
 * console.log(`Metric: ${info.config.metric}`)
 * ```
 *
 ...
 * index_name = "my-index"
 *
 * # Optional: AI binding for semantic search
 * [ai]
 * binding = "AI"
 * ```
 */
