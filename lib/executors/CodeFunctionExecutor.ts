/**
 * @module lib/executors/CodeFunctionExecutor
 *
 * CodeFunctionExecutor - Execute TypeScript/JavaScript code-based functions.
 *
 * This executor runs user-provided handler functions in a controlled environment
 * with comprehensive security, resource management, and observability features.
 * It is the fastest and most deterministic execution mode in the cascade hierarchy.
 *
 * ## Features
 *
 * - **Context Injection**: Provides env, state, services, logging, and event emission
 * - **Sandboxed Execution**: Restricts access to dangerous globals (eval, process, require)
 * - **Timeout Handling**: Configurable execution timeouts with cancellation support
 * - **Retry Logic**: Multiple backoff strategies (fixed, exponential, linear, jitter)
 * - **Streaming Output**: Async iterator support for incremental results
 * - **Resource Limits**: Enforce memory, CPU time, and output size constraints
 * - **Schema Validation**: JSON schema validation for inputs and outputs
 *
 * ## Security Model
 *
 * When sandboxed mode is enabled (default), the executor:
 * - Blocks access to `eval`, `Function`, `require`, `process`, `global`
 * - Prevents prototype pollution attacks
 * - Replaces console with sandboxed logger
 * - Limits available globals to safe subset (JSON, Math, Date, etc.)
 *
 * @example Basic Execution
 * ```typescript
 * import { CodeFunctionExecutor } from 'dotdo/lib/executors'
 *
 ...
 * const handler = async (input: { userId: string }, ctx) => {
 *   ctx.log.info('Processing user', { userId: input.userId })
 *
 *   // Access persistent state
 *   const cached = await ctx.state.get(`user:${input.userId}`)
 ...
 *   )
 *
 *   await ctx.state.put(`user:${input.userId}`, user)
 *   return user
 * }
 *
 * // Execute with options
 * const result = await executor.execute(handler, { userId: 'u123' }, {
 *   timeout: 5000,
 *   sandboxed: true,
 *   retry: {
 *     maxAttempts: 3,
 ...
 * ```typescript
 * // Execute with streaming for incremental results
 * const stream = await executor.executeStreaming(
 *   async (input, ctx) => {
 *     for (const item of input.items) {
 *       await ctx.emit('stream.chunk', { data: processItem(item) })
 *     }
 *     return { processed: input.items.length }
 *   },
 *   { items: largeDataset },
 *   { functionId: 'process-items' }
 * )
 *
 * // Consume stream
 ...
 * ```typescript
 * // Validate input and output with JSON schemas
 ...
 *     maxOutputSize: 10 * 1024 * 1024, // 10MB output
 *   }
 * })
 *
 * if (!result.success && result.error instanceof ExecutionResourceError) {
 *   console.log('Resource limit exceeded:', result.error.message)
 * }
 * ```
 *
 * @example Retry with Custom Conditions
 * ```typescript
 ...
 * @see {@link CascadeExecutor} for automatic fallback to AI/human execution
 */
