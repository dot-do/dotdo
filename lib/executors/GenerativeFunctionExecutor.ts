/**
 * @module lib/executors/GenerativeFunctionExecutor
 *
 * GenerativeFunctionExecutor - Execute generative AI functions with LLM models.
 *
 * This executor calls LLM models (Claude, GPT, etc.) with prompt templates,
 * structured output schemas, streaming support, and comprehensive configuration
 * options. It bridges the gap between simple code execution and complex agentic
 * workflows by providing single-call AI inference.
 *
 * ## Features
 *
 * - **Multi-Model Support**: Claude, GPT-4, and other LLM providers
 * - **Prompt Templates**: Variable substitution with `{{variable}}` syntax
 * - **Structured Output**: JSON schema validation with automatic retries
 * - **Streaming**: Real-time token streaming with async iterators
 * - **Tool Integration**: Optional tool calling within generation
 * - **Conversation History**: Persistent multi-turn conversations
 * - **Rate Limit Handling**: Automatic retry with backoff on rate limits
 * - **Content Filtering**: Handle and report filtered content
 *
 * ## Model Configuration
 *
 ...
 * ```typescript
 * import { GenerativeFunctionExecutor } from 'dotdo/lib/executors'
 *
 * // Create executor with AI service
 * const executor = new GenerativeFunctionExecutor({
 *   state: this.state,
 *   env: this.env,
 *   aiService: {
 *     generate: (opts) => aiProvider.generate(opts),
 *     stream: (opts) => aiProvider.stream(opts),
 *     generateWithTools: (opts) => aiProvider.generateWithTools(opts)
 *   },
 ...
 * ```
 *
 * @example Prompt Templates with Variables
 * ```typescript
 ...
 *     topic: 'the upcoming product launch'
 *   }
 * })
 * ```
 *
 ...
 * }
 *
 * const result = await executor.execute<ProductAnalysis>({
 *   model: 'claude-sonnet-4-20250514',
 *   prompt: 'Analyze this product review: {{review}}',
 *   variables: { review: customerReview },
 *   schema: {
 *     type: 'object',
 *     properties: {
 ...
 *     },
 *     required: ['sentiment', 'keywords', 'summary', 'confidence']
 *   },
 *   coerceTypes: true,
 ...
 * }
 *
 * // Or collect full text
 * const fullText = await stream.toText()
 *
 * // Or parse as JSON
 * const jsonResult = await stream.toJSON<MySchema>()
 * ```
 *
 * @example Tool-Augmented Generation
 * ```typescript
 * // Use tools during generation (mini-agent mode)
 * const result = await executor.execute({
 *   model: 'claude-sonnet-4-20250514',
 *   prompt: 'What is the current weather in San Francisco and Tokyo?',
 *   tools: [
 *     {
 *       name: 'get_weather',
 ...
 *         required: ['city']
 *       },
 *       execute: async (input) => weatherService.get(input.city)
 *     }
 *   ],
 ...
 *   saveHistory: true
 * })
 * // result2.result will include "Alice"
 * ```
 *
 ...
 * ```
 *
 * @see {@link AgenticFunctionExecutor} for multi-step agentic workflows
