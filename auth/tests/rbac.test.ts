...
describe('RBAC - Role Hierarchy', () => {
  describe('ROLE_HIERARCHY', () => {
    it('should define hierarchy levels for all built-in roles', () => {
      expect(ROLE_HIERARCHY.guest).toBeDefined()
      expect(ROLE_HIERARCHY.viewer).toBeDefined()
      expect(ROLE_HIERARCHY.api).toBeDefined()
    ...
    it('should have correct ordering', () => {
      ...
      expect(ROLE_HIERARCHY.viewer).toBeLessThan(ROLE_HIERARCHY.api)
      expect(ROLE_HIERARCHY.api).toBeLessThan(ROLE_HIERARCHY.editor)
      expect(ROLE_HIERARCHY.editor).toBeLessThan(ROLE_HIERARCHY.owner)
  ...
  describe('isRoleAtLeast', () => {
    it('should return true when role A is higher', () => {
    ...
    it('should return false when role A is lower', () => {
  ...
  describe('getHighestRole', () => {
    it('should return the highest role from a list', () => {
      expect(getHighestRole(['viewer', 'editor', 'guest'])).toBe('editor')
      expect(getHighestRole(['super_admin', 'admin', 'editor'])).toBe('super_admin')
    ...
    it('should handle unknown roles', () => {
      expect(getHighestRole(['unknown', 'viewer'])).toBe('viewer')
      // Unknown roles have level -1 and are not valid highest roles
      expect(getHighestRole(['unknown'])).toBeUndefined()
    })
  })

  describe('userHasAtLeastRole', () => {
    it('should check user highest role against required role', () => {
      const adminUser: UserContext = { id: '1', roles: ['admin', 'viewer'] }
      const editorUser: UserContext = { id: '2', roles: ['editor'] }
      const guestUser: UserContext = { id: '3', roles: ['guest'] }

      expect(userHasAtLeastRole(adminUser, 'editor')).toBe(true)
      expect(userHasAtLeastRole(adminUser, 'admin')).toBe(true)
      expect(userHasAtLeastRole(adminUser, 'super_admin')).toBe(false)

      expect(userHasAtLeastRole(editorUser, 'viewer')).toBe(true)
      expect(userHasAtLeastRole(editorUser, 'admin')).toBe(false)

      expect(userHasAtLeastRole(guestUser, 'viewer')).toBe(false)
    })

    it('should return false for user with no roles', () => {
      const noRolesUser: UserContext = { id: '1', roles: [] }
      expect(userHasAtLeastRole(noRolesUser, 'guest')).toBe(false)
    })
  })
})
