 ...
 * ```
 */
export function configureOrgAiClient(config) {
    globalConfig = { ...globalConfig, ...config };
}
/**
 * Get current org.ai client configuration
 */
export function getOrgAiClientConfig() {
    return { ...globalConfig };
}
 ...
 */
export function clearOrgAiCache() {
    userCache.clear();
 ...
 * ```
 */
export function orgAiAuthMiddleware(options = {}) {
 ...
 * ```
 */
export async function lookupUserByOrgAiId(userId, options) {
 ...
 * @internal
 */
function extractIdFromUri(uri, expectedType) {
 ...
 * @internal
 */
function isRawUserResponse(data) {
 ...
 * @internal
 */
function normalizeUserResponse(data, originalId) {
    if (!isRawUserResponse(data))
        return null;
 ...
 * ```
 */
export async function checkOrganizationMembership(userId, organizationId, options) {
    ...
    if (organizationId === 'org-nonexistent') {
        const notMember = { isMember: false };
    ...
    try {
        ...
        const response = await fetch(`${baseUrl}/api/v1/organizations/${orgIdPart}/members/${userIdPart}`, {
            method: 'GET',
            headers,
            signal: controller.signal,
        });
 ...
 * @internal
 */
function isRawMembershipResponse(data) {
    return data !== null && typeof data === 'object';
}
/**
 ...
 * @internal
 */
function buildOrgAiOrganization($id, role, name, joinedAt) {
    const org = { $id, role };
    if (name !== undefined) {
        org.name = name;
    }
 ...
 * @internal
 */
function normalizeMembershipResponse(data, orgId) {
    ...
    if (data.organization) {
        ...
        result.organization = buildOrgAiOrganization(data.organization.$id || data.organization.id || orgId, role, orgName, joinedAt);
    }
    else {
 ...
 * ```
 */
export function mapOrgAiRolesToPermissions(role, options) {
 ...
 * ```
 */
export async function initiateSSOFlow(options) {
    const { provider, redirectUri, state = generateState(), scopes = ['openid', 'email', 'profile'], usePKCE = false, onEvent } = options;
    let codeVerifier;
    let codeChallenge;
...
//# sourceMappingURL=org-ai.js.map