...
export class MissingTokenError extends TokenValidationError {
  constructor(location: 'header' | 'cookie' | 'both' = 'header') {
    const messages = {
...
export class MissingSubjectError extends TokenValidationError {
  constructor() {
    super(
      'Token is missing required subject (sub) claim',
      TokenErrorCode.MISSING_SUBJECT,
      {
...
/**
 * Import jose error types for type checking
 */
...
export function mapJoseError(
  ...
  switch (errorCode) {
    case 'ERR_JWT_EXPIRED':
      ...
      const expPayload = (error as { payload?: JWTPayload }).payload
    ...
    case 'ERR_JWT_CLAIM_VALIDATION_FAILED': {
      const claimError = error as { claim?: string; reason?: string; payload?: JWTPayload }
...
export function getWWWAuthenticateHeader(
  ...
  const errorDescriptionMap: Record<TokenErrorCode, string> = {
