 ...
 * ```
 */
export function validateSecretPresent(
  secret: SecretValue | undefined | null,
  name: string,
  context?: string
): asserts secret is SecretValue {
 ...
 * ```
 */
export function validateSecretLength(secret: string, minLength: number, name: string): void {
 ...
 * ```
 */
export function getRequiredSecret<T extends Record<string, unknown>>(
  env: T,
  key: keyof T & string,
  context?: string
): string {
