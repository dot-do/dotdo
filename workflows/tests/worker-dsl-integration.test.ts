/**
 * Worker DSL Integration Tests
 *
 * Tests for integrating digital-workers with ai-workflows DSL.
 * Provides $.notify, $.approve, $.ask, $.decide, $.do operations on Worker targets.
 */
...
function createMockContext(): WorkflowContext {
  ...
  const actions: Array<{ action: string; data: unknown }> = []
  ...
  return {
    ...
    do: async (action, data) => {
      actions.push({ action, data })
      return { success: true, durable: true } as unknown
    },
    on: new Proxy({}, {
      get: () => new Proxy({}, {
        get: () => () => {},
...
// ============================================================================
// withWorkers Convenience API Tests
// ============================================================================
...
describe('withWorkers($)', () => {
  ...
  it('should provide $.work(target, task) as alias for do', async () => {
    const $ = createMockContext()
    const worker$ = withWorkers($)
...
// ============================================================================
// Durable Action Semantics Tests
// ============================================================================
