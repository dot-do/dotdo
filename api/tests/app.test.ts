...
describe('api/app.ts - createAPI', () => {
  ...
  describe('CORS Middleware', () => {
    it('should not add CORS headers by default (secure)', async () => {
      const app = createAPI()
      const res = await app.request('http://localhost/health', {
        headers: { 'Origin': 'https://malicious.com' }
      })

      // Secure default: no CORS headers when no origins configured
      expect(res.headers.get('Access-Control-Allow-Origin')).toBeNull()
    })

    it('should allow configured origins', async () => {
      const app = createAPI({
        cors: { allowedOrigins: ['https://trusted.com'] }
      })
      const res = await app.request('http://localhost/health', {
    ...
    it('should support multiple allowed origins', async () => {
      const app = createAPI({
        cors: { allowedOrigins: ['https://app.example.com', 'https://dashboard.example.com'] }
      })
      ...
      expect(res2.headers.get('Access-Control-Allow-Origin')).toBe('https://dashboard.example.com')

      // Non-listed origin should be rejected
      const res3 = await app.request('http://localhost/health', {
