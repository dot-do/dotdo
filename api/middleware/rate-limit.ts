...
export class RateLimiter {
   ...
   * 2. Authorization header with Bearer token (JWT decoded to extract sub/user_id claim)
   * 3. Falls back to 'anonymous' if no user identification found
   *
   * @issue do-e9tp - Properly decode JWT and extract user ID
   */
  private extractUser(request: Request): string {
    // Check X-User-ID header first (highest priority - explicit user identification)
    ...
    if (authHeader?.startsWith('Bearer ')) {
      ...
      // Decode JWT payload (without verification - this is for rate limiting only)
      ...
      // If JWT decoding failed or no user ID claim found, use a hash of the token
      // This ensures different tokens still get different rate limit buckets
      // Use first 32 chars of token as identifier (safe substring of the header part)
      return `bearer:${token.substring(0, 32)}`
    }
