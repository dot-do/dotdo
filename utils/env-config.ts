...
export interface EnvValidationResult<T> {
  ...
  success: boolean
  /** Validated environment values (empty object if failed) */
  env: T
...
export interface EnvValidationError {
  ...
  missing: boolean
  /** The invalid value (if present but invalid) */
  value?: unknown
}
...
export interface EnvConfigOptions {
  ...
  /** Log validation result (default: true in development) */
  logResult?: boolean
}
 ...
 * ```
 */
export function validateWorkerEnv<T>(
  schema: ZodType<T>,
  env: CloudflareEnv,
  options: EnvConfigOptions = {}
): EnvValidationResult<T> {
  return validateEnv(schema, env as Record<string, unknown>, options)
}
...
/** Inferred type from BaseDotdoEnvSchema */
export type BaseDotdoEnv = z.infer<typeof BaseDotdoEnvSchema>

/** Inferred type from DOEnvSchema */
export type DOEnv = z.infer<typeof DOEnvSchema>

/** Inferred type from ApiEnvSchema */
export type ApiEnv = z.infer<typeof ApiEnvSchema>

/** Inferred type from AuthEnvSchema */
export type AuthEnv = z.infer<typeof AuthEnvSchema>

/** Inferred type from AIEnvSchema */
export type AIEnv = z.infer<typeof AIEnvSchema>

/** Inferred type from DBEnvSchema */
export type DBEnv = z.infer<typeof DBEnvSchema>

/** Inferred type from DotdoEnvSchema */
export type DotdoEnv = z.infer<typeof DotdoEnvSchema>
 ...
 */
function formatZodErrors(error: ZodError): EnvValidationError[] {
  return error.issues.map((issue: ZodIssue) => {
    ...
    return {
      ...
      missing: isMissing,
      value: isMissing ? undefined : (issue as { received?: unknown }).received,
    }
  })
}
 ...
 */
export function isDevelopment(env: Record<string, unknown>): boolean {
  const envValue = env['ENVIRONMENT'] ?? env['NODE_ENV']
 ...
 */
export function isTest(env: Record<string, unknown>): boolean {
  const envValue = env['ENVIRONMENT'] ?? env['NODE_ENV']
...
export function optionalEnv<T extends string | number | boolean>(
  ...
  if (typeof defaultValue === 'boolean') {
    ...
    return (strValue === 'true' || strValue === '1' || strValue === 'yes') as T
  }
