  ...
  describe('edge cases and error handling', () => {
    describe('Stripe signature edge cases', () => {
      ...
      it('should handle very long payload', async () => {
        const secret = 'whsec_test'
        const timestamp = Math.floor(Date.now() / 1000)
        const payload = 'x'.repeat(100000) // 100KB payload
      ...
      it('should handle special characters in payload', async () => {
        const secret = 'whsec_test'
        const timestamp = Math.floor(Date.now() / 1000)
        const payload = '{"emoji":"🎉","unicode":"\\u0000","special":"<>&\'"}'
    ...
    describe('Twilio signature edge cases', () => {
      it('should handle URL with query parameters', async () => {
        ...
        const result = await verifyTwilioSignature(url, params, expectedSignature, authToken)
    ...
    describe('HMAC signature edge cases', () => {
      it('should handle binary-like content in payload', async () => {
        const secret = 'test_secret'
        const payload = '\x00\x01\x02\x03' // Binary-ish content
      ...
      it('should reject signature with wrong case', async () => {
        const secret = 'test_secret'
        const payload = 'test_payload'
        ...
        // Uppercase the signature (should still work since we compare lowercase internally)
        const result = await verifyHmacSignature(payload, hexSignature.toUpperCase(), secret)

        // Note: This may fail depending on implementation - documents current behavior
        // The implementation uses lowercase hex, so uppercase will fail
        expect(result.valid).toBe(false)
      })

      it('should handle very short secret', async () => {
        const secret = 'x' // Minimum valid secret (1 character)
        const payload = 'test_payload'
      ...
      it('should handle timestamp at exact tolerance boundary', async () => {
        const secret = 'test_secret'
        const payload = 'test_payload'
        const now = 1614556800
        const boundaryTimestamp = now - 300 // Exactly at boundary

        vi.useFakeTimers()
        vi.setSystemTime(new Date(now * 1000))

        const signedPayload = `${boundaryTimestamp}.${payload}`
        const encoder = new TextEncoder()
        const keyData = encoder.encode(secret)
        const messageData = encoder.encode(signedPayload)
        const cryptoKey = await crypto.subtle.importKey('raw', keyData, { name: 'HMAC', hash: 'SHA-256' }, false, ['sign'])
        ...
        const result = await verifyHmacSignature(payload, hexSignature, secret, boundaryTimestamp)

        // At exactly 300 seconds, should still be valid (tolerance uses <=)
        expect(result.valid).toBe(true)
