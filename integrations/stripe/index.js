...
export class StripeIntegration {
    ...
    async handleWebhook(request) {
        if (this._status !== 'ready' || !this.config) {
            return new Response(JSON.stringify({ error: 'Integration not initialized' }), { status: 503, headers: { 'Content-Type': 'application/json' } });
        }
        try {
            ...
            if (this.config.webhookSecret && signature) {
                ...
                if (!verification.valid) {
                    return new Response(JSON.stringify({ error: 'Signature verification failed' }), { status: 401, headers: { 'Content-Type': 'application/json' } });
                }
            }
