...
1. **Parallelization Strategies Evaluated**: See sections above comparing type-based, entity-based, and hybrid sharding.

2. **DO Routing Strategy Defined**: `GraphLoadBalancer` with round-robin, least-busy, and capability-based routing.

3. **Error Handling Design**: `BulkheadCircuitBreaker` with category isolation implemented in `objects/circuit-breaker-bulkhead.ts`.

4. **Monitoring Requirements**: See metrics sections below.

5. **Code Utilities Created**: `db/schema/resolvers/parallel.ts` provides:
   - `createTypeShardingStrategy()` - Type-based sharding
   - `createEntityShardingStrategy()` - Hash-based entity sharding
   - `createHybridShardingStrategy()` - Combined type + entity sharding
   - `CascadeBatchPlanner` - Plans batches with dependency ordering
   - `ParallelCascadeExecutor` - Executes batches across DOs
   - `ParallelCascadeCoordinator` - High-level API combining planning + execution
   - `validateCascadePlan()` - Pre-execution validation
...
// For 3.6M entities across types
const typeEstimates = new Map([
  ['Startup', 1000],
  ['LeanCanvas', 1000],
  ['Customer', 100000],
  ['Feature', 500000],
...
// Estimate shards needed (1 per 10k entities)
const shardCount = estimateShardCount(typeEstimates, 10000) // ~360 shards

const coordinator = createParallelCascadeCoordinator(doInvoker, {
...
// {
//   totalDuration: 45000,
//   totalEntities: 3602000,
//   shardsUsed: 360,
//   batchesExecuted: 3602,
//   avgBatchLatency: 125,
//   peakConcurrency: 100,
//   failureRate: 0.001,
// }
...
|--------|------|-------------|
| `cascade.total_duration_ms` | Histogram | End-to-end cascade time |
| `cascade.entities_generated` | Counter | Total entities created |
| `cascade.shards_used` | Gauge | Active shard count |
| `cascade.batch_latency_ms` | Histogram | Per-batch execution time |
| `cascade.peak_concurrency` | Gauge | Maximum parallel batches |
| `cascade.failure_rate` | Gauge | Percentage of failed batches |
| `cascade.retry_count` | Counter | Retried operations |
| `cascade.circuit_breaker.open` | Gauge | Open circuit count by category |
...
const result = await coordinator.generateCascade(request)

metrics.histogram('cascade.total_duration_ms', result.metrics.totalDuration)
metrics.counter('cascade.entities_generated', result.metrics.totalEntities)
metrics.gauge('cascade.shards_used', result.metrics.shardsUsed)
metrics.gauge('cascade.failure_rate', result.metrics.failureRate)
```
...
- **`BulkheadCircuitBreaker`** provides fault isolation by category

The new `ParallelCascadeCoordinator` in `db/schema/resolvers/parallel.ts` builds on these primitives to provide a complete solution for parallel cascade execution with DO routing, error handling, and observability.
...
- Shard balance (operations per shard variance)
