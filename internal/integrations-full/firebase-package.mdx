---
title: "@dotdo/firebase"
description: Firebase-compatible in-memory SDK for edge environments and testing.
---
...
<Callout type="info">
Looking for persistent storage? See [firebase.do](/docs/integrations/firebase/service) for a fully managed Firebase-compatible edge database.
</Callout>
...
**This is a proxy/compatibility layer.** It can either run entirely locally on in-memory storage (development/testing) or connect to real Firebase APIs (production). The API surface matches Firebase's official SDK.
...
const snapshot = await getDoc(doc(db, 'users/alice'))
const user = snapshot.data() // User | undefined
```
...
```typescript
import { collection, getCountFromServer, getAggregateFromServer, sum, average, count } from '@dotdo/firebase'
...
## Testing with @dotdo/firebase

@dotdo/firebase is ideal for unit tests - no Firebase emulator required:

```typescript
import { initializeApp, getFirestore, collection, addDoc, getDoc } from '@dotdo/firebase'
...
describe('UserRepository', () => {
  ...
  it('should create a user', async () => {
    const result = await addDoc(collection(db, 'users'), { name: 'Alice', age: 30 })
...
- [firebase.do](/docs/integrations/firebase/service) - Managed Firebase on the edge
