/**
 * DOBase Response Shape Integration Tests - RED Phase
 *
 * Tests that verify DOBase's REST handler uses the new response shape utilities
 * for consistent JSON-LD/MDXLD formatted responses across all endpoints.
 *
 ...
 *
 * These tests are expected to FAIL until DOBase integrates the response shape utilities.
 *
 ...
 * - $context: parent namespace URL (e.g., "https://Startups.Studio")
 * - $type: the DO namespace URL itself
 * - $id: same as $type
 * - collections: Record with count info
 */
 ...
 *
 * Expected per acceptance criteria:
 * - $context: namespace URL
 * - $type: type URL (ns + '/' + pluralized type)
 * - $id: same as $type for collections
 * - count: total number of items
 * - links: navigation links
 * - actions: available actions (create, etc.)
 * - items: array with full $context/$type/$id shape
 */
 ...
 * - $context: namespace URL
 * - $type: type URL (ns + '/' + pluralized type)
 * - $id: full item URL (ns + '/' + type + '/' + id)
 * - links: navigation and edit links
 * - actions: available actions (update, delete, etc.)
 */
...
describe('Response Shape Integration: Root Response (GET /)', () => {
  it('returns $context as parent namespace URL', async () => {
    ...
    // RED: $context should be the parent namespace, e.g., "https://Startups.Studio"
    // For a DO at https://headless.ly, the parent is its studio
    expect(body.$context).toBeDefined()
    expect(typeof body.$context).toBe('string')

    // The acceptance criteria specifies: $context is parent
    // This test expects $context to be a PARENT reference, not the ns itself
  ...
  it('returns $type as the namespace URL', async () => {
    ...
    // RED: $type for root should be the DO's namespace URL itself
  ...
  it('returns $id same as $type for root', async () => {
    ...
    // RED: $id should equal $type for root response
  ...
  it('each collection in record has $id, $type: Collection, and count', async () => {
    ...
    if (Array.isArray(body.collections)) {
      // RED: This should not be an array
      expect(Array.isArray(body.collections)).toBe(false)
      return
    }
...
describe('Response Shape Integration: Collection Response (GET /:type)', () => {
  it('returns $context as namespace URL', async () => {
    ...
    // RED: $context should be the namespace URL
  ...
  it('returns $type as full type URL (ns + pluralized type)', async () => {
    ...
    // RED: $type should be the full type URL
  ...
  it('returns $id same as $type for collections', async () => {
    ...
    // RED: $id equals $type for collection responses
  ...
  it('returns count field with total number of items', async () => {
    ...
    // RED: Should have 'count' field (not 'total' or 'totalItems')
  ...
  it('returns links object with home and first', async () => {
    ...
    // RED: Should have links object with navigation links
  ...
  it('items have full $context/$type/$id shape', async () => {
    ...
    if (body.items && body.items.length > 0) {
      ...
      // RED: Each item should have full linked data shape
...
describe('Response Shape Integration: Item Response (GET /:type/:id)', () => {
  ...
  it('returns $type as type URL', async () => {
    ...
    // RED: $type should be the collection type URL
    expect(body.$type).toBe('https://headless.ly/customers')
  })
  ...
  it('returns $id as full item URL', async () => {
    ...
    // RED: $id should be the full item URL
  ...
  it('returns actions object with update and delete', async () => {
    ...
    // RED: Should have actions object
    expect(body.actions).toBeDefined()
...
// ============================================================================
// 4. Namespace Derivation from Request Origin
// ============================================================================
...
describe('Response Shape Integration: Namespace from Request Origin', () => {
  it('derives response URLs from request origin for root', async () => {
    ...
    // RED: All URLs should be based on the request origin
    expect(body.$type).toBe('https://headless.ly')
    expect(body.$id).toBe('https://headless.ly')
  })
  ...
  it('derives response URLs from request origin for collection', async () => {
    ...
    // RED: URLs should use request origin as base
    expect(body.$context).toBe('https://headless.ly')
...
// ============================================================================
// 5. Full Response Shape Validation (Integration)
// ============================================================================
...
describe('Response Shape Integration: Full Shape Validation', () => {
  it('root response has complete expected shape', async () => {
    ...
    // RED: Verify complete root shape per acceptance criteria
  ...
  it('collection response has complete expected shape', async () => {
    ...
    // RED: Verify complete collection shape per acceptance criteria
    expect(body).toMatchObject({
  ...
  it('item response has complete expected shape', async () => {
    ...
    // RED: Verify complete item shape per acceptance criteria
    expect(body).toMatchObject({
      $context: 'https://headless.ly',
      $type: 'https://headless.ly/customers',
...
describe('Response Shape Integration: Edge Cases', () => {
  it('handles special characters in item IDs with URL encoding', async () => {
    ...
    // RED: $id should have URL-encoded special characters
