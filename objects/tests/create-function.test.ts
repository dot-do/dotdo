...
describe('createFunction Factory', () => {
  ...
  describe('Registration', () => {
    ...
    // ========================================================================
    // TDD: Promise Chain Error Handling (dotdo-g7bvs)
    // Tests for proper error propagation in createFunctionImpl promise chain
    // ========================================================================
    describe('Promise Chain Error Handling', () => {
      describe('registry.has() failure propagation', () => {
        it('propagates error when registry.has() fails', async () => {
          const failingRegistry: FunctionRegistry = {
            ...createMockRegistry(),
            has: vi.fn().mockRejectedValue(new Error('Registry unavailable')),
          }

          await expect(
            createFunction(validCodeFunction, { registry: failingRegistry, env: mockEnv })
          ).rejects.toThrow('Registry unavailable')
        })

        it('does not leave partial state when registry.has() fails', async () => {
          const failingRegistry: FunctionRegistry = {
            ...createMockRegistry(),
            has: vi.fn().mockRejectedValue(new Error('Connection failed')),
          }

          await expect(
            createFunction(validCodeFunction, {
          ...
          // Should NOT have stored in state since registry check happens first
          const stored = await mockState.storage.get(`function:${validCodeFunction.name}`)
          expect(stored).toBeUndefined()
        })

        it('preserves original error type when registry.has() throws custom error', async () => {
          class CustomRegistryError extends Error {
            constructor(message: string) {
              super(message)
              this.name = 'CustomRegistryError'
            }
          }

          const failingRegistry: FunctionRegistry = {
            ...createMockRegistry(),
            has: vi.fn().mockRejectedValue(new CustomRegistryError('Custom failure')),
          }

          try {
            await createFunction(validCodeFunction, { registry: failingRegistry, env: mockEnv })
            expect.fail('Should have thrown')
          } catch (error) {
            expect((error as Error).name).toBe('CustomRegistryError')
            expect((error as Error).message).toBe('Custom failure')
          }
        })
      })

      describe('state.storage.put() failure propagation', () => {
        it('propagates error when storage.put() fails', async () => {
          const failingState: DurableObjectState = {
            ...mockState,
            storage: {
              ...mockState.storage,
        ...
        it('does not register function when storage.put() fails', async () => {
          const failingState: DurableObjectState = {
            ...mockState,
            storage: {
              ...mockState.storage,
          ...
          // Registration should NOT have been called since storage put happens before register
          expect(mockRegistry.register).not.toHaveBeenCalled()
        })
      })

      describe('registry.register() failure propagation', () => {
        it('propagates error when registry.register() fails', async () => {
          const failingRegistry: FunctionRegistry = {
            ...createMockRegistry(),
        ...
        it('stores in state even when registry.register() fails later', async () => {
          const failingRegistry: FunctionRegistry = {
            ...createMockRegistry(),
            has: vi.fn().mockResolvedValue(false),
            register: vi.fn().mockRejectedValue(new Error('Registration failed')),
          ...
          // Storage put happens BEFORE register, so it should have succeeded
          const stored = await mockState.storage.get(`function:${validCodeFunction.name}`)
          expect(stored).toBeDefined()
        })
      })

      describe('registry.unregister() failure during replace', () => {
        it('propagates error when registry.unregister() fails during replace', async () => {
          // First register succeeds
          await createFunction(validCodeFunction, { registry: mockRegistry, env: mockEnv })

          // Make unregister fail for replacement
      ...
      describe('async operation sequencing', () => {
        it('executes promise chain steps in correct order', async () => {
          const executionOrder: string[] = []

          const sequenceRegistry: FunctionRegistry = {
            has: vi.fn(async () => {
              executionOrder.push('registry.has')
              return false
            }),
            register: vi.fn(async () => {
              executionOrder.push('registry.register')
            }),
            get: vi.fn(),
            list: vi.fn(),
            unregister: vi.fn(),
          ...
          const sequenceState: DurableObjectState = {
            ...mockState,
            storage: {
              ...mockState.storage,
              put: vi.fn(async () => {
                executionOrder.push('storage.put')
              }),
            },
          } as unknown as DurableObjectState
          ...
          // Order should be: has (check duplicate) -> put (store) -> register
          expect(executionOrder).toEqual(['registry.has', 'storage.put', 'registry.register'])
        })
        ...
        it('stops chain execution on first error', async () => {
          ...
          expect(executionOrder).toEqual(['registry.has'])
        })
      })

      describe('event callback error isolation', () => {
        it('propagates errors from onEvent callbacks during registration', async () => {
          const failingOnEvent = vi.fn(() => {
            throw new Error('onEvent callback failed')
          })

          // This tests whether throwing in onEvent affects the promise chain
          // The current implementation calls onEvent but doesn't await/catch it
          // This test documents expected behavior
          await expect(
            createFunction(validCodeFunction, {
              registry: mockRegistry,
              env: mockEnv,
              onEvent: failingOnEvent,
            })
          ).rejects.toThrow('onEvent callback failed')
        })

        it('async onEvent rejections are fire-and-forget (not awaited)', async () => {
          // This test documents that async onEvent callbacks are NOT awaited
          // An async rejection here creates an unhandled rejection but doesn't
          // affect the createFunction promise resolution
          const asyncFailingOnEvent = vi.fn(async () => {
            await Promise.resolve()
            throw new Error('Async onEvent failed')
          })

          // Because the async callback is not awaited, the function creation
          // should succeed despite the async rejection
          // NOTE: This creates an unhandled promise rejection which is a potential issue
          // but this test documents current behavior
          const fn = await createFunction(validCodeFunction, {
            registry: mockRegistry,
            env: mockEnv,
            onEvent: asyncFailingOnEvent,
          })

          expect(fn).toBeDefined()
          expect(asyncFailingOnEvent).toHaveBeenCalled()
        })
      })

      describe('resource cleanup on failure', () => {
        it('cleans up storage on registration failure when autoRegister is true', async () => {
          const cleanupState = createMockState()
          const putSpy = vi.spyOn(cleanupState.storage as Storage, 'put')
          const deleteSpy = vi.spyOn(cleanupState.storage as Storage, 'delete')
          ...
          // Verify put was called (storage was attempted)
          expect(putSpy).toHaveBeenCalled()

          // NOTE: Current implementation does NOT clean up storage on register failure
          // This test documents the current behavior - storage put succeeds even when
          // registration fails, potentially leaving orphaned data
          // TODO: Consider adding cleanup on registration failure
        })
      })
    })
  })
