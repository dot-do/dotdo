/**
 * RED TDD: withFs Capability Integration Tests
 *
 * Tests for integrating fsx.do's withFs capability with dotdo Durable Objects.
 * These tests verify the $.fs capability is properly added to WorkflowContext.
 *
 * Key requirements (from fsx.do):
 * 1. withFs capability adds $.fs to WorkflowContext
 ...
 *
 * The withFs capability wraps fsx.do's FsModule which provides:
...
// These imports will fail until the capability is implemented
// This is intentional for TDD RED phase
import {
  withFs,
  type WithFsContext,
  type WithFsOptions,
} from '../capabilities/fs'

// Re-export from fsx.do - these should be available when implemented
import type { FsModule } from '../capabilities/fs'
