...
/**
 * Clone state for transform function
 */
 ...
 */
interface StagedPrepareResult extends CloneResult {
  phase: 'prepared'
  token: string
  expiresAt: Date
}
 ...
 */
interface EventualCloneHandle extends CloneResult {
  id: string
  status: string
  getProgress: () => { percentage: number; thingsTransferred: number }
  getSyncStatus: () => { synced: boolean; lag: number }
}

/**
 * Resumable clone handle
 */
interface ResumableCloneHandle extends CloneResult {
  id: string
  status: string
  checkpoints: unknown[]
  pause: () => Promise<void>
...
class TestDO {
  ...
  async clone(target: string, options: {
    ...
    if (!['atomic', 'staged', 'eventual', 'resumable'].includes(mode)) {
    ...
    if (options.colo && !VALID_COLOS.has(options.colo)) {
      throw new Error(`Invalid colo code: ${options.colo}`)
    }
    ...
    const things = this._getThings()
    const relationships = (this.ctx._sqlData.get('relationships') || []) as Relationship[]

    let sourceThings = things
    if (options.branch) {
      const branches = this._getBranches()
      if (options.branch !== 'main' && !branches.some(b => b.name === options.branch)) {
        throw new Error(`Branch not found: ${options.branch}`)
      }
      const branchFilter = options.branch === 'main' ? null : options.branch
      sourceThings = things.filter(t => t.branch === branchFilter)
    }

    if (options.version !== undefined) {
      const thingAtVersion = things.find(t => t.rowid === options.version)
      if (!thingAtVersion) {
        throw new Error(`Version not found: ${options.version}`)
      }
      sourceThings = things.filter(t => (t.rowid || 0) <= options.version!)
    }

    if (includeState && sourceThings.filter(t => !t.deleted).length === 0) {
      throw new Error('No state to clone - source is empty')
    }

    await this.emitEvent('clone.started', { target, mode, correlationId: options.correlationId })

    const startTime = Date.now()

    try {
      let cloneState: CloneState = {
        things: includeState ? sourceThings.filter(t => !t.deleted).map(t => ({
          id: t.id, type: t.type, branch: t.branch, name: t.name, data: t.data || {}, deleted: t.deleted,
        })) : [],
        relationships: shallow ? [] : relationships.filter(r =>
          sourceThings.some(t => t.id === r.from) || sourceThings.some(t => t.id === r.to)
        ),
      }

      if (options.transform) {
        try {
          cloneState = await options.transform(cloneState)
        } catch (e) {
      ...
      await stub.fetch(new Request(`https://${target}/init`, {
        method: 'POST',
        body: JSON.stringify({ things: cloneState.things, relationships: cloneState.relationships }),
      }))

      const duration = Date.now() - startTime

      await this.emitEvent('clone.completed', {
        target, doId: doId.toString(), mode,
        thingsCloned: cloneState.things.length, relationshipsCloned: cloneState.relationships?.length || 0,
        correlationId: options.correlationId,
      })

      if (mode === 'staged') {
        return { phase: 'prepared', token: crypto.randomUUID(), expiresAt: new Date(Date.now() + 300000), ns: target, doId: doId.toString(), mode } as StagedPrepareResult
      }
      if (mode === 'eventual') {
        return { id: crypto.randomUUID(), status: 'pending', getProgress: () => ({ percentage: 0, thingsTransferred: 0 }), getSyncStatus: () => ({ synced: false, lag: 0 }), ns: target, doId: doId.toString(), mode } as EventualCloneHandle
      }
      if (mode === 'resumable') {
        return { id: crypto.randomUUID(), status: 'transferring', checkpoints: [], pause: async () => {}, resume: async () => {}, ns: target, doId: doId.toString(), mode } as ResumableCloneHandle
      }

      return { ns: target, doId: doId.toString(), mode, thingsCloned: cloneState.things.length, relationshipsCloned: cloneState.relationships?.length || 0, historyIncluded: includeHistory, duration } as CloneResult & { thingsCloned: number; relationshipsCloned: number; historyIncluded: boolean; duration: number }
    } catch (error) {
      await this.emitEvent('clone.failed', { target, error: (error as Error).message, correlationId: options.correlationId })
      throw error
    }
  }
  ...
  async promote(thingId: string, options: {
    ...
    const linkParent = options.linkParent !== false
    ...
    const newDoId = options.newId || `${this.ns}/${thingId}`

    await this.emitEvent('promote.started', { thingId, newDoId })

    const doId = this.env.DO.newUniqueId(options.colo ? { locationHint: options.colo } : undefined)
    const stub = this.env.DO.get(doId)

    const actions = preserveHistory ? (this.ctx._sqlData.get('actions') as Action[] || []).filter(a => a.target?.includes(thingId)) : []
    const events = preserveHistory ? (this.ctx._sqlData.get('events') as Event[] || []).filter(e => e.source?.includes(thingId)) : []

    await stub.fetch(new Request(`https://${newDoId}/init`, {
      method: 'POST',
      body: JSON.stringify({ thing: { ...thing, id: 'root', branch: null, rowid: 1 }, actions, events, parentNs: linkParent ? this.ns : null }),
    }))

    const thingIndex = things.findIndex(t => t.id === thingId)
    if (thingIndex >= 0) {
      things[thingIndex] = { ...things[thingIndex], deleted: true, data: { ...things[thingIndex].data, promotedTo: newDoId } }
    }

    await this.emitEvent('promote.completed', { thingId, newDoId, doId: doId.toString(), actionsMigrated: actions.length, eventsMigrated: events.length, parentLinked: linkParent })

    return { ns: newDoId, doId: doId.toString(), thingId, actionsMigrated: actions.length, eventsMigrated: events.length, parentLinked: linkParent }
  }
  ...
  async demote(sourceDoNs: string, options: {
    thingId?: string
    preserveHistory?: boolean
    recursive?: boolean
    ...
    try {
      ...
      throw new Error(`Invalid source namespace: ${sourceDoNs}`)
    }

    if (sourceDoNs === this.ns) {
      throw new Error('Cannot demote self')
    }

    await this.emitEvent('demote.started', { sourceDoNs })

    const sourceState = {
      things: [{ id: 'root', type: 1, branch: null, name: 'Source Thing', data: { source: sourceDoNs }, deleted: false }],
      actions: preserveHistory ? [{ id: 'action-1', verb: 'create', target: 'root' }] : [],
      events: preserveHistory ? [{ id: 'event-1', verb: 'created', source: sourceDoNs }] : [],
    }

    const newThingId = options.thingId || `demoted-${crypto.randomUUID().slice(0, 8)}`
    const things = this._getThings()

    const newThing: MockThing = {
      id: newThingId,
      type: sourceState.things[0]?.type || 1,
      branch: null,
      name: sourceState.things[0]?.name || 'Demoted Thing',
      data: { ...sourceState.things[0]?.data, demotedFrom: sourceDoNs },
      deleted: false,
    ...
    if (preserveHistory) {
      const actions = this.ctx._sqlData.get('actions') as Action[]
      const events = this.ctx._sqlData.get('events') as Event[]
      actions.push(...sourceState.actions.map(a => ({ ...a, target: newThingId })) as Action[])
      events.push(...sourceState.events.map(e => ({ ...e, source: `${this.ns}/${newThingId}` })) as Event[])
    }

    await this.emitEvent('demote.completed', { sourceDoNs, thingId: newThingId, actionsMigrated: sourceState.actions.length, eventsMigrated: sourceState.events.length })

    return { thingId: newThingId, sourceDoNs, actionsMigrated: sourceState.actions.length, eventsMigrated: sourceState.events.length }
  }
}
...
describe('DO Lifecycle Operations', () => {
  ...
  describe('clone(target, options)', () => {
    describe('Basic Clone Behavior', () => {
      it('clones state to target namespace', async () => {
        await testDO._addThing({ id: 'data', type: 1, branch: null, name: 'Data', data: { value: 1 }, deleted: false })

        const result = await testDO.clone('https://clone.example.com.ai', { mode: 'atomic' })

        expect(result.ns).toBe('https://clone.example.com.ai')
        expect(result.doId).toBeDefined()
      ...
      it('validates target namespace URL', async () => {
        ...
        await expect(testDO.clone('not-a-url', { mode: 'atomic' })).rejects.toThrow(/invalid.*namespace/i)
        await expect(testDO.clone('ftp://wrong-protocol.com', { mode: 'atomic' })).rejects.toThrow(/invalid/i)
      })

      it('rejects cloning to same namespace', async () => {
        await testDO._addThing({ id: 'data', type: 1, branch: null, name: 'Data', data: {}, deleted: false })

        await expect(testDO.clone('https://test.example.com.ai', { mode: 'atomic' })).rejects.toThrow(/same.*namespace/i)
      })

      it('rejects cloning empty source', async () => {
        await expect(testDO.clone('https://empty.example.com.ai', { mode: 'atomic' })).rejects.toThrow(/empty|no.*state/i)
      })
    })

    describe('Clone Modes', () => {
      beforeEach(async () => {
        await testDO._addThing({ id: 'data', type: 1, branch: null, name: 'Data', data: { value: 1 }, deleted: false })
      })

      it('supports atomic mode', async () => {
        const result = await testDO.clone('https://atomic.example.com.ai', { mode: 'atomic' })
        expect(result.mode).toBe('atomic')
      })

      it('supports staged mode', async () => {
        const result = await testDO.clone('https://staged.example.com.ai', { mode: 'staged' }) as StagedPrepareResult
      ...
      it('supports eventual mode', async () => {
        const result = await testDO.clone('https://eventual.example.com.ai', { mode: 'eventual' }) as EventualCloneHandle
        expect(result.mode).toBe('eventual')
        expect(result.id).toBeDefined()
        expect(result.status).toBe('pending')
        expect(typeof result.getProgress).toBe('function')
        expect(typeof result.getSyncStatus).toBe('function')
      })

      it('supports resumable mode', async () => {
        const result = await testDO.clone('https://resumable.example.com.ai', { mode: 'resumable' }) as ResumableCloneHandle
      ...
      it('rejects invalid mode', async () => {
        await expect(testDO.clone('https://invalid.example.com.ai', { mode: 'invalid' as 'atomic' })).rejects.toThrow(/invalid.*mode/i)
      })
    })

    describe('Clone Options', () => {
      beforeEach(async () => {
        await testDO._addThing({ id: 'data-1', type: 1, branch: null, name: 'Data 1', data: { value: 1 }, deleted: false })
        await testDO._addThing({ id: 'data-2', type: 1, branch: null, name: 'Data 2', data: { value: 2 }, deleted: false })
        mockState._sqlData.set('relationships', [
          { id: 'rel-1', verb: 'relatedTo', from: 'data-1', to: 'data-2', data: null },
        ])
      })

      it('performs deep clone by default', async () => {
        const result = await testDO.clone('https://deep.example.com.ai', { mode: 'atomic' })
        expect((result as { relationshipsCloned: number }).relationshipsCloned).toBe(1)
      })

      it('performs shallow clone when specified', async () => {
        const result = await testDO.clone('https://shallow.example.com.ai', { mode: 'atomic', shallow: true })
        expect((result as { relationshipsCloned: number }).relationshipsCloned).toBe(0)
      })

      it('excludes state when includeState is false', async () => {
        const result = await testDO.clone('https://no-state.example.com.ai', { mode: 'atomic', includeState: false })
        expect((result as { thingsCloned: number }).thingsCloned).toBe(0)
      })

      it('includes history when includeHistory is true', async () => {
        const result = await testDO.clone('https://with-history.example.com.ai', { mode: 'atomic', includeHistory: true })
        expect((result as { historyIncluded: boolean }).historyIncluded).toBe(true)
      })

      it('validates colo option', async () => {
        await expect(testDO.clone('https://bad-colo.example.com.ai', { mode: 'atomic', colo: 'invalid' })).rejects.toThrow(/invalid.*colo/i)
      })

      it('validates timeout option', async () => {
        await expect(testDO.clone('https://bad-timeout.example.com.ai', { mode: 'atomic', timeout: -1 })).rejects.toThrow(/invalid.*timeout/i)
      })
    })

    describe('Clone Transform', () => {
      beforeEach(async () => {
        await testDO._addThing({ id: 'data', type: 1, branch: null, name: 'Data', data: { value: 1 }, deleted: false })
      })

      it('applies transform function', async () => {
        const transform = (state: CloneState): CloneState => ({
          ...state,
          things: state.things.map(t => ({ ...t, data: { ...t.data, transformed: true } })),
        })

        const result = await testDO.clone('https://transformed.example.com.ai', { mode: 'atomic', transform })
        expect(result).toBeDefined()
      })

      it('supports async transform', async () => {
        const transform = async (state: CloneState): Promise<CloneState> => {
          await new Promise(resolve => setTimeout(resolve, 1))
          return { ...state, things: state.things.filter(t => t.data.value === 1) }
        }

        const result = await testDO.clone('https://async-transform.example.com.ai', { mode: 'atomic', transform })
        expect(result).toBeDefined()
      })

      it('handles transform errors', async () => {
        const transform = (): CloneState => {
          throw new Error('Transform failed')
        }

        await expect(testDO.clone('https://error-transform.example.com.ai', { mode: 'atomic', transform })).rejects.toThrow(/transform/i)
      })
    })

    describe('Clone Branch/Version Options', () => {
      beforeEach(async () => {
        await testDO._addThing({ id: 'main-data', type: 1, branch: null, name: 'Main', data: {}, deleted: false, rowid: 1 })
        await testDO._addThing({ id: 'main-data', type: 1, branch: null, name: 'Main v2', data: {}, deleted: false, rowid: 2 })
        await testDO._addBranch({ name: 'feature', thingId: 'main-data', head: 2, base: 1, forkedFrom: 'main', description: null, createdAt: new Date(), updatedAt: new Date() })
        await testDO._addThing({ id: 'feature-data', type: 1, branch: 'feature', name: 'Feature', data: {}, deleted: false, rowid: 3 })
      })

      it('clones specific branch', async () => {
        const result = await testDO.clone('https://branch.example.com.ai', { mode: 'atomic', branch: 'feature' })
        expect((result as { thingsCloned: number }).thingsCloned).toBe(1)
      })

      it('clones at specific version', async () => {
        const result = await testDO.clone('https://version.example.com.ai', { mode: 'atomic', version: 1 })
        expect(result).toBeDefined()
      })

      it('rejects invalid branch', async () => {
        await expect(testDO.clone('https://bad-branch.example.com.ai', { mode: 'atomic', branch: 'nonexistent' })).rejects.toThrow(/branch.*not found/i)
      })

      it('rejects invalid version', async () => {
        await expect(testDO.clone('https://bad-version.example.com.ai', { mode: 'atomic', version: 999 })).rejects.toThrow(/version.*not found/i)
      })
    })

    describe('Clone Events', () => {
      beforeEach(async () => {
        await testDO._addThing({ id: 'data', type: 1, branch: null, name: 'Data', data: {}, deleted: false })
      })

      it('emits clone.started event', async () => {
        await testDO.clone('https://events.example.com.ai', { mode: 'atomic' })

        const events = mockState._sqlData.get('events') as Event[]
        expect(events.some(e => e.verb === 'clone.started')).toBe(true)
      })

      it('emits clone.completed event on success', async () => {
        await testDO.clone('https://events-complete.example.com.ai', { mode: 'atomic' })

        const events = mockState._sqlData.get('events') as Event[]
        expect(events.some(e => e.verb === 'clone.completed')).toBe(true)
      })
    })
  })
  ...
  describe('promote(thingId, options)', () => {
    describe('Basic Promote Behavior', () => {
      it('promotes thing to its own DO', async () => {
        await testDO._addThing({ id: 'customer-1', type: 1, branch: null, name: 'Customer', data: { name: 'Acme' }, deleted: false })

        const result = await testDO.promote('customer-1')

        expect(result.ns).toBeDefined()
        expect(result.doId).toBeDefined()
        expect(result.thingId).toBe('customer-1')
      })

      it('marks original thing as deleted', async () => {
        await testDO._addThing({ id: 'customer-2', type: 1, branch: null, name: 'Customer', data: {}, deleted: false })

        await testDO.promote('customer-2')

        const things = testDO._getThings()
        const original = things.find(t => t.id === 'customer-2')
        expect(original?.deleted).toBe(true)
      })

      it('rejects promoting nonexistent thing', async () => {
        await expect(testDO.promote('nonexistent')).rejects.toThrow(/not found/i)
      })

      it('rejects promoting deleted thing', async () => {
        await testDO._addThing({ id: 'deleted-thing', type: 1, branch: null, name: 'Deleted', data: {}, deleted: true })

        await expect(testDO.promote('deleted-thing')).rejects.toThrow(/not found/i)
      })
    })

    describe('Promote Options', () => {
      beforeEach(async () => {
        await testDO._addThing({ id: 'promotable', type: 1, branch: null, name: 'Promotable', data: { key: 'value' }, deleted: false })
        mockState._sqlData.set('actions', [
          { id: 'action-1', verb: 'create', target: 'Thing/promotable' },
        ])
        mockState._sqlData.set('events', [
          { id: 'event-1', verb: 'created', source: 'test/promotable' },
        ])
      })

      it('preserves history by default', async () => {
        const result = await testDO.promote('promotable')
        expect(result.actionsMigrated).toBeGreaterThanOrEqual(0)
        expect(result.eventsMigrated).toBeGreaterThanOrEqual(0)
      })

      it('excludes history when preserveHistory is false', async () => {
        const result = await testDO.promote('promotable', { preserveHistory: false })
        expect(result.actionsMigrated).toBe(0)
        expect(result.eventsMigrated).toBe(0)
      })

      it('links parent by default', async () => {
        const result = await testDO.promote('promotable')
        expect(result.parentLinked).toBe(true)
      })

      it('does not link parent when linkParent is false', async () => {
        const result = await testDO.promote('promotable', { linkParent: false })
        expect(result.parentLinked).toBe(false)
      })

      it('accepts custom newId', async () => {
        const result = await testDO.promote('promotable', { newId: 'https://custom.example.com.ai/customer' })
        expect(result.ns).toBe('https://custom.example.com.ai/customer')
      })

      it('validates colo option', async () => {
        await expect(testDO.promote('promotable', { colo: 'invalid' })).rejects.toThrow(/invalid.*colo/i)
      })
    })

    describe('Promote Events', () => {
      beforeEach(async () => {
        await testDO._addThing({ id: 'for-events', type: 1, branch: null, name: 'For Events', data: {}, deleted: false })
      })

      it('emits promote.started event', async () => {
        await testDO.promote('for-events')

        const events = mockState._sqlData.get('events') as Event[]
        expect(events.some(e => e.verb === 'promote.started')).toBe(true)
      })
      ...
      it('emits promote.completed event on success', async () => {
        ...
        const events = mockState._sqlData.get('events') as Event[]
        expect(events.some(e => e.verb === 'promote.completed')).toBe(true)
      })
    })
  })
  ...
  describe('demote(sourceDoNs, options)', () => {
    describe('Basic Demote Behavior', () => {
      it('demotes DO to thing', async () => {
        const result = await testDO.demote('https://external.example.com.ai')

        expect(result.thingId).toBeDefined()
        expect(result.sourceDoNs).toBe('https://external.example.com.ai')
      })

      it('creates thing with demotedFrom reference', async () => {
        await testDO.demote('https://source.example.com.ai')

        const things = testDO._getThings()
        const demotedThing = things.find(t => t.data?.demotedFrom === 'https://source.example.com.ai')
        expect(demotedThing).toBeDefined()
      })

      it('rejects demoting self', async () => {
        await expect(testDO.demote('https://test.example.com.ai')).rejects.toThrow(/cannot demote self/i)
      })

      it('validates source namespace URL', async () => {
        await expect(testDO.demote('not-a-url')).rejects.toThrow(/invalid.*namespace/i)
      })
    })

    describe('Demote Options', () => {
      it('accepts custom thingId', async () => {
        const result = await testDO.demote('https://custom.example.com.ai', { thingId: 'custom-thing-id' })
        expect(result.thingId).toBe('custom-thing-id')
      })

      it('preserves history by default', async () => {
        const result = await testDO.demote('https://history.example.com.ai')
        expect(result.actionsMigrated).toBeGreaterThanOrEqual(0)
        expect(result.eventsMigrated).toBeGreaterThanOrEqual(0)
      })

      it('excludes history when preserveHistory is false', async () => {
        const result = await testDO.demote('https://no-history.example.com.ai', { preserveHistory: false })
        expect(result.actionsMigrated).toBe(0)
        expect(result.eventsMigrated).toBe(0)
      })
    })

    describe('Demote Events', () => {
      it('emits demote.started event', async () => {
        await testDO.demote('https://events.example.com.ai')

        const events = mockState._sqlData.get('events') as Event[]
        expect(events.some(e => e.verb === 'demote.started')).toBe(true)
      })

      it('emits demote.completed event on success', async () => {
        await testDO.demote('https://events-complete.example.com.ai')

        const events = mockState._sqlData.get('events') as Event[]
        expect(events.some(e => e.verb === 'demote.completed')).toBe(true)
      })
    })
  })
})
