...
class InvocationTrackingDO extends DO {
   ...
   *
   * The relationship record should have:
   * - id: unique relationship ID
   * - verb: 'invoked'
   * - from: executor URL (session or Agent/Human Thing $id)
   * - to: tool URL (ns/tools/toolName)
   * - data: InvocationData { input, output, error, duration, startedAt, completedAt, executor }
   * - createdAt: timestamp
   */
  async getToolInvocations(toolName: string): Promise<Relationship[]> {
    const toolId = `${this.ns}/tools/${toolName}`

    // Try to access the MCP invocation tracker if available
    // The implementation should add a getToolInvocations method or expose invocations
    const mcpHandler = (this as unknown as { _mcpInvocations?: Relationship[] })._mcpInvocations
    if (mcpHandler) {
      return mcpHandler.filter(r => r.to === toolId && r.verb === 'invoked')
    }
    ...
    // Feature not implemented - this is the expected RED phase failure
    throw new Error(
      `Invocation tracking not implemented.\n` +
      `Expected: MCP tools/call handler should create invocation relationships.\n` +
      `Tool ID: ${toolId}\n` +
      `The implementation should:\n` +
      `  1. Create relationship with verb='invoked' before tool execution\n` +
      `  2. Store input, startedAt, executor info\n` +
      `  3. Update with output/error, duration, completedAt after execution`
    )
  }
}
