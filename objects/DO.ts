...
import type {
  ...
  TransactionLogEntry,
  TransactionAuditLog,
  RecoveredTransactionState,
  ParticipantRecoveryResponse,
  ParticipantStateHistory,
  ResumedTransaction,
  StagedCloneOptions2PC,
...
export class DO<E extends Env = Env> extends DurableObject<E> {
  ...
  async promote(
    ...
    // Validate custom namespace binding if specified
    if (namespaceBinding) {
      const customNamespace = (this.env as Record<string, unknown>)[namespaceBinding]
      if (!customNamespace) {
        this._promotingThings.delete(thingId)
    ...
    return this.ctx.blockConcurrencyWhile(async () => {
      ...
      try {
        ...
        // Delete the original thing from parent DO (after successful promotion)
        await this.db.delete(schema.things).where(eq(schema.things.id, thingId))
        ...
        // Emit thing.promoted event for observability
        await this.emitEvent('thing.promoted', {
          thingId,
          correlationId,
          newNs,
          doId: doIdString,
  ...
  protected static readonly DEFAULT_COORDINATOR_TIMEOUT = 30000 // 30 seconds
  protected static readonly DEFAULT_ACK_TIMEOUT = 10000 // 10 seconds
  protected static readonly DEFAULT_MAX_RETRIES = 3

  /**
   * Broadcast callback registry for 2PC (in-memory, per instance)
   */
  ...
  private async prepareStagedClone(
    ...
    // Store 2PC prepare decision (before any participant communication)
    ...
    await this.ctx.storage.put(`${DO.TWO_PC_PREFIX}prepare:${token}`, prepareDecision)

    // Store 2PC config for timeouts
    const twopcConfig = {
      coordinatorTimeout: options.coordinatorTimeout ?? DO.DEFAULT_COORDINATOR_TIMEOUT,
      participantAckTimeout: options.participantAckTimeout ?? DO.DEFAULT_ACK_TIMEOUT,
      maxAckRetries: options.maxAckRetries ?? DO.DEFAULT_MAX_RETRIES,
      prepareTimeout: options.prepareTimeout,
      commitTimeout: options.commitTimeout,
      abortTimeout: options.abortTimeout,
    }
    await this.ctx.storage.put(`${DO.TWO_PC_PREFIX}config:${token}`, twopcConfig)
    ...
    await this.ctx.storage.put(`${DO.TWO_PC_PREFIX}history:${token}:${target}`, participantHistory)
    ...
    // Create participant acknowledgment
   ...
   */
  async commitClone(token: string, options?: { timeout?: number }): Promise<StagedCommitResult & { participantAcks?: ParticipantAck[] }> {
