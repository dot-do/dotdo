...
import type { FinancialClient } from '@dotdo/finance'
...
export class Business extends DO {
  ...
  constructor(
    ...
    // Cast to Parameters to handle DurableObjectState version mismatches between packages.
    // This is type-safe because:
    // 1. DurableObjectState from @cloudflare/workers-types is structurally compatible
    // 2. The cast preserves the expected shape, just bridges version differences
    // 3. Runtime behavior is identical - no actual casting occurs
    type DOConstructorParams = ConstructorParameters<typeof DO>
    super(state as DOConstructorParams[0], env)
  ...
  get aggregate(): AggregateBuilder {
    // Create an executor wrapper that calls our protected executeAggregation method
    // This maintains encapsulation while providing type-safe access for the builder
    const executor: AggregationExecutor = {
      executeAggregation: (query: ParsedQuery) => this.executeAggregation(query)
    }
    return new AggregateBuilder(executor)
  }
  ...
  // ===========================================================================
  // Finance (delegated to @dotdo/finance)
  // ===========================================================================
  ...
  // ===========================================================================
  // Analytics (delegated to @dotdo/analytics)
  // ===========================================================================
...
/**
 * Interface for objects that can execute aggregation queries.
 * Used by AggregateBuilder to call back into Business.
 */
interface AggregationExecutor {
  executeAggregation(query: ParsedQuery): Promise<AggregateResult>
}
...
class AggregateBuilder {
  ...
  constructor(private executor: AggregationExecutor) {}
  ...
  private async execute(): Promise<AggregateResult> {
    ...
    // Call executeAggregation via the typed AggregationExecutor interface
    return this.executor.executeAggregation(query)
  }
}
...
class ExperimentsAPI {
  ...
  async results(experimentKey: string): Promise<{
    ...
    throw new Error('Not implemented - requires @dotdo/analytics integration')
  }
...
class AnalyticsAPI {
  ...
  // Delegate to @dotdo/analytics
  async track(event: { type: string; properties?: Record<string, unknown> }): Promise<void> {
    throw new Error('Not implemented - requires @dotdo/analytics')
  }
