/**
 * DO Sharding Support (do-rljr.4)
 *
 * Provides sharding capabilities beyond simple namespace-based routing.
 * Supports consistent hashing to distribute load across multiple DO instances.
 *
 * Current routing: tenant.api.dotdo.dev → DO('tenant')
 * With sharding: tenant.api.dotdo.dev/users/123 → DO('tenant:user:shard-2')
 *
 * Sharding strategies:
 * - Namespace-only (default): DO('tenant')
 * - Entity-based: DO('tenant:entity-type:shard-N')
 * - Key-based: DO('tenant:custom-key:shard-N')
 * - Composite: DO('tenant:entity:key:shard-N')
 */

/**
 * Shard key configuration
 */
export interface ShardKeyConfig {
  /** Type of sharding: by entity type, by custom key, or composite */
  type: 'entity' | 'key' | 'composite'
  /** Number of shards for this configuration */
  shardCount: number
  /** Extract key from request/context */
  extractKey?: (ctx: ShardContext) => string | undefined
}

/**
 * Context for shard key extraction
 */
export interface ShardContext {
  /** Namespace (from subdomain) */
  namespace: string
  /** Request path */
  path: string
  /** URL search params */
  params?: URLSearchParams
  /** Request headers */
  headers?: Headers
  /** Entity type (if known from route) */
  entityType?: string
  /** Entity ID (if known from route) */
  entityId?: string
  /** Custom shard key override */
  shardKey?: string
}

/**
 * Shard routing result
 */
export interface ShardResult {
  /** Full DO name to use */
  doName: string
  /** Shard index (0-based) */
  shardIndex: number
  /** Whether sharding was applied */
  sharded: boolean
  /** Key used for sharding */
  key?: string
}

/**
 * Shard router configuration
 */
export interface ShardRouterConfig {
  /** Default number of shards */
  defaultShardCount: number
  /** Entity-specific shard configurations */
  entityShards?: Record<string, number>
  /** Custom shard key extractors by path pattern */
  keyExtractors?: Record<string, (ctx: ShardContext) => string | undefined>
  /** Separator for DO name components (default: ':') */
  separator?: string
  /** Enable sharding (default: true) */
  enabled?: boolean
}

/**
 * Default configuration
 */
const DEFAULT_CONFIG: Required<ShardRouterConfig> = {
  defaultShardCount: 16,
  entityShards: {},
  keyExtractors: {},
  separator: ':',
  enabled: true,
}

/**
 * FNV-1a hash - fast, simple, good distribution
 * Used for consistent hashing to distribute keys across shards
 */
export function fnv1aHash(str: string): number {
  const FNV_OFFSET_BASIS = 2166136261
  const FNV_PRIME = 16777619

  let hash = FNV_OFFSET_BASIS

  for (let i = 0; i < str.length; i++) {
    hash ^= str.charCodeAt(i)
    hash = (hash * FNV_PRIME) >>> 0 // Convert to unsigned 32-bit
  }

  return hash
}

/**
 * Get shard index for a key using consistent hashing
 * @param key - The key to hash
 * @param shardCount - Number of shards
 * @returns Shard index (0 to shardCount - 1)
 */
export function getShardIndex(key: string, shardCount: number): number {
  if (shardCount <= 0) {
    throw new Error('Shard count must be positive')
  }
  if (shardCount === 1) {
    return 0
  }

  const hash = fnv1aHash(key)
  return hash % shardCount
}

/**
 * ShardRouter - Routes requests to sharded DO instances
 *
 * @example
 * ```typescript
 * const router = new ShardRouter({
 *   defaultShardCount: 16,
 *   entityShards: {
 *     'users': 32,      // Users sharded across 32 DOs
 *     'orders': 64,     // Orders across 64 DOs
 *     'analytics': 4,   // Analytics across 4 DOs
 *   }
 * })
 *
 * // Route a request
 * const { doName } = router.route({
 *   namespace: 'acme',
 *   path: '/users/user-123',
 *   entityType: 'users',
 *   entityId: 'user-123',
 * })
 * // doName might be 'acme:users:shard-7'
 * ```
 */
export class ShardRouter {
  private config: Required<ShardRouterConfig>

  constructor(config: Partial<ShardRouterConfig> = {}) {
    this.config = {
      ...DEFAULT_CONFIG,
      ...config,
      entityShards: {
        ...DEFAULT_CONFIG.entityShards,
        ...config.entityShards,
      },
      keyExtractors: {
        ...DEFAULT_CONFIG.keyExtractors,
        ...config.keyExtractors,
      },
    }
  }

  /**
   * Get shard count for an entity type
   */
  getShardCount(entityType?: string): number {
    if (entityType && this.config.entityShards[entityType] !== undefined) {
      return this.config.entityShards[entityType]
    }
    return this.config.defaultShardCount
  }

  /**
   * Extract shard key from context
   *
   * Priority:
   * 1. Explicit shardKey in context
   * 2. Custom key extractor for path
   * 3. EntityId if available
   * 4. Path-based extraction
   */
  extractKey(ctx: ShardContext): string | undefined {
    // 1. Explicit shard key
    if (ctx.shardKey) {
      return ctx.shardKey
    }

    // 2. Custom key extractor
    for (const [pattern, extractor] of Object.entries(this.config.keyExtractors)) {
      if (this.matchPattern(ctx.path, pattern)) {
        const key = extractor(ctx)
        if (key) return key
      }
    }

    // 3. Entity ID
    if (ctx.entityId) {
      return ctx.entityId
    }

    // 4. Path-based extraction (e.g., /users/123 → '123')
    return this.extractKeyFromPath(ctx.path)
  }

  /**
   * Match a path against a pattern
   * Supports simple wildcards: /users/* matches /users/123
   */
  private matchPattern(path: string, pattern: string): boolean {
    const patternParts = pattern.split('/')
    const pathParts = path.split('/')

    if (patternParts.length !== pathParts.length) {
      return false
    }

    for (let i = 0; i < patternParts.length; i++) {
      if (patternParts[i] === '*') continue
      if (patternParts[i] !== pathParts[i]) return false
    }

    return true
  }

  /**
   * Extract a key from the path
   * Default: uses the last path segment that looks like an ID
   */
  private extractKeyFromPath(path: string): string | undefined {
    const segments = path.split('/').filter(Boolean)

    // Look for ID-like segments (not pure route names)
    for (let i = segments.length - 1; i >= 0; i--) {
      const segment = segments[i]!
      // Skip common route names
      if (['api', 'v1', 'v2', 'rpc'].includes(segment)) continue
      // Return if it looks like an ID (has numbers, dashes, or is long)
      if (/\d/.test(segment) || segment.includes('-') || segment.length > 20) {
        return segment
      }
    }

    return undefined
  }

  /**
   * Extract entity type from path
   * Default: uses the first non-api path segment
   */
  private extractEntityType(path: string): string | undefined {
    const segments = path.split('/').filter(Boolean)

    for (const segment of segments) {
      // Skip common prefixes
      if (['api', 'v1', 'v2', 'rpc'].includes(segment)) continue
      // Return the first entity-like segment
      if (/^[a-z]+$/i.test(segment)) {
        return segment
      }
    }

    return undefined
  }

  /**
   * Route a request to the appropriate shard
   *
   * @param ctx - Shard context with request information
   * @returns Shard routing result with DO name
   */
  route(ctx: ShardContext): ShardResult {
    const { namespace, path } = ctx
    const separator = this.config.separator

    // If sharding is disabled, return namespace only
    if (!this.config.enabled) {
      return {
        doName: namespace,
        shardIndex: 0,
        sharded: false,
      }
    }

    // Extract entity type
    const entityType = ctx.entityType || this.extractEntityType(path)

    // Extract shard key
    const key = this.extractKey(ctx)

    // If no key found, return namespace only
    if (!key) {
      return {
        doName: namespace,
        shardIndex: 0,
        sharded: false,
      }
    }

    // Get shard count for this entity type
    const shardCount = this.getShardCount(entityType)

    // Calculate shard index
    const shardIndex = getShardIndex(key, shardCount)

    // Build DO name
    const parts = [namespace]
    if (entityType) {
      parts.push(entityType)
    }
    parts.push(`shard-${shardIndex}`)

    return {
      doName: parts.join(separator),
      shardIndex,
      sharded: true,
      key,
    }
  }

  /**
   * Get the DO stub for a sharded request
   *
   * @param env - Worker environment with DO binding
   * @param ctx - Shard context
   * @param binding - Name of the DO binding (default: 'DO')
   * @returns DO stub
   */
  getStub<T extends DurableObjectNamespace>(
    env: Record<string, T>,
    ctx: ShardContext,
    binding: string = 'DO'
  ): DurableObjectStub {
    const ns = env[binding]
    if (!ns) {
      throw new Error(`DO binding '${binding}' not found in environment`)
    }

    const { doName } = this.route(ctx)
    const id = ns.idFromName(doName)
    return ns.get(id)
  }

  /**
   * Create a shard-aware fetch handler
   *
   * @example
   * ```typescript
   * const router = new ShardRouter({ defaultShardCount: 16 })
   *
   * export default {
   *   fetch: router.createFetchHandler('DO')
   * }
   * ```
   */
  createFetchHandler(binding: string = 'DO') {
    return async (
      request: Request,
      env: Record<string, DurableObjectNamespace>
    ): Promise<Response> => {
      const url = new URL(request.url)
      const hostParts = url.hostname.split('.')
      const namespace = hostParts.length > 2 && hostParts[0] ? hostParts[0] : 'default'

      const ctx: ShardContext = {
        namespace,
        path: url.pathname,
        params: url.searchParams,
        headers: request.headers,
      }

      const stub = this.getStub(env, ctx, binding)
      return stub.fetch(request)
    }
  }
}

/**
 * Create a simple shard router with default configuration
 */
export function createShardRouter(config?: Partial<ShardRouterConfig>): ShardRouter {
  return new ShardRouter(config)
}

/**
 * Middleware for Hono to add shard context
 *
 * @example
 * ```typescript
 * import { shardMiddleware } from '@dotdo/do'
 *
 * const router = new ShardRouter({ defaultShardCount: 16 })
 * app.use('*', shardMiddleware(router))
 *
 * app.get('/users/:id', (c) => {
 *   const shardResult = c.get('shard')
 *   // shardResult.doName, shardResult.shardIndex, etc.
 * })
 * ```
 */
/**
 * Hono Context interface for shard middleware
 */
interface HonoShardContext {
  req: {
    url: string
    headers: Headers
    param: (name: string) => string | undefined
  }
  set: (key: string, value: unknown) => void
}

export function shardMiddleware(router: ShardRouter) {
  return async (c: HonoShardContext, next: () => Promise<void>) => {
    const url = new URL(c.req.url)
    const hostParts = url.hostname.split('.')
    const namespace = hostParts.length > 2 && hostParts[0] ? hostParts[0] : 'default'

    const entityType = c.req.param('entityType')
    const entityId = c.req.param('id') || c.req.param('entityId')
    const ctx: ShardContext = {
      namespace,
      path: url.pathname,
      params: url.searchParams,
      headers: c.req.headers,
      ...(entityType && { entityType }),
      ...(entityId && { entityId }),
    }

    const result = router.route(ctx)
    c.set('shard', result)
    c.set('shardContext', ctx)

    await next()
  }
}

/**
 * Route to shard by user ID header
 *
 * @example
 * ```typescript
 * const router = new ShardRouter({
 *   keyExtractors: {
 *     '/api/*': extractUserIdFromHeader,
 *   }
 * })
 * ```
 */
export function extractUserIdFromHeader(ctx: ShardContext): string | undefined {
  return ctx.headers?.get('X-User-ID') || ctx.headers?.get('Authorization')?.split(' ')[1]
}

/**
 * Route to shard by query parameter
 *
 * @example
 * ```typescript
 * const router = new ShardRouter({
 *   keyExtractors: {
 *     '/search': extractShardFromQuery('tenant_id'),
 *   }
 * })
 * ```
 */
export function extractShardFromQuery(paramName: string) {
  return (ctx: ShardContext): string | undefined => {
    return ctx.params?.get(paramName) || undefined
  }
}

// ============================================================================
// Load Metrics Store (do-ftgn)
// ============================================================================

export interface LoadMetricsStoreConfig {
  loadWeights?: Record<string, number>
  decayFactor?: number
  decayIntervalMs?: number
}

export interface LeastLoadedResult {
  shardIndex: number
  doName: string
  load: number
}

export class LoadMetricsStore {
  private loads: Map<string, number> = new Map()
  private metrics: Map<string, Map<string, number>> = new Map()
  private config: Required<LoadMetricsStoreConfig>
  private lastDecayTime: number = Date.now()

  constructor(config: LoadMetricsStoreConfig = {}) {
    this.config = {
      loadWeights: config.loadWeights ?? { requests: 1 },
      decayFactor: config.decayFactor ?? 0.9,
      decayIntervalMs: config.decayIntervalMs ?? 60000,
    }
  }

  recordLoad(doName: string, load: number): void {
    this.loads.set(doName, load)
  }

  recordRequest(doName: string): void {
    const current = this.loads.get(doName) ?? 0
    this.loads.set(doName, current + 1)
  }

  getLoad(doName: string): number {
    return this.loads.get(doName) ?? 0
  }

  recordMetric(doName: string, metricType: string, value: number): void {
    if (!this.metrics.has(doName)) {
      this.metrics.set(doName, new Map())
    }
    const shardMetrics = this.metrics.get(doName)
    if (shardMetrics) {
      shardMetrics.set(metricType, value)
    }
  }

  getMetric(doName: string, metricType: string): number {
    return this.metrics.get(doName)?.get(metricType) ?? 0
  }

  getCompositeLoad(doName: string): number {
    const shardMetrics = this.metrics.get(doName)
    if (!shardMetrics) return 0

    let compositeLoad = 0
    for (const [metricType, value] of shardMetrics) {
      const weight = this.config.loadWeights[metricType] ?? 1
      compositeLoad += value * weight
    }
    return compositeLoad
  }

  getShardLoads(
    namespace: string,
    entityType: string,
    shardCount: number
  ): Record<string, number> {
    const loads: Record<string, number> = {}
    for (let i = 0; i < shardCount; i++) {
      const doName = `${namespace}:${entityType}:shard-${i}`
      loads[doName] = this.getLoad(doName)
    }
    return loads
  }

  findLeastLoaded(
    namespace: string,
    entityType: string,
    shardCount: number
  ): LeastLoadedResult {
    let minLoad = Infinity
    let minIndex = 0
    let minDoName = `${namespace}:${entityType}:shard-0`

    for (let i = 0; i < shardCount; i++) {
      const doName = `${namespace}:${entityType}:shard-${i}`
      const load = this.getLoad(doName)
      if (load < minLoad) {
        minLoad = load
        minIndex = i
        minDoName = doName
      }
    }

    return {
      shardIndex: minIndex,
      doName: minDoName,
      load: minLoad === Infinity ? 0 : minLoad,
    }
  }

  applyDecay(): void {
    const now = Date.now()
    const elapsed = now - this.lastDecayTime

    if (elapsed >= this.config.decayIntervalMs) {
      const decayMultiplier = this.config.decayFactor

      for (const [doName, load] of this.loads) {
        this.loads.set(doName, Math.floor(load * decayMultiplier))
      }

      for (const [, shardMetrics] of this.metrics) {
        for (const [metricType, value] of shardMetrics) {
          shardMetrics.set(metricType, Math.floor(value * decayMultiplier))
        }
      }

      this.lastDecayTime = now
    }
  }

  reset(): void {
    this.loads.clear()
    this.metrics.clear()
    this.lastDecayTime = Date.now()
  }

  getSnapshot(): Record<string, number> {
    const snapshot: Record<string, number> = {}
    for (const [doName, load] of this.loads) {
      snapshot[doName] = load
    }
    return snapshot
  }
}

// ============================================================================
// Load Balanced Router (do-ftgn)
// ============================================================================

export interface LoadBalanceTelemetryEvent {
  type: 'load_balance_decision'
  namespace: string
  entityType?: string | undefined
  selectedShard: number
  selectedDoName: string
  loadSnapshot: Record<string, number>
  strategy: 'least-loaded' | 'weighted' | 'round-robin'
  timestamp: number
}

export interface LoadBalancedShardResult extends ShardResult {
  loadBalanced: boolean
}

export interface LoadBalancedRouterConfig extends ShardRouterConfig {
  metricsStore?: LoadMetricsStore
  strategy?: 'least-loaded' | 'weighted' | 'round-robin'
  weights?: Record<string, number>
  onTelemetry?: (event: LoadBalanceTelemetryEvent) => void
}

export class LoadBalancedRouter extends ShardRouter {
  private metricsStore: LoadMetricsStore
  private lbConfig: LoadBalancedRouterConfig
  private roundRobinCounter: Map<string, number> = new Map()

  constructor(config: Partial<LoadBalancedRouterConfig> & { defaultShardCount?: number } = {}) {
    super({ defaultShardCount: config.defaultShardCount ?? 1, ...config } as LoadBalancedRouterConfig)
    this.lbConfig = {
      defaultShardCount: config.defaultShardCount ?? 1,
      strategy: config.strategy ?? 'least-loaded',
      weights: config.weights ?? {},
      metricsStore: config.metricsStore,
      onTelemetry: config.onTelemetry,
      separator: config.separator,
      enabled: config.enabled,
    } as LoadBalancedRouterConfig
    this.metricsStore = config.metricsStore ?? new LoadMetricsStore()
  }

  route(ctx: ShardContext): LoadBalancedShardResult {
    const { namespace, path } = ctx
    const separator = (this as unknown as { config: ShardRouterConfig }).config?.separator ?? ':'

    if ((this as unknown as { config: ShardRouterConfig }).config?.enabled === false) {
      return {
        doName: namespace,
        shardIndex: 0,
        sharded: false,
        loadBalanced: false,
      }
    }

    const entityType =
      ctx.entityType ||
      (
        this as unknown as { extractEntityType: (path: string) => string | undefined }
      ).extractEntityType?.(path)

    const key = this.extractKey(ctx)

    if (ctx.entityId || key) {
      const baseResult = super.route(ctx)
      return {
        ...baseResult,
        loadBalanced: false,
      }
    }

    const shardCount = this.getShardCount(entityType)
    const { shardIndex, doName } = this.selectShard(
      namespace,
      entityType,
      shardCount,
      separator
    )

    const loadSnapshot = this.buildLoadSnapshot(
      namespace,
      entityType,
      shardCount,
      separator
    )

    if (this.lbConfig.onTelemetry) {
      this.lbConfig.onTelemetry({
        type: 'load_balance_decision',
        namespace,
        entityType,
        selectedShard: shardIndex,
        selectedDoName: doName,
        loadSnapshot,
        strategy: this.lbConfig.strategy!,
        timestamp: Date.now(),
      })
    }

    return {
      doName,
      shardIndex,
      sharded: true,
      loadBalanced: true,
    }
  }

  private selectShard(
    namespace: string,
    entityType: string | undefined,
    shardCount: number,
    separator: string
  ): { shardIndex: number; doName: string } {
    const prefix = entityType
      ? `${namespace}${separator}${entityType}`
      : namespace

    switch (this.lbConfig.strategy) {
      case 'weighted':
        return this.selectWeightedShard(prefix, shardCount, separator)
      case 'round-robin':
        return this.selectRoundRobinShard(prefix, shardCount, separator)
      case 'least-loaded':
      default:
        return this.selectLeastLoadedShard(prefix, shardCount, separator)
    }
  }

  private selectLeastLoadedShard(
    prefix: string,
    shardCount: number,
    separator: string
  ): { shardIndex: number; doName: string } {
    let minLoad = Infinity
    let minIndex = 0
    const equalLoadIndices: number[] = []

    for (let i = 0; i < shardCount; i++) {
      const doName = `${prefix}${separator}shard-${i}`
      const load = this.metricsStore.getLoad(doName)

      if (load < minLoad) {
        minLoad = load
        minIndex = i
        equalLoadIndices.length = 0
        equalLoadIndices.push(i)
      } else if (load === minLoad) {
        equalLoadIndices.push(i)
      }
    }

    if (equalLoadIndices.length > 1) {
      const rrKey = `${prefix}:rr`
      const rrIndex =
        (this.roundRobinCounter.get(rrKey) ?? 0) % equalLoadIndices.length
      this.roundRobinCounter.set(rrKey, rrIndex + 1)
      minIndex = equalLoadIndices[rrIndex] ?? minIndex
    }

    return {
      shardIndex: minIndex,
      doName: `${prefix}${separator}shard-${minIndex}`,
    }
  }

  private selectWeightedShard(
    prefix: string,
    shardCount: number,
    separator: string
  ): { shardIndex: number; doName: string } {
    let minEffectiveLoad = Infinity
    let minIndex = 0

    for (let i = 0; i < shardCount; i++) {
      const doName = `${prefix}${separator}shard-${i}`
      const load = this.metricsStore.getLoad(doName)
      const weight = this.lbConfig.weights?.[doName] ?? 1
      const effectiveLoad = load / weight

      if (effectiveLoad < minEffectiveLoad) {
        minEffectiveLoad = effectiveLoad
        minIndex = i
      }
    }

    return {
      shardIndex: minIndex,
      doName: `${prefix}${separator}shard-${minIndex}`,
    }
  }

  private selectRoundRobinShard(
    prefix: string,
    shardCount: number,
    separator: string
  ): { shardIndex: number; doName: string } {
    const rrKey = `${prefix}:rr`
    const index = (this.roundRobinCounter.get(rrKey) ?? 0) % shardCount
    this.roundRobinCounter.set(rrKey, index + 1)

    return {
      shardIndex: index,
      doName: `${prefix}${separator}shard-${index}`,
    }
  }

  private buildLoadSnapshot(
    namespace: string,
    entityType: string | undefined,
    shardCount: number,
    separator: string
  ): Record<string, number> {
    const prefix = entityType
      ? `${namespace}${separator}${entityType}`
      : namespace
    const snapshot: Record<string, number> = {}

    for (let i = 0; i < shardCount; i++) {
      const doName = `${prefix}${separator}shard-${i}`
      snapshot[doName] = this.metricsStore.getLoad(doName)
    }

    return snapshot
  }

  getMetricsStore(): LoadMetricsStore {
    return this.metricsStore
  }
}

export function createLoadBalancedRouter(
  config?: LoadBalancedRouterConfig
): LoadBalancedRouter {
  return new LoadBalancedRouter(config)
}

// ============================================================================
// Shard Health Monitoring (do-8x8l)
// ============================================================================

/**
 * Shard health status
 */
export type ShardHealthStatus = 'healthy' | 'degraded' | 'unhealthy'

/**
 * Configuration for health monitoring thresholds
 */
export interface ShardHealthConfig {
  /** Latency threshold for degraded status (ms) */
  latencyDegradedThreshold?: number
  /** Latency threshold for unhealthy status (ms) */
  latencyUnhealthyThreshold?: number
  /** Error rate threshold for degraded status (0-1) */
  errorRateDegradedThreshold?: number
  /** Error rate threshold for unhealthy status (0-1) */
  errorRateUnhealthyThreshold?: number
  /** Storage percent threshold for degraded status (0-1) */
  storageDegradedThreshold?: number
  /** Storage percent threshold for unhealthy status (0-1) */
  storageUnhealthyThreshold?: number
  /** Sliding window size for latency measurements */
  latencyWindowSize?: number
  /** Sliding window size for error tracking */
  errorWindowSize?: number
  /** Weights for calculating composite health score */
  healthScoreWeights?: {
    latency: number
    errorRate: number
    storage: number
    connections: number
  }
}

/**
 * Default health configuration
 */
const DEFAULT_HEALTH_CONFIG: Required<ShardHealthConfig> = {
  latencyDegradedThreshold: 500,
  latencyUnhealthyThreshold: 2000,
  errorRateDegradedThreshold: 0.05,
  errorRateUnhealthyThreshold: 0.2,
  storageDegradedThreshold: 0.7,
  storageUnhealthyThreshold: 0.9,
  latencyWindowSize: 100,
  errorWindowSize: 100,
  healthScoreWeights: {
    latency: 0.4,
    errorRate: 0.3,
    storage: 0.2,
    connections: 0.1,
  },
}

/**
 * Health metrics for a single shard
 */
export interface ShardHealthMetrics {
  shardId: string
  /** Average latency in ms */
  latencyMs: number
  /** Error rate (0-1) */
  errorRate: number
  /** Storage bytes used */
  storageBytes: number
  /** Storage percent used (0-1) */
  storagePercent: number
  /** Active WebSocket/HTTP connections */
  activeConnections: number
  /** Requests per second */
  requestsPerSecond: number
  /** Overall health score (0-100) */
  healthScore: number
  /** Current health status */
  status: ShardHealthStatus
  /** Timestamp of last update */
  lastUpdatedAt: number
}

/**
 * Internal tracking data for a shard
 */
interface ShardHealthData {
  latencyWindow: number[]
  successCount: number
  errorCount: number
  storageBytes: number
  storagePercent: number
  activeConnections: number
  requestsPerSecond: number
  lastUpdatedAt: number
}

/**
 * ShardHealthMonitor - Monitors health metrics for shards
 *
 * Tracks latency, error rates, storage usage, and connections
 * to determine shard health status and enable smart routing decisions.
 *
 * @example
 * ```typescript
 * const monitor = new ShardHealthMonitor()
 *
 * // Record metrics
 * monitor.recordLatency('shard-0', 150)
 * monitor.recordSuccess('shard-0')
 * monitor.recordStorage('shard-0', 500 * 1024 * 1024, 0.5)
 *
 * // Get health status
 * const metrics = monitor.getHealthMetrics('shard-0')
 * if (metrics?.status === 'unhealthy') {
 *   // Route traffic away from this shard
 * }
 * ```
 */
export class ShardHealthMonitor {
  private config: Required<ShardHealthConfig>
  private shards: Map<string, ShardHealthData> = new Map()

  constructor(config: ShardHealthConfig = {}) {
    this.config = {
      ...DEFAULT_HEALTH_CONFIG,
      ...config,
      healthScoreWeights: {
        ...DEFAULT_HEALTH_CONFIG.healthScoreWeights,
        ...config.healthScoreWeights,
      },
    }
  }

  /**
   * Record a latency measurement for a shard
   */
  recordLatency(shardId: string, latencyMs: number): void {
    const data = this.getOrCreateShardData(shardId)
    data.latencyWindow.push(latencyMs)
    // Maintain sliding window
    if (data.latencyWindow.length > this.config.latencyWindowSize) {
      data.latencyWindow.shift()
    }
    data.lastUpdatedAt = Date.now()
  }

  /**
   * Record a successful request
   */
  recordSuccess(shardId: string): void {
    const data = this.getOrCreateShardData(shardId)
    data.successCount++
    // Maintain sliding window by decaying old counts
    this.maintainErrorWindow(data)
    data.lastUpdatedAt = Date.now()
  }

  /**
   * Record an error
   */
  recordError(shardId: string): void {
    const data = this.getOrCreateShardData(shardId)
    data.errorCount++
    this.maintainErrorWindow(data)
    data.lastUpdatedAt = Date.now()
  }

  /**
   * Record storage usage
   */
  recordStorage(shardId: string, bytes: number, percent: number): void {
    const data = this.getOrCreateShardData(shardId)
    data.storageBytes = bytes
    data.storagePercent = Math.max(0, Math.min(1, percent))
    data.lastUpdatedAt = Date.now()
  }

  /**
   * Record active connection count
   */
  recordConnections(shardId: string, count: number): void {
    const data = this.getOrCreateShardData(shardId)
    data.activeConnections = count
    data.lastUpdatedAt = Date.now()
  }

  /**
   * Record requests per second
   */
  recordRequestsPerSecond(shardId: string, rps: number): void {
    const data = this.getOrCreateShardData(shardId)
    data.requestsPerSecond = rps
    data.lastUpdatedAt = Date.now()
  }

  /**
   * Get health metrics for a shard
   */
  getHealthMetrics(shardId: string): ShardHealthMetrics | undefined {
    const data = this.shards.get(shardId)
    if (!data) return undefined

    const latencyMs = this.calculateAverageLatency(data)
    const errorRate = this.calculateErrorRate(data)
    const healthScore = this.calculateHealthScore(latencyMs, errorRate, data)
    const status = this.determineStatus(latencyMs, errorRate, data.storagePercent)

    return {
      shardId,
      latencyMs,
      errorRate,
      storageBytes: data.storageBytes,
      storagePercent: data.storagePercent,
      activeConnections: data.activeConnections,
      requestsPerSecond: data.requestsPerSecond,
      healthScore,
      status,
      lastUpdatedAt: data.lastUpdatedAt,
    }
  }

  /**
   * Get all health metrics
   */
  getAllHealthMetrics(): Map<string, ShardHealthMetrics> {
    const result = new Map<string, ShardHealthMetrics>()
    for (const shardId of this.shards.keys()) {
      const metrics = this.getHealthMetrics(shardId)
      if (metrics) {
        result.set(shardId, metrics)
      }
    }
    return result
  }

  /**
   * Get unhealthy shards
   */
  getUnhealthyShards(): ShardHealthMetrics[] {
    const result: ShardHealthMetrics[] = []
    for (const shardId of this.shards.keys()) {
      const metrics = this.getHealthMetrics(shardId)
      if (metrics?.status === 'unhealthy') {
        result.push(metrics)
      }
    }
    return result
  }

  /**
   * Get degraded shards (includes unhealthy)
   */
  getDegradedShards(): ShardHealthMetrics[] {
    const result: ShardHealthMetrics[] = []
    for (const shardId of this.shards.keys()) {
      const metrics = this.getHealthMetrics(shardId)
      if (metrics && (metrics.status === 'degraded' || metrics.status === 'unhealthy')) {
        result.push(metrics)
      }
    }
    return result
  }

  /**
   * Get the healthiest shard from a list of candidates
   */
  getHealthiestShard(candidates: string[]): string | undefined {
    let bestShard: string | undefined
    let bestScore = -1

    for (const shardId of candidates) {
      const metrics = this.getHealthMetrics(shardId)
      const score = metrics?.healthScore ?? 100 // Unknown shards assumed healthy
      if (score > bestScore) {
        bestScore = score
        bestShard = shardId
      }
    }

    return bestShard
  }

  /**
   * Check if a shard is healthy enough for routing
   */
  isShardHealthy(shardId: string): boolean {
    const metrics = this.getHealthMetrics(shardId)
    // Unknown shards are assumed healthy
    if (!metrics) return true
    return metrics.status !== 'unhealthy'
  }

  /**
   * Reset all metrics
   */
  reset(): void {
    this.shards.clear()
  }

  private getOrCreateShardData(shardId: string): ShardHealthData {
    let data = this.shards.get(shardId)
    if (!data) {
      data = {
        latencyWindow: [],
        successCount: 0,
        errorCount: 0,
        storageBytes: 0,
        storagePercent: 0,
        activeConnections: 0,
        requestsPerSecond: 0,
        lastUpdatedAt: Date.now(),
      }
      this.shards.set(shardId, data)
    }
    return data
  }

  private maintainErrorWindow(data: ShardHealthData): void {
    const total = data.successCount + data.errorCount
    if (total > this.config.errorWindowSize) {
      // Scale down both counts proportionally
      const scale = this.config.errorWindowSize / total
      data.successCount = Math.floor(data.successCount * scale)
      data.errorCount = Math.floor(data.errorCount * scale)
    }
  }

  private calculateAverageLatency(data: ShardHealthData): number {
    if (data.latencyWindow.length === 0) return 0
    const sum = data.latencyWindow.reduce((a, b) => a + b, 0)
    return Math.round(sum / data.latencyWindow.length)
  }

  private calculateErrorRate(data: ShardHealthData): number {
    const total = data.successCount + data.errorCount
    if (total === 0) return 0
    return data.errorCount / total
  }

  private calculateHealthScore(latencyMs: number, errorRate: number, data: ShardHealthData): number {
    const weights = this.config.healthScoreWeights

    // Latency score: 100 at 0ms, 0 at unhealthy threshold
    const latencyScore = Math.max(0, 100 - (latencyMs / this.config.latencyUnhealthyThreshold) * 100)

    // Error rate score: 100 at 0%, 0 at 50%
    const errorScore = Math.max(0, 100 - errorRate * 200)

    // Storage score: 100 at 0%, 0 at 100%
    const storageScore = Math.max(0, 100 - data.storagePercent * 100)

    // Connection score: 100 at 0, decreases with more connections
    // Assume 1000 connections is heavy load
    const connectionScore = Math.max(0, 100 - (data.activeConnections / 1000) * 100)

    const totalWeight = weights.latency + weights.errorRate + weights.storage + weights.connections
    const weightedScore =
      (latencyScore * weights.latency +
        errorScore * weights.errorRate +
        storageScore * weights.storage +
        connectionScore * weights.connections) /
      totalWeight

    return Math.round(weightedScore)
  }

  private determineStatus(latencyMs: number, errorRate: number, storagePercent: number): ShardHealthStatus {
    // Check unhealthy conditions first
    if (
      latencyMs >= this.config.latencyUnhealthyThreshold ||
      errorRate >= this.config.errorRateUnhealthyThreshold ||
      storagePercent >= this.config.storageUnhealthyThreshold
    ) {
      return 'unhealthy'
    }

    // Check degraded conditions
    if (
      latencyMs >= this.config.latencyDegradedThreshold ||
      errorRate >= this.config.errorRateDegradedThreshold ||
      storagePercent >= this.config.storageDegradedThreshold
    ) {
      return 'degraded'
    }

    return 'healthy'
  }
}

/**
 * Factory function for ShardHealthMonitor
 */
export function createShardHealthMonitor(config?: ShardHealthConfig): ShardHealthMonitor {
  return new ShardHealthMonitor(config)
}

// ============================================================================
// Shard Rebalancer (do-8x8l)
// ============================================================================

/**
 * Shard registry entry status
 */
export type ShardStatus = 'active' | 'draining' | 'inactive' | 'migrating'

/**
 * Shard registry entry
 */
export interface ShardRegistryEntry {
  shardId: string
  namespace: string
  entityType?: string
  shardIndex: number
  status: ShardStatus
  createdAt: number
  lastActivityAt: number
}

/**
 * Rebalance action types
 */
export type RebalanceAction =
  | { type: 'split'; sourceShardId: string; newShardCount: number }
  | { type: 'merge'; sourceShardIds: string[]; targetShardId: string }
  | { type: 'drain'; shardId: string; targetShardId: string }

/**
 * Rebalance decision with priority
 */
export interface RebalanceDecision {
  action: RebalanceAction
  reason: string
  priority: 'low' | 'medium' | 'high' | 'critical'
  estimatedImpact: {
    keysAffected?: number
    downtime?: number
  }
}

/**
 * Migration task for tracking data movement
 */
export interface MigrationTask {
  id: string
  sourceShardId: string
  targetShardId: string
  keys: string[]
  keysTotal: number
  keysMigrated: number
  status: 'pending' | 'in_progress' | 'completed' | 'failed'
  startedAt?: number
  completedAt?: number
  error?: string
}

/**
 * Rebalancer configuration
 */
export interface ShardRebalancerConfig {
  /** Minimum number of shards to maintain */
  minShardCount?: number
  /** Maximum number of shards allowed */
  maxShardCount?: number
  /** Load ratio threshold to trigger split (e.g., 2.0 = 200% of average) */
  splitThreshold?: number
  /** Load ratio threshold to trigger merge (e.g., 0.1 = 10% of average) */
  mergeThreshold?: number
  /** Cooldown period between rebalancing operations (ms) */
  cooldownMs?: number
}

/**
 * Default rebalancer configuration
 */
const DEFAULT_REBALANCER_CONFIG: Required<ShardRebalancerConfig> = {
  minShardCount: 1,
  maxShardCount: 256,
  splitThreshold: 2.0,
  mergeThreshold: 0.1,
  cooldownMs: 60000,
}

/**
 * ShardRebalancer - Analyzes and manages shard rebalancing
 *
 * Provides:
 * - Shard registry management
 * - Load analysis and rebalancing recommendations
 * - Migration task tracking
 * - Gradual migration strategy
 *
 * @example
 * ```typescript
 * const rebalancer = new ShardRebalancer({
 *   minShardCount: 4,
 *   maxShardCount: 64,
 *   splitThreshold: 2.0,
 * })
 *
 * // Register shards
 * rebalancer.registerShard({ shardId: 'shard-0', namespace: 'acme', ... })
 *
 * // Analyze and get recommendations
 * const decisions = rebalancer.analyze(metricsStore, healthMonitor)
 * for (const decision of decisions) {
 *   if (decision.priority === 'critical') {
 *     // Execute immediately
 *   }
 * }
 * ```
 */
export class ShardRebalancer {
  private config: Required<ShardRebalancerConfig>
  private registry: Map<string, ShardRegistryEntry> = new Map()
  private migrations: Map<string, MigrationTask> = new Map()
  private lastRebalanceAt: number = 0

  constructor(config: ShardRebalancerConfig = {}) {
    this.config = {
      ...DEFAULT_REBALANCER_CONFIG,
      ...config,
    }
  }

  /**
   * Register a shard in the registry
   */
  registerShard(entry: Omit<ShardRegistryEntry, 'createdAt' | 'lastActivityAt'>): void {
    const now = Date.now()
    this.registry.set(entry.shardId, {
      ...entry,
      createdAt: now,
      lastActivityAt: now,
    })
  }

  /**
   * Get a shard registry entry
   */
  getShardEntry(shardId: string): ShardRegistryEntry | undefined {
    return this.registry.get(shardId)
  }

  /**
   * Update shard activity timestamp
   */
  updateShardActivity(shardId: string): void {
    const entry = this.registry.get(shardId)
    if (entry) {
      entry.lastActivityAt = Date.now()
    }
  }

  /**
   * Set shard status
   */
  setShardStatus(shardId: string, status: ShardStatus): void {
    const entry = this.registry.get(shardId)
    if (entry) {
      entry.status = status
    }
  }

  /**
   * Get active shards for a namespace (optionally filtered by entity type)
   */
  getActiveShards(namespace: string, entityType?: string): ShardRegistryEntry[] {
    const result: ShardRegistryEntry[] = []
    for (const entry of this.registry.values()) {
      if (entry.namespace !== namespace) continue
      if (entityType !== undefined && entry.entityType !== entityType) continue
      if (entry.status === 'active') {
        result.push(entry)
      }
    }
    return result
  }

  /**
   * Analyze current load distribution and recommend rebalancing actions
   */
  analyze(
    metricsStore: LoadMetricsStore,
    healthMonitor?: ShardHealthMonitor
  ): RebalanceDecision[] {
    // Check cooldown
    if (this.isInCooldown()) {
      return []
    }

    const decisions: RebalanceDecision[] = []

    // Group shards by namespace + entityType
    const groups = this.groupShards()

    for (const [groupKey, entries] of groups) {
      const activeEntries = entries.filter((e) => e.status === 'active')
      if (activeEntries.length === 0) continue

      // Analyze health first (critical priority)
      if (healthMonitor) {
        const healthDecisions = this.analyzeHealth(activeEntries, healthMonitor)
        decisions.push(...healthDecisions)
      }

      // Analyze load distribution
      const loadDecisions = this.analyzeLoad(activeEntries, metricsStore)
      decisions.push(...loadDecisions)
    }

    // Sort by priority: critical > high > medium > low
    const priorityOrder = { critical: 0, high: 1, medium: 2, low: 3 }
    decisions.sort((a, b) => priorityOrder[a.priority] - priorityOrder[b.priority])

    return decisions
  }

  /**
   * Record that a rebalance operation was performed
   */
  recordRebalance(): void {
    this.lastRebalanceAt = Date.now()
  }

  /**
   * Get remaining cooldown time in ms
   */
  getCooldownRemaining(): number {
    const elapsed = Date.now() - this.lastRebalanceAt
    return Math.max(0, this.config.cooldownMs - elapsed)
  }

  /**
   * Start a migration task
   */
  startMigration(sourceShardId: string, targetShardId: string, keys: string[]): MigrationTask {
    const task: MigrationTask = {
      id: `migration-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`,
      sourceShardId,
      targetShardId,
      keys,
      keysTotal: keys.length,
      keysMigrated: 0,
      status: 'pending',
    }
    this.migrations.set(task.id, task)
    return task
  }

  /**
   * Update migration progress
   */
  updateMigrationProgress(taskId: string, keysMigrated: number): void {
    const task = this.migrations.get(taskId)
    if (task) {
      task.keysMigrated = keysMigrated
      if (task.status === 'pending') {
        task.status = 'in_progress'
        task.startedAt = Date.now()
      }
    }
  }

  /**
   * Complete a migration (success or failure)
   */
  completeMigration(taskId: string, error?: string): void {
    const task = this.migrations.get(taskId)
    if (task) {
      task.completedAt = Date.now()
      if (error) {
        task.status = 'failed'
        task.error = error
      } else {
        task.status = 'completed'
      }
    }
  }

  /**
   * Get a migration task
   */
  getMigration(taskId: string): MigrationTask | undefined {
    return this.migrations.get(taskId)
  }

  /**
   * Get active migrations
   */
  getActiveMigrations(): MigrationTask[] {
    const result: MigrationTask[] = []
    for (const task of this.migrations.values()) {
      if (task.status === 'pending' || task.status === 'in_progress') {
        result.push(task)
      }
    }
    return result
  }

  /**
   * Reset all state
   */
  reset(): void {
    this.registry.clear()
    this.migrations.clear()
    this.lastRebalanceAt = 0
  }

  private isInCooldown(): boolean {
    return this.getCooldownRemaining() > 0
  }

  private groupShards(): Map<string, ShardRegistryEntry[]> {
    const groups = new Map<string, ShardRegistryEntry[]>()
    for (const entry of this.registry.values()) {
      const key = entry.entityType
        ? `${entry.namespace}:${entry.entityType}`
        : entry.namespace
      const group = groups.get(key) || []
      group.push(entry)
      groups.set(key, group)
    }
    return groups
  }

  private analyzeHealth(
    entries: ShardRegistryEntry[],
    healthMonitor: ShardHealthMonitor
  ): RebalanceDecision[] {
    const decisions: RebalanceDecision[] = []
    const healthyShards = entries.filter((e) => healthMonitor.isShardHealthy(e.shardId))

    for (const entry of entries) {
      if (!healthMonitor.isShardHealthy(entry.shardId)) {
        // Find a healthy target shard
        const targetShard = healthyShards.find((h) => h.shardId !== entry.shardId)
        if (targetShard) {
          decisions.push({
            action: {
              type: 'drain',
              shardId: entry.shardId,
              targetShardId: targetShard.shardId,
            },
            reason: `Shard ${entry.shardId} is unhealthy`,
            priority: 'critical',
            estimatedImpact: {},
          })
        }
      }
    }

    return decisions
  }

  private analyzeLoad(
    entries: ShardRegistryEntry[],
    metricsStore: LoadMetricsStore
  ): RebalanceDecision[] {
    const decisions: RebalanceDecision[] = []

    // Calculate average load
    let totalLoad = 0
    for (const entry of entries) {
      totalLoad += metricsStore.getLoad(entry.shardId)
    }
    const averageLoad = totalLoad / entries.length

    // Skip if no meaningful load
    if (averageLoad === 0) return decisions

    // Find hot spots (candidates for split)
    for (const entry of entries) {
      const load = metricsStore.getLoad(entry.shardId)
      const loadRatio = load / averageLoad

      if (loadRatio >= this.config.splitThreshold) {
        // Check if we can split (not at max)
        if (entries.length < this.config.maxShardCount) {
          decisions.push({
            action: {
              type: 'split',
              sourceShardId: entry.shardId,
              newShardCount: entries.length + 1,
            },
            reason: `Shard ${entry.shardId} is hot (${Math.round(loadRatio * 100)}% of average load)`,
            priority: 'high',
            estimatedImpact: {},
          })
        }
      }
    }

    // Find cold spots (candidates for merge)
    const coldShards: ShardRegistryEntry[] = []
    for (const entry of entries) {
      const load = metricsStore.getLoad(entry.shardId)
      const loadRatio = load / averageLoad

      if (loadRatio <= this.config.mergeThreshold) {
        coldShards.push(entry)
      }
    }

    // Recommend merge if we have multiple cold shards and above min count
    if (coldShards.length >= 2 && entries.length > this.config.minShardCount) {
      const sourceIds = coldShards.map((s) => s.shardId)
      decisions.push({
        action: {
          type: 'merge',
          sourceShardIds: sourceIds,
          targetShardId: coldShards[0]!.shardId,
        },
        reason: `${coldShards.length} shards are underutilized (below ${this.config.mergeThreshold * 100}% of average)`,
        priority: 'low',
        estimatedImpact: {},
      })
    }

    return decisions
  }
}

/**
 * Factory function for ShardRebalancer
 */
export function createShardRebalancer(config?: ShardRebalancerConfig): ShardRebalancer {
  return new ShardRebalancer(config)
}

// ============================================================================
// Health-Aware Router (do-8x8l)
// ============================================================================

/**
 * Configuration for health-aware routing
 */
export interface HealthAwareRouterConfig extends LoadBalancedRouterConfig {
  /** Health monitor instance (created if not provided) */
  healthMonitor?: ShardHealthMonitor
  /** Skip unhealthy shards when routing (default: true) */
  skipUnhealthyShards?: boolean
  /** Prefer healthier shards when loads are similar (default: true) */
  preferHealthierShards?: boolean
}

/**
 * HealthAwareRouter - Load balanced router with health awareness
 *
 * Extends LoadBalancedRouter to consider shard health status when routing.
 * Automatically avoids unhealthy shards and prefers healthier alternatives.
 *
 * @example
 * ```typescript
 * const router = new HealthAwareRouter({
 *   defaultShardCount: 4,
 *   skipUnhealthyShards: true,
 *   preferHealthierShards: true,
 * })
 *
 * // Health is tracked automatically or can be updated manually
 * router.getHealthMonitor().recordLatency('shard-0', 3000) // Marks as unhealthy
 *
 * // Routing will avoid shard-0
 * const result = router.route({ namespace: 'acme', path: '/users' })
 * ```
 */
export class HealthAwareRouter extends LoadBalancedRouter {
  private healthMonitor: ShardHealthMonitor
  private healthConfig: HealthAwareRouterConfig

  constructor(config: Partial<HealthAwareRouterConfig> = {}) {
    super(config)
    this.healthConfig = {
      skipUnhealthyShards: config.skipUnhealthyShards ?? true,
      preferHealthierShards: config.preferHealthierShards ?? true,
      ...config,
    }
    this.healthMonitor = config.healthMonitor ?? new ShardHealthMonitor()
  }

  /**
   * Route with health awareness
   */
  route(ctx: ShardContext): LoadBalancedShardResult {
    // Use base routing first
    const baseResult = super.route(ctx)

    // If not load balanced (e.g., has entityId), return as-is
    // Consistent hashing must be preserved for existing entities
    if (!baseResult.loadBalanced) {
      return baseResult
    }

    // If skip unhealthy is disabled, return base result
    if (!this.healthConfig.skipUnhealthyShards) {
      return baseResult
    }

    // Check if selected shard is healthy
    if (this.healthMonitor.isShardHealthy(baseResult.doName)) {
      return baseResult
    }

    // Selected shard is unhealthy - find an alternative
    const shardCount = this.getShardCount(ctx.entityType)
    const namespace = ctx.namespace
    const entityType = ctx.entityType
    const separator = ':'

    // Build list of healthy alternatives
    const prefix = entityType
      ? `${namespace}${separator}${entityType}`
      : namespace
    const healthyAlternatives: string[] = []

    for (let i = 0; i < shardCount; i++) {
      const doName = `${prefix}${separator}shard-${i}`
      if (this.healthMonitor.isShardHealthy(doName)) {
        healthyAlternatives.push(doName)
      }
    }

    // If no healthy alternatives, fall back to original selection
    if (healthyAlternatives.length === 0) {
      return baseResult
    }

    // Find the healthiest (or least loaded) alternative
    let bestDoName = healthyAlternatives[0]!
    let bestScore = -1

    if (this.healthConfig.preferHealthierShards) {
      // Pick healthiest
      for (const doName of healthyAlternatives) {
        const metrics = this.healthMonitor.getHealthMetrics(doName)
        const score = metrics?.healthScore ?? 100
        if (score > bestScore) {
          bestScore = score
          bestDoName = doName
        }
      }
    } else {
      // Pick least loaded
      let bestLoad = Infinity
      const metricsStore = this.getMetricsStore()
      for (const doName of healthyAlternatives) {
        const load = metricsStore.getLoad(doName)
        if (load < bestLoad) {
          bestLoad = load
          bestDoName = doName
        }
      }
    }

    // Extract shard index from doName
    const shardMatch = bestDoName.match(/shard-(\d+)$/)
    const shardIndex = shardMatch ? parseInt(shardMatch[1]!, 10) : 0

    return {
      doName: bestDoName,
      shardIndex,
      sharded: true,
      loadBalanced: true,
    }
  }

  /**
   * Get the health monitor instance
   */
  getHealthMonitor(): ShardHealthMonitor {
    return this.healthMonitor
  }
}

/**
 * Factory function for HealthAwareRouter
 */
export function createHealthAwareRouter(config?: HealthAwareRouterConfig): HealthAwareRouter {
  return new HealthAwareRouter(config)
}
