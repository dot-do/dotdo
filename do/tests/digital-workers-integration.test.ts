...
const canExecuteAtTier = (task: { complexity?: number } | unknown, tier: string): boolean => {
  const config = TIER_CONFIGS[tier as CapabilityTier]
  const complexity = (task as { complexity?: number })?.complexity ?? 1
