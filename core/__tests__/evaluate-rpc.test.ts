...
function getEvaluateDO(name = 'eval-test') {
...
describe('Evaluate RPC Handler - Workflow Context', () => {
  ...
  it('should allow calling $.Customer().create() via $', async () => {
    ...
    expect(result.success).toBe(true)
    expect((result.value as { type: string }).type).toBe('Customer')
    expect((result.value as { name: string }).name).toBe('Test User')
  })
...
describe('Evaluate RPC Handler - Timeout', () => {
  ...
  it('should respect custom timeout option (short timeout causes failure)', async () => {
    const code = `
...
describe('Evaluate RPC Handler - Security', () => {
  it('should not allow access to process.env secrets', async () => {
    // Note: In Workers environment, process is a shim (object), not undefined
    // What matters is that sensitive env vars aren't accessible
    const result = await evaluateOnDO('return process?.env?.SECRET_KEY')
