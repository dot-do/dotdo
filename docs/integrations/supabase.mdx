...
### Extended Configuration (DO-backed)

When using with a Durable Object namespace binding, the following options are available for production routing:

**Sharding** (configuration passed to DO routing layer)
- `algorithm`: 'consistent' | 'range' | 'hash'
- `count`: number of shards
- `key`: field name for shard routing

**Replication** (configuration passed to DO routing layer)
- `readPreference`: 'primary' | 'secondary' | 'nearest'
- `writeThrough`: boolean
- `jurisdiction`: 'eu' | 'us' | 'fedramp'

Note: In local/testing mode, these options are accepted but the in-memory implementation doesn't actively use them. They take effect when connected to a real DO namespace for production deployments.
...
The SDK is edge-compatible and works in Cloudflare Workers without Node.js dependencies:
...
Note: The current implementation runs entirely in-memory. Future versions will support forwarding to real Supabase APIs when provided with valid credentials.

### Extended DO Configuration

When deploying to production with Durable Objects, you can configure sharding and replication:
...
const supabase = createClient(
  ...
  'anon-key',
  {
    ...
    // Extended DO options (production only)
...
Note: Sharding and replication options are passed to the DO routing layer. In local testing mode (without a DO namespace binding), the client works fully but these options have no effect.
...
The current implementation provides a complete in-memory Supabase SDK that's API-compatible with `@supabase/supabase-js`. This allows testing and development without requiring a real Supabase project.
...
+-------------------------------------------------------------+
|  Database:   In-memory Map<table, Row[]>                    |
|  Auth:       In-memory user/session storage                 |
|  Storage:    In-memory Blob storage                         |
|  Realtime:   In-memory event dispatch                       |
|  Functions:  Mock invocation handler                        |
+-------------------------------------------------------------+
```

Each `createClient()` call creates an isolated instance with its own storage, making it ideal for testing where you need clean state between tests.
