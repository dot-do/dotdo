---
title: NATS
description: Drop-in replacement for nats.js with edge-native pub/sub, request/reply, and JetStream.
---
...
## Outbound - Connecting to Real NATS
...
|    (In-Memory DO)       |     |   (Real NATS)           |
+-------------------------+     +-------------------------+
|                         |     |                         |
|  -> In-memory storage   |     |  -> nats://server:4222  |
|  -> No network calls    |     |  -> Full NATS cluster   |
|  -> Sub-10ms latency    |     |  -> Production-ready    |
|  -> JetStream emulated  |     |  -> External infra      |
|  -> KV Store built-in   |     |                         |
|                         |     |                         |
+-------------------------+     +-------------------------+
```
...
- **No infrastructure**: No NATS server to manage
- **Predictable**: No external dependencies
- **Testable**: Perfect for unit tests
- **Edge-native**: Runs in V8 isolates
...
```typescript
import { connect, AckPolicy } from '@dotdo/nats'
...
- [JetStream Guide](https://docs.nats.io/nats-concepts/jetstream) - Official NATS JetStream docs
