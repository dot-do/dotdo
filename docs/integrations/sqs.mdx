...
**This is a proxy/compatibility layer.** It can either forward to real AWS SQS (production) or run entirely locally using in-memory storage backed by Durable Objects. The API surface matches the official AWS SDK v3.
...
## Inbound Mode - Using sqs.do Locally
...
## Outbound Mode - Connecting to Real AWS SQS
...
|  -> Actual queues       |     |  -> DO SQLite backing   |
|  -> Production traffic  |     |  -> Instant operations  |
|                         |     |                         |
+-------------------------+     +-------------------------+
```
...
- [Workflows](/docs/workflows) - Durable workflow execution
