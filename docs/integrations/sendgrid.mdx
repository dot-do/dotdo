...
Drop-in replacement for `@sendgrid/mail`. Your existing `@sendgrid/mail` code works unchanged - just swap the import.
...
```typescript
// Helper: Convert ArrayBuffer to base64 (edge-compatible, no Buffer)
...
// Get PDF content - from storage or generated
const pdfData = await env.R2.get('invoices/order-123.pdf')
const pdfBase64 = arrayBufferToBase64(await pdfData.arrayBuffer())

// Get image for inline attachment
const logoData = await env.R2.get('assets/logo.png')
const logoBase64 = arrayBufferToBase64(await logoData.arrayBuffer())
...
// Helper to add delay between batches for rate limiting
...
async function sendNewsletter(
  ...
  // Batch into chunks of 1000 (SendGrid limit per request)
  ...
  for (let i = 0; i < chunks.length; i++) {
    ...
    try {
      ...
      // Rate limiting: wait 100ms between batches to avoid hitting SendGrid limits
      // SendGrid has a default rate limit of 600 requests/minute
      if (i < chunks.length - 1) {
        await delay(100)
      }
    } catch (error) {
      result.failed += chunk.length
      result.errors.push({ chunk: i, error: error as Error })

      // Optional: continue with remaining batches even if one fails
      // Remove this continue statement to stop on first error
      continue
    }
  }
