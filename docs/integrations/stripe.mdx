...
## Checkout Sessions (Coming Soon)

Checkout Sessions are not yet implemented in @dotdo/stripe. For now, use the official Stripe SDK for checkout flows, or use PaymentIntents directly:

```typescript
// Alternative: Use PaymentIntents for custom checkout
const paymentIntent = await stripe.paymentIntents.create({
  amount: 2000,
  currency: 'usd',
  customer: 'cus_xxx',
...
Connect APIs (Accounts, Transfers, Payouts) are not yet implemented. For marketplace/platform use cases, use the official Stripe SDK alongside @dotdo/stripe:

```typescript
import { Stripe } from '@dotdo/stripe'
import StripeOfficial from 'stripe'

// Use @dotdo/stripe for core payment operations
const dotdoStripe = new Stripe(env.STRIPE_SECRET_KEY)

// Use official SDK for Connect-specific operations
const officialStripe = new StripeOfficial(env.STRIPE_SECRET_KEY)
const account = await officialStripe.accounts.create({
  type: 'express',
  country: 'US',
...
The `stripeAccount` option IS supported for on-behalf-of requests:

```typescript
// This works - make requests on behalf of a connected account
const paymentIntent = await stripe.paymentIntents.create(
  {
    amount: 2000,
    currency: 'usd',
