...
```typescript
import { Auth0APIError, Auth0ValidationError, Auth0AuthenticationError } from '@dotdo/auth0'
...
|------------|-------------|
| `Auth0APIError` | Base error for all API errors |
| `Auth0ValidationError` | Invalid request parameters |
| `Auth0AuthenticationError` | Authentication/authorization failures |
| `Auth0RateLimitError` | Rate limit exceeded |
  ...
  if (error instanceof Auth0AuthenticationError) {
    switch (error.errorCode) {
      case 'invalid_grant':
        ...
        break
      case 'mfa_required':
        console.error('MFA token:', error.mfaToken)
...
```typescript
import { Auth0RateLimitError } from '@dotdo/auth0'

try {
  // Bulk operations
  for (const user of users) {
    await management.users.create(user)
  }
} catch (error) {
  if (error instanceof Auth0RateLimitError) {
