...
> Note: The Metadata API uses a simplified REST-like approach. Full SOAP envelope support for metadata deployment is not yet implemented.
...
> Note: Streaming types (`StreamingChannelType`, `StreamingSubscription`, `StreamingMessage`) are exported for future compatibility but the Streaming API is not yet implemented.
