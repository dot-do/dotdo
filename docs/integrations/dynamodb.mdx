---
title: DynamoDB
description: In-memory DynamoDB-compatible SDK for testing and edge development with AWS SDK v3 API compatibility.
---
...
## Using with AWS SDK Code

The `@dotdo/dynamodb` package matches the AWS SDK v3 API, making it easy to use your existing DynamoDB code in tests.

### Test Setup

```typescript
// In your tests, swap the import
...
+-------------------------------------------------------------+
|                                                              |
|  -> Global Map<string, TableMeta> for tables                 |
|  -> Map<string, Item> per table for items                    |
|  -> Full expression parsing (Key, Filter, Update, etc.)      |
|  -> clearAllTables() to reset state between tests            |
|                                                              |
+-------------------------------------------------------------+
```
