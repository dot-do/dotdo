---
title: DynamoDB
description: Drop-in replacement for the AWS DynamoDB SDK with edge compatibility and local testing support.
---

# DynamoDB

Drop-in replacement for the AWS DynamoDB JavaScript SDK (v3). Your existing `@aws-sdk/client-dynamodb` code works unchanged - just swap the import.
...
| AWS credentials required | No credentials for local development |
| Pay per request/provisioned | Free in local mode |
| Regional deployment | Runs on edge (300+ cities) |

**This is a proxy/compatibility layer.** It can either forward to real AWS DynamoDB (production) or run entirely locally backed by Durable Objects with SQLite storage for testing and development. The API surface matches AWS SDK v3.
...
## Inbound Mode - Using dynamodb.do Locally

For testing and development, the client runs entirely in-memory backed by Durable Objects with SQLite storage:
...
### Conditional Writes in Local Mode
...
## Outbound Mode - Connecting to Real AWS DynamoDB

For production, configure the client with AWS credentials to connect to real DynamoDB:

```typescript
import { DynamoDBClient, PutItemCommand } from '@dotdo/dynamodb'
...
// All requests go to dynamodb.us-east-1.amazonaws.com
await client.send(new PutItemCommand({
  TableName: 'ProductionUsers',
  Item: {
    pk: { S: 'USER#123' },
    sk: { S: 'PROFILE' },
    name: { S: 'Alice' },
...
```typescript
// worker.ts
import { DynamoDBClient, QueryCommand, marshallItem, unmarshallItem } from '@dotdo/dynamodb'
...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    const items = result.Items?.map(unmarshallItem) ?? []
...
### Extended Configuration (DO Backing)

```typescript
const client = new DynamoDBClient({
  region: 'us-east-1',
  // DO namespace for distributed storage
  doNamespace: env.DYNAMODB_DO,
...
```bash
# Remove
npm uninstall @aws-sdk/client-dynamodb
...
Your existing DynamoDB code should work unchanged:

```typescript
// This code works with both @aws-sdk/client-dynamodb and @dotdo/dynamodb
const client = new DynamoDBClient({ region: 'us-east-1' })
...
|   (AWS DynamoDB)        |     |     (DO + SQLite)       |
+-------------------------+     +-------------------------+
|                         |     |                         |
|  -> dynamodb.aws.com    |     |  -> In-memory storage   |
|  -> Real tables         |     |  -> No network calls    |
|  -> AWS credentials     |     |  -> No credentials      |
|  -> Pay per use         |     |  -> Free local testing  |
|                         |     |                         |
+-------------------------+     +-------------------------+
```
