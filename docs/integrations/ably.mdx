...
Drop-in replacement for `ably`. Your existing `ably` code works unchanged - just swap the import.
...
| Feature | @dotdo/ably | Ably |
|---------|-------------|------|
| **Connections/channel** | Unlimited | 200 (Free), Custom (Enterprise) |
| **Per-connection fees** | No | Yes ($0.0015/peak connection/day) |
| **Per-message fees** | No | Yes ($2.50/million messages) |
| **Channel limit** | Unlimited | 200 per app (Free) |
| **Message size** | 128KB | 64KB |
| **Latency** | Edge (300+ cities) | Multi-region |
| **Cold start** | 0ms (V8 isolates) | N/A (hosted) |
| **Self-hosted** | Yes | No |
