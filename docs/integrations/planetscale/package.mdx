---
title: "@dotdo/planetscale"
description: Drop-in replacement for @planetscale/database serverless driver with DO-backed storage and real PlanetScale proxy support.
---

# @dotdo/planetscale

Drop-in replacement for `@planetscale/database` serverless driver. Your existing PlanetScale code works unchanged - just swap the import. Backed by Durable Objects for local development or proxies to real PlanetScale for production.
...
<Callout type="info">
Looking for a managed edge MySQL service? See [planetscale.do](/docs/integrations/planetscale/service) for a fully managed MySQL-compatible edge database.
</Callout>
...
- **Edge-compatible** - Runs natively on Cloudflare Workers, no external connections needed
...
## Testing with @dotdo/planetscale

Use DO-backed storage for isolated, fast tests:
...
- [planetscale.do](/docs/integrations/planetscale/service) - Managed MySQL on the edge
