...
const recording = await client.calls('CA123').recordings.create({
  recordingChannels: 'dual',
  recordingStatusCallback: 'https://example.com/recording-status',
})

// List recordings
const recordings = await client.recordings.list({ callSid: 'CA123' })

// Get recording
const recording = await client.recordings('RE123').fetch()

// Delete recording
await client.recordings('RE123').delete()

// Pause/resume recording
await client.calls('CA123').recordings.update('RE123', { status: 'paused' })
await client.calls('CA123').recordings.update('RE123', { status: 'in-progress' })
```
...
// Get conference
const conference = await client.conferences('CF123').fetch()

// Add participant
const participant = await client.conferences('CF123').participants.create({
  from: '+15017122661',
  to: '+15558675310',
  startConferenceOnEnter: true,
})
...
// Update participant
await client.conferences('CF123').participants('CA123').update({
  muted: true,
  hold: false,
})

// Remove participant
await client.conferences('CF123').participants('CA123').delete()

// End conference
await client.conferences('CF123').update({ status: 'completed' })
```
