...
## Inbound - Using linear.do Locally
...
## Outbound - Connecting to Real Linear

For production, the client connects to Linear's API:
...
// Get triaged issues
async function getTriagedIssues(teamId: string) {
  return linear.issues({
    filter: {
      team: { id: { eq: teamId } },
      state: { type: { eq: 'unstarted' } },
      priority: { neq: 0 }, // Has priority set
    },
    orderBy: 'priority',
  })
}
...
async function autoAssign(issueId: string, teamId: string) {
  const users = await linear.users()
  const assignments = await Promise.all(
    users.nodes.map(async (user) => {
      const assigned = await linear.issues({
        filter: {
          assignee: { id: { eq: user.id } },
          state: { type: { eq: 'started' } },
        },
      })
      return { user, count: assigned.nodes.length }
    })
  )

  const leastBusy = assignments.sort((a, b) => a.count - b.count)[0]

  await linear.issueUpdate(issueId, {
    assigneeId: leastBusy.user.id,
  })
}
```
