...
interface DuckDBInstance {
  ...
  registerBuffer(name: string, buffer: ArrayBuffer | Uint8Array): Promise<BufferRegistrationResult>
...
createFTSIndex(db: DuckDBInstance, config: {
  ...
  stemmer?: StemmerLanguage  // 'english', 'porter', etc.
  overwrite?: boolean
...
```typescript
import { createDuckDB, isCached, clearCache } from '@dotdo/duckdb-wasm'
