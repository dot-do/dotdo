...
Drop-in replacement for `resend`. Your existing `resend` code works unchanged - just swap the import.
...
| Feature | @dotdo/emails | Resend |
|---------|---------------|--------|
| **API compatibility** | 100% compatible | Native |
| **Runtime** | Edge (300+ cities) | Node.js |
| **Cold start** | 0ms (V8 isolates) | Variable |
| **Provider fallback** | Yes (MailChannels, SendGrid) | No |
| **Local development** | Full DO capture | Sandbox mode |
| **React Email** | Supported | Native |
