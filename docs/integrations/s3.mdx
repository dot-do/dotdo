...
## Inbound - s3.do as Drop-in Replacement

Use s3.do to replace AWS S3 in existing applications:

```typescript
// Existing AWS S3 code - works unchanged
import { S3Client, PutObjectCommand, GetObjectCommand } from '@dotdo/s3'
...
## Outbound - Connecting to Real AWS S3 or R2
...
For maximum performance, use R2 bindings directly with the `R2Store` utility:

```typescript
import { createR2Store } from 'dotdo/cloudflare/r2'
...
Cloudflare R2 is S3-compatible with zero egress fees. The dotdo S3 integration is built on R2.
...
import { createR2Store, type R2Store } from 'dotdo/cloudflare/r2'
...
The R2 integration uses standardized paths for multi-tenant storage:
...
```typescript
import { buildPath, parsePath } from 'dotdo/cloudflare/r2'
...
// R2 Store types (from dotdo/cloudflare/r2)
import type {
  R2Store,
  R2StoreConfig,
  R2Object,
...
async function processImage(bucket: string, key: string) {
  ...
  const imageData = await original.Body?.transformToByteArray()
...
async function restoreLatestBackup() {
  ...
  const backup = await client.send(new GetObjectCommand({
    Bucket: 'backups',
    Key: latest.Key,
  }))

  return backup.Body?.transformToString()
}
```
