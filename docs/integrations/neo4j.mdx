...
**This is a compatibility layer.** It runs entirely locally using Durable Objects storage for development/testing. The API surface matches Neo4j's official JavaScript driver.
...
**Advanced Storage** (via primitives integration)
- Time-travel queries with versioned storage
- Snapshot and restore
- Partition routing for distributed graphs
...
For advanced use cases, the `GraphStorage` class provides versioned storage with time-travel queries and partitioning.

```typescript
import { createGraphStorage } from '@dotdo/neo4j'
...
await storage.createRelationship('KNOWS', alice.id, bob.id, { since: 2020 })

// Time-travel query: get state at a specific time
const pastState = await storage.getNode(alice.id, { asOf: Date.now() - 86400000 })
...
// Partition routing for distributed processing
const partition = storage.getNodePartition(alice.id)
const nodesByPartition = storage.routeNodes([alice.id, bob.id])
```
...
For advanced use cases, you can use the Cypher parser directly to parse queries into an AST.

```typescript
import { CypherParser, parseCypher, parseCypherWhere } from '@dotdo/neo4j'

// Parse full Cypher query
const queries = parseCypher(
  'MATCH (n:Person {name: $name})-[:KNOWS]->(friend) WHERE friend.age > 21 RETURN friend',
  { name: 'Alice' }
)

// queries[0] contains:
// - type: 'MATCH'
// - patterns: [{ variable: 'n', labels: ['Person'], properties: {...}, relationships: [...] }]
// - where: { type: 'predicate', column: 'friend.age', op: '>', value: 21 }
// - return: { items: [{ expression: 'friend', alias: undefined }], distinct: false }

// Parse WHERE clause to AST
const whereAST = parseCypherWhere('n.age > 21 AND n.status = "active"', {})
// Returns unified QueryNode AST for integration with query engine
```
...
Use @dotdo/neo4j for all environments - no external Neo4j server needed:
...
- [MongoDB Integration](/docs/integrations/mongodb) - Document database support
