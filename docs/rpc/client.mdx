...
const startup = $Context('https://my-startup.example.com')
...
<Callout type="info">
The namespace URL is the root of your DO deployment. All RPC calls go to `{namespace}/rpc`.
</Callout>
...
export default {
  namespace: 'https://my-startup.example.com',
}
```

## Promise Pipelining Over HTTP

The client builds a chain of operations and sends them all in one HTTP request:

```typescript
// Build the chain - no await yet
...
The server receives the entire chain:
...
```json
{
  "chain": [
    ...
    { "type": "call", "key": "recent", "args": [5] },
    { "type": "call", "key": "map", "args": [...] }
  ]
}
```

One round trip. No matter how deep the chain.
...
export default {
  async fetch(request) {
    const $ = $Context('https://other-service.example.com')
...
| `BLOCKED_ACCESS` | Method is private or blocked |
| `INVALID_CHAIN` | Malformed RPC chain |
| `EXECUTION_ERROR` | Server-side execution failed |
| `HTTP_ERROR` | Server returned 5xx status |
...
const $: RpcClient = $Context('https://example.com')
