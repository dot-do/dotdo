...
<Callout type="info" title="Part of RPC">
This page is part of the [Cap'n Web RPC](/docs/rpc) documentation.
</Callout>
...
When using agents through the workflow proxy, mapping with agent calls triggers batch execution:

```typescript
// Agents accessed via workflow proxy for Magic Map support
const tasks = $.Agent('priya')`break down ${spec} into tasks`
const implementations = tasks.map(task => $.Agent('ralph')`implement ${task}`)
await $.Agent('quinn')`test all: ${implementations}`
```

Here, the `.map()` creates N agent calls - one per task. The server:
...
const implementations = tasks.map(
  task => $.Agent('ralph')`implement ${task}`,
...
For the definitive guide on when to use Magic Map vs await-then-loop, see [When to Await](/docs/rpc/when-to-await).
