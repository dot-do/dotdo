...
<Cards>
  ...
  <Card title="When to Await" href="/docs/rpc/when-to-await">
    **The quick reference.** Decision tree, rules, and consolidated patterns for every await decision.
  </Card>
</Cards>
...
const results = await Promise.all(
  tasks.map(async task => await $.Agent('ralph')`implement ${task}`)  // N round trips!
)
```

```typescript title="Fix: Use Magic Map with Workflow Proxy"
const tasks = $.Agent('priya')`list tasks`
const results = await tasks.map(task => $.Agent('ralph')`implement ${task}`)  // 1 round trip
```

Note: Magic Map requires using agents through the workflow proxy (`$`). Direct imports like `import { ralph } from 'agents.do'` use simple promise chaining instead.
