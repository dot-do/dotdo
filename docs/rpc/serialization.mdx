...
Cap'n Web RPC detects and handles circular references:
...
// Nested reference (future support)
{ "$ref": "#/child/nested" }
```
...
Binary format is more compact for large payloads:
...
// Binary is typically 10-30% smaller
```
...
| Capability | `"Capability"` | `{ $type: "Capability", id, type, methods, ... }` |
