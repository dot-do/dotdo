...
description: When to await and when to pipeline - the rules are simple once you know them
---
...
Often you can replace loops with Magic Map:
...
Magic Map keeps everything in a single pipeline.
</Callout>
...
const results = await Promise.all(
  tasks.map(async task => {
    const impl = await ralph`implement ${task}`  // N awaits!
    return impl
  })
)
```

**Fix:** Use Magic Map or avoid async callbacks:
