...
Configure individual channels with retry policies and test hooks:
...
notifications.configure({
  email: {
    ...
    // For testing: simulate delivery failures
    simulateFailure: false,
    // For testing: hook called before each send attempt
    beforeSend: () => { /* custom logic */ }
  },
  slack: {
    retryPolicy: {
      maxRetries: 2,
...
interface ChannelConfig {
  simulateFailure?: boolean    // For testing: always fail delivery
  beforeSend?: () => void      // For testing: hook before each attempt
  retryPolicy?: {
...
<Callout type="info">
**Exponential backoff** (default) doubles the delay each retry. **Linear backoff** adds the initial delay each time.
</Callout>
...
The dispatcher provides specific error messages:
...
<Callout type="tip">
1. **Respect preferences**: Let users choose their notification channels
...
- [Approval Workflows](./approvals) - Send approval notifications
- [Escalation Policies](./escalation) - Alert on escalations
- [Review Queues](./queues) - Notify reviewers of new items
