---
title: "@dotdo/inngest"
description: Drop-in replacement for Inngest SDK with edge compatibility and in-memory testing support.
---
...
<Callout type="info">
Looking for managed edge deployment? See [inngest.do](/docs/integrations/inngest/service) for a fully managed durable function service on Cloudflare Workers.
</Callout>

## Why @dotdo/inngest?

| Inngest SDK | @dotdo/inngest |
|-------------|----------------|
| Requires Inngest service | Runs on Durable Objects or in-memory |
| Events leave infrastructure | Events stay in your Workers |
| ~50ms cold start | 0ms cold start (V8 isolates) |
| 5 regions | 300+ cities |
| 2 hour max duration | Unlimited with DO storage |
| Per-step billing | Included |
| Self-hosted (Enterprise) | Always self-hosted |

**This is a compatibility layer.** It runs entirely in-memory for testing, or uses Durable Object SQLite for production. The API surface matches the Inngest SDK exactly.
...
**Function Creation**
- `createFunction()` - Event and cron triggers
- Event triggers with pattern matching
- Cron triggers with standard expressions
- String shorthand for simple events

**Step Tools**
- `step.run()` - Durable step execution with memoization
- `step.sleep()` - Durable sleep (function pauses, not blocked)
- `step.sleepUntil()` - Sleep until specific timestamp
- `step.waitForEvent()` - Wait for correlated events
- `step.sendEvent()` - Send events from within functions
- `step.invoke()` - Invoke other functions
- `step.parallel()` - Run steps in parallel

**Function Configuration**
- Retry policies (attempts, backoff, intervals)
- Timeouts (function and step level)
- Rate limiting (key-based)
- Throttling (key-based)
- Debouncing (key-based)
- Concurrency limits (global and key-based)
- Batch events (maxSize, timeout, key)
- Cancel on events (event matching)

**Error Handling**
- `NonRetriableError` - Errors that should not retry
- `RetryAfterError` - Errors with retry delay
- `StepError` - Step execution errors
- `CancellationError` - Function cancellation

**Middleware**
- `onSendEvent` - Transform outgoing events
- `onFunctionRun` - Lifecycle hooks for function execution
- `beforeExecution`, `afterExecution` - Pre/post hooks
- `transformInput`, `transformOutput` - Data transformation

**HTTP Handler**
- `serve()` - Framework-agnostic HTTP handler

### Not Yet Implemented

- `step.ai.infer()` - AI inference step
- Encrypted channels
- Replay/recovery UI
...
## Durable Object Mode (Production)
...
```typescript
import { CancellationError } from '@dotdo/inngest'
...
| `step.sleepUntil(id, timestamp)` | Sleep until timestamp |
...
- [inngest.do](/docs/integrations/inngest/service) - Managed durable functions on the edge
