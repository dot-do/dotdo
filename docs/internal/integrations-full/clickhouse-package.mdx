---
title: "@dotdo/clickhouse"
description: ClickHouse-compatible analytics engine for edge environments and testing.
---

# @dotdo/clickhouse

ClickHouse-compatible analytics engine that runs entirely in-memory. Provides SQL interface, MergeTree tables, and OLAP aggregations for edge environments (Cloudflare Workers) and testing scenarios where no external database is needed.
...
await client.query(`INSERT INTO hits VALUES ('2024-01-01', 1, 100)`)
...
<Callout type="info">
Looking for persistent storage? See [clickhouse.do](/docs/integrations/clickhouse/service) for a fully managed ClickHouse-compatible edge database.
</Callout>

## Why @dotdo/clickhouse?

| ClickHouse Server | @dotdo/clickhouse |
|-------------------|-------------------|
...
| Network latency to cluster | Zero latency (in-memory) |
| Separate analytics stack | Unified with your application |

**This is an in-memory analytics implementation** with a ClickHouse-compatible SQL interface. Data does not persist between restarts. Ideal for testing, request-scoped analytics, and edge environments where you need OLAP capabilities without external dependencies.
...
**MergeTree Table Engines (Schema Only)**
- `MergeTree` - Basic sorted table with ordered storage
- `ReplacingMergeTree` - Schema support (versionColumn stored but deduplication not active)
- `AggregatingMergeTree` - Schema support for pre-aggregated data types
- `SummingMergeTree` - Schema support (summation behavior not active on merge)
- `CollapsingMergeTree` - Schema support (signColumn stored but collapsing not active)
- `VersionedCollapsingMergeTree` - Schema support

Note: Engine types are stored for compatibility but merge/compaction behaviors are not implemented. All engines currently behave like basic MergeTree (sorted storage).
...
- `WITH` clause (Common Table Expressions - single CTE)
- `DROP TABLE` and `TRUNCATE`

**Aggregate Functions**
- Basic: `count`, `sum`, `avg`, `min`, `max`
- Distinct: `uniq`, `uniqExact` (exact via Set), `uniqHLL12` (HyperLogLog approximation)
...
- Approximate: `anyHeavy` (Space-Saving algorithm)

**Window Functions (via Table API)**
- Ranking: `rowNumber`, `rank`, `denseRank` (via `selectWithWindow`)
- Offset: `lag`, `lead` with custom offset and defaults
- Running aggregates: `sum`, `avg` with frame specification
- Frame specification: ROWS, UNBOUNDED PRECEDING/FOLLOWING, CURRENT ROW

Note: Window functions are available via the `MergeTreeTable.selectWithWindow()` method, not via SQL queries.

**OLAP Operations (via Table API)**
- `cube()` - All dimension combinations
- `rollup()` - Hierarchical subtotals
- `groupingSets()` - Custom groupings
- `groupBy()` with `withTotals` option

Note: CUBE/ROLLUP/GROUPING SETS are available via table methods, not SQL.

**Materialized Views**
- Pre-computed aggregations with caching
- On-demand or interval-based refresh
- Staleness detection via `isStale(maxAge)`
...
- Window functions via SQL (use Table API instead)
- CUBE/ROLLUP via SQL (use Table API instead)
- Actual merge/compaction behavior for specialized engines
...
## Testing with ClickHouseClient

ClickHouseClient is ideal for unit tests - no external ClickHouse required:
...
describe('AnalyticsRepository', () => {
  ...
  beforeEach(() => {
    ...
    client = new ClickHouseClient({ mode: 'memory' })
  })
  ...
  it('should aggregate events', async () => {
    ...
    expect(result.rows).toContainEqual({ event_type: 'click', total: 30 })
    expect(result.rows).toContainEqual({ event_type: 'view', total: 100 })
  })
})
```
...
- [clickhouse.do](/docs/integrations/clickhouse/service) - Managed ClickHouse on the edge
