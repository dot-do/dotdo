---
title: "@dotdo/redis"
description: Drop-in replacement for ioredis/node-redis with edge compatibility and optional real Redis backend.
---
...
<Callout type="info">
Looking for persistent edge storage? See [redis.do](/docs/integrations/redis/service) for a fully managed Redis-compatible edge database.
</Callout>
...
## Testing with @dotdo/redis

@dotdo/redis is ideal for unit tests - no external Redis required:
...
describe('RateLimiter', () => {
  ...
  it('should allow requests under limit', async () => {
    for (let i = 0; i < 10; i++) {
      await redis.incr('rate:user123')
    }
    const count = await redis.get('rate:user123')
...
- [redis.do](/docs/integrations/redis/service) - Managed Redis on the edge
