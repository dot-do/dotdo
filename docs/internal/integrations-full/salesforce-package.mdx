...
<Callout type="info">
Looking for managed Salesforce access without infrastructure? See [salesforce.do](/docs/integrations/salesforce/service) for a fully managed edge service.
</Callout>
...
- CSV data handling
- Job status monitoring with event emitter (`queue`, `response`, `error` events)
...
**Metadata API (Partial)**
- `metadata.read()` - Read metadata components
- `metadata.list()` - List metadata by type

<Callout type="warn">
The Metadata API uses a simplified REST-like approach. Full SOAP envelope support for metadata deployment is not yet implemented.
</Callout>
...
- Streaming API (PushTopic, Platform Events, Change Data Capture) - types defined, no runtime implementation
- Metadata deployment/retrieve (create, update, delete, deploy, retrieve operations)
- Apex REST/SOAP callouts
- Tooling API
- Chatter API
- Composite Graph API
- SalesforceLocal (local testing mode) - planned for future release
...
<Callout type="info">
Streaming types (`StreamingChannelType`, `StreamingSubscription`, `StreamingMessage`) are exported for future compatibility but the Streaming API is not yet implemented.
</Callout>
...
| Query Builder | Full | Fluent interface with operators |
| Bulk API 2.0 | Full | Insert, update, upsert, delete, query, queryAll |
| OAuth 2.0 | Full | Username/password, web server, JWT, client credentials |
| Describe | Full | Global and per-object with caching |
| Analytics | Full | Reports (sync/async) and dashboards |
| Metadata API | Partial | Read and list only (no deploy/retrieve) |
| Identity | Full | User info via userinfo endpoint |
| Limits | Full | Org API limits |
| Streaming API | Types only | PushTopic, Platform Events, CDC (not implemented) |
| Tooling API | Not yet | |
| Chatter API | Not yet | |
| Composite Graph | Not yet | |

## Related

- [salesforce.do](/docs/integrations/salesforce/service) - Managed Salesforce on the edge
