---
title: mongo.do
description: MongoDB on the Edge — A fully managed MongoDB-compatible database running on Cloudflare Workers with vector search and AI agent support.
---
...
<Callout type="info">
Looking for an in-memory implementation for testing? See [@dotdo/mongodb](/docs/integrations/mongo/package) for a zero-dependency document store.
</Callout>

## Why mongo.do?

Traditional databases require infrastructure management, connection pooling, and careful scaling. mongo.do eliminates all of that by running directly on Cloudflare's edge network:
...
mongo.do translates MongoDB queries to SQLite at runtime:
...
mongo.do includes an integrated analytics layer for OLAP workloads:
...
- [@dotdo/mongodb](/docs/integrations/mongo/package) - In-memory MongoDB for testing
