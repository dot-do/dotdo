...
- **TypeScript** familiarity (basic understanding)
- **Stripe account** for payment processing (test keys work for development)
- **org.ai account** for authentication (optional, but recommended)
...
export class Auth extends DO {
  async init() {
    ...
    $.on.User.login(async (event) => {
      // Fire-and-forget: track for analytics
      $.send('analytics', {
        event: 'login',
...
curl -X POST http://localhost:8787/auth/signup \
  ...
  -d '{"email":"test@example.com.ai","password":"test123","name":"Test User"}'
