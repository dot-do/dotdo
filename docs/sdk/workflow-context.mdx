...
|----------|-----|-------------|
| **Durability** | `$.track()` | Fire-and-forget telemetry |
| | `$.send()` | Durable event emission |
| | `$.try()` | Single attempt, fallible |
| | `$.do()` | Durable with retries |
| **Events** | `$.on.Noun.verb()` | Subscribe to events |
| | `send.Noun.verb()` | Emit events (DSL) |
| **Scheduling** | `$.every.*` | Fluent cron schedules |
| **Cross-DO** | `$.Noun(id)` | RPC to other DOs |
| **AI** | `$.ai`, `$.write`, etc. | Template literal AI |
| **Control Flow** | `when()`, `waitFor()` | Conditionals and human-in-the-loop |
| **State** | `$.state`, `$.log()` | State management |
