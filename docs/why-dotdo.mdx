...
| **Compat APIs** | 40+ SDKs | Integrations | Tools | Plugins |
...
dotdo uses **promise pipelining** with the workflow proxy. You express your entire workflow, and it executes as a single batched operation:

```typescript
// Using the workflow proxy for true promise pipelining
const user = $.User(userId).get()           // Not awaited
const orders = $.Order(user).list()         // Uses unawaited user
const total = await $.Analytics(orders).summarize()  // Await triggers execution

// All three operations execute in ONE network round trip
```

This is Cap'n Proto's promise pipelining adapted for the web. When using the workflow proxy (`$`), the **Magic Map** captures your callbacks, sends them to the server, and executes them where the data lives.

Result: **N operations in 1 round trip** instead of N round trips. See [Promise Pipelining](/docs/rpc) for details.
