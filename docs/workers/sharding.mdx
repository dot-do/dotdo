...
// Full key with all components
const key1 = hashKey('tenant-a', 'customers', '123')
// Result: 'tenant-a:customers:123'

// Use '_' for missing components (convention)
const key2 = hashKey('tenant-a', '_', '_')
// Result: 'tenant-a:_:_'

// The key format is always 'ns:type:id'
// This ensures consistent hashing for the same resource
```

Use the key with the hash ring:
...
> **Note:** Initialize the hash ring at module level, not per-request. Workers can handle thousands of requests per second, and creating a new ring on each request would be wasteful.
...
```typescript
// Get all physical nodes (returns a copy, safe to modify)
const nodes = ring.getNodes()
// ['shard-0', 'shard-1', ..., 'shard-15']

// Get configured virtual node count
const vnodes = ring.getVirtualNodeCount()
// 150

// Note: Duplicate addNode calls are safely ignored
ring.addNode('shard-0') // No-op if already exists
```
...
The algorithm:
1. Start with the FNV offset basis (2166136261)
2. XOR each character's code point
3. Multiply by the FNV prime (16777619)
4. Use `>>> 0` to ensure unsigned 32-bit integers
