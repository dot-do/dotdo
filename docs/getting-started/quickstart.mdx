...
`ralph`build ${spec}`` isn't string interpolation—it's an RPC call to the Ralph agent. The backticks invoke the agent, the template contents become the task, and you get back a promise-like proxy.
</Callout>
...
`$` is your workflow context, available as `this.$` inside DO methods. It provides event handlers (`$.on.Customer.signup`), scheduling (`$.every.Monday.at9am`), and durability levels (`$.do()`, `$.try()`, `$.send()`). In code examples, we typically use `const $ = this.$` for cleaner syntax.
</Callout>
