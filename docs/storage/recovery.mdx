...
const rows = sql.exec('SELECT id, type, data FROM things').toArray()
const state = new Map()

for (const row of rows) {
  const thing = JSON.parse(row.data)
  state.set(row.id, thing)
}
```
  ...
  switch (type) {
    ...
    case 'thing.updated':
      ...
      if (existing) {
        ...
        // Create from update (event sourcing)
        state.set(entityId, {
          $id: entityId,
          $type: payload.$type ?? 'Unknown',
          $version: 1,
...
interface RecoveryResult {
  source: 'sqlite' | 'iceberg' | 'empty'  // Recovery source
  thingsLoaded: number                     // Base state count
  eventsReplayed: number                   // Pipeline events applied
  state: Map<string, ThingData>            // Final state
...
interface RecoveryOptions {
  ...
  iceberg?: IcebergReader      // Optional: Iceberg for fallback
  namespace: string            // Required: Tenant namespace
  pipelineEvents?: PipelineEvent[]  // Optional: Events to replay
}
```
