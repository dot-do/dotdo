...
|  string[] accessOrder     |  <- LRU tracking
|  number totalBytes        |  <- Memory tracking
+---------------------------+
```
...
- **Memory estimation** - Tracks approximate byte usage for threshold-based eviction
...
// $id and $type are always preserved, even if included in updates
const withOverrides = manager.update(customer.$id, {
  $id: 'different_id',        // Ignored - $id preserved
  $type: 'DifferentType',     // Ignored - $type preserved
  name: 'Bob'
})
console.log(withOverrides.$id)   // Original ID preserved
console.log(withOverrides.$type) // Original type preserved
```

<Callout type="warning">
  Updating a non-existent ID throws an error: `Storage error: Thing with id '...' not found for update operation`. Always check existence first or use try/catch.
</Callout>
...
interface ThingData {
  ...
  data?: Record<string, unknown> // Optional nested data object
...
interface InMemoryStateManagerOptions {
  ...
  onEvict?: (entries: ThingData[]) => void  // Called with evicted entries
}
```
...
interface StateManagerStats {
  entryCount: number      // Total entries in cache
  dirtyCount: number      // Entries modified since last checkpoint
  estimatedBytes: number  // Approximate memory usage (2 bytes per JSON char)
}
```
...
interface CacheEntry {
  data: ThingData    // The actual thing data
  size: number       // Estimated byte size
  accessTime: number // Last access timestamp for LRU
}
```

## ID Generation

When `$id` is not provided, IDs are auto-generated using the `generateId()` function:
...
**Format:** `{type}_{timestamp}{random}`

- **type** - Lowercase `$type` value
- **timestamp** - Base36 encoded `Date.now()`
- **random** - 8 character random string (base36)
...
Memory usage is estimated at 2 bytes per JSON character. This is a rough approximation that accounts for string encoding overhead:
...
For more accurate memory tracking in production, consider implementing a custom size estimator based on your data patterns.
