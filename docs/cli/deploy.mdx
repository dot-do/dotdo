...
```
Authenticating...
Logged in as nathan@example.com
...
Uses settings from `wrangler.jsonc`.
...
Requires environment configuration in `wrangler.jsonc`:
...
```json title="wrangler.jsonc"
{
  ...
  // R2 Buckets
  "r2_buckets": [
    { "binding": "STORAGE", "bucket_name": "my-startup-storage" }
  ],

  // D1 Databases
  "d1_databases": [
    { "binding": "DB", "database_name": "my-startup-db", "database_id": "xyz789" }
  ],
...
{
  "migrations": [
    { "tag": "v4", "deleted_classes": ["DeprecatedClass"] }
  ]
}
```
