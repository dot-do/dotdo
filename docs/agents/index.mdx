...
Agents compose through chaining with `.map()` for sequential operations:

```typescript
// Chain agents sequentially
const result = await priya`plan the sprint`
  .map(spec => ralph`build ${spec}`)
...
const [spec, marketing] = await Promise.all([
  ...
  mark`draft marketing plan`
])
```

The `.map()` on agent results transforms a single value (like `.then()`). For server-side Magic Map with batch execution, see the [RPC documentation](/docs/rpc/magic-map).
...
AI does the work. Humans make the decisions. This is not an afterthought - it's a core pattern.

Autonomous doesn't mean unsupervised. The best AI systems know their limits. When confidence drops, stakes rise, or judgment is required, agents escalate to humans.

```typescript
import { priya, ralph, tom } from 'agents.do'
import { legal, ceo } from 'humans.do'

// AI builds, humans approve
const spec = await priya`define MVP for ${hypothesis}`
const app = await ralph`build ${spec}`

// Human gate: CEO approves before launch
const approved = await ceo`approve ${app} for production`

// High-stakes trigger escalation
...
<Callout type="info">
**AI + Human Collaboration is first-class.** Agents and humans share the same calling convention. Your workflow doesn't care who handles the task - just that it gets handled correctly. See the [Human-in-the-Loop guide](/docs/humans) for the full pattern.
</Callout>
