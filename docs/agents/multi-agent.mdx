...
Chain agents with `.map()` for sequential transformations:
...
// For batch operations over arrays, use Promise.all
const tasks = ['auth', 'dashboard', 'api']
const reviewed = await Promise.all(
  tasks.map(async task => {
    const code = await ralph`implement ${task}`
    return tom`review ${code}`
  })
)
```

The `.map()` on agent results transforms a single value (like `.then()`). For server-side Magic Map with batch execution, use the [workflow proxy](/docs/rpc/magic-map).
