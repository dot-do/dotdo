...
```typescript
import { FsError } from 'dotdo/errors'
...
| Feature | Node.js fs | fsx |
|---------|-----------|-----|
| Storage | Local disk | SQLite + R2 |
| Transactions | No | Yes |
| Tiered storage | No | Yes |
| Edge deployment | No | Yes |
| Watch (cross-DO) | No | Yes |
| Max file size | Disk limit | 5GB (R2) |
| Concurrent access | File locks | Per-DO isolation |
