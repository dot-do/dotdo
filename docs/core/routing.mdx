...
DOCore uses [Hono](https://hono.dev) for HTTP routing, providing a fast and lightweight router with excellent TypeScript support. Routes handle incoming `fetch()` requests to the Durable Object.
...
export class MyDO extends DOCore {
  protected createApp() {
    const app = super.createApp() // Get base app with core routes
...
// Request context - adds request ID and DO state
app.use('*', async (c, next) => {
...
DOCore includes a default error handler:
...
| `/ready` | GET | Readiness check (after onStart) |
