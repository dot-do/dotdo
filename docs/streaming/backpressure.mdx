...
Backpressure occurs when producers generate messages faster than consumers can process them. Common causes include:

- **Slow clients** that cannot keep up with the message rate
- **Network congestion** delaying message delivery
- **Processing bottlenecks** where client-side handling is slower than server-side production

Without backpressure handling, unbounded queues lead to memory exhaustion and service degradation.
...
```
PAUSE MODE                    DROP MODE                     BUFFER MODE
============                  ============                  ============

[Producer] ---> [Buffer]      [Producer] ---> [Buffer]      [Producer] ---> [Buffer]
                   |                             |                             |
              highWaterMark                 highWaterMark                 bufferLimit
                   |                             |                             |
                   v                             v                             v
              [PAUSED]                    [Drop Oldest]                  [REJECTED]
                   |                             |                             |
              lowWaterMark                  (always                      (warning at
                   |                         accepts)                    threshold)
                   v
              [RESUMED]

Result:                       Result:                       Result:
- No data loss               - Oldest data dropped          - No data loss*
- Producer must wait         - Producer never waits         - Producer rejected when full
- Memory bounded             - Memory bounded               - Memory bounded by limit
```
...
> **Note:** Pause mode requires producers to check `isPaused` or handle rejected pushes. If your producer cannot be paused (e.g., external event source), consider drop or buffer mode.
...
> **Warning:** Drop mode silently discards data. Ensure your application can tolerate gaps. Log dropped messages for observability.
...
When not specified, buffer mode uses these defaults:

- `bufferLimit`: `highWaterMark * 10` (10x multiplier)
- `warningThreshold`: `0.8` (80% utilization)

The warning callback fires once when crossing the threshold and resets when buffer drains below it.
...
> **Warning:** Buffer mode can consume significant memory. Always set a reasonable `bufferLimit` and monitor via `onWarning`. The default limit is `highWaterMark * 10`.
...
- [Rate Limiting](/streaming/rate-limiting) - Control request rates with token buckets
- [WebSocket Hub](/streaming/websocket-hub) - Combine with per-connection backpressure
