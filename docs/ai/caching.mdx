...
The AI module includes an LRU (Least Recently Used) cache with configurable TTL (Time To Live) for repeated queries. This reduces costs and improves response times for identical prompts.

<Callout type="info">
Cached responses consume zero budget. Enable caching to extend your budget significantly.
</Callout>
...
```typescript
interface AICache {
  ttl: number       // Current TTL in milliseconds
  clear: () => number  // Clear cache, returns count of removed entries
}
```
...
// Same model + mode + prompt = cache hit
await ai`Analyze: ${text}`           // First call - cache miss
await ai`Analyze: ${text}`           // Second call - cache hit!
```
...
When the cache reaches `maxSize`, the least recently used entry is evicted. The implementation uses a `Map` for O(1) operations:
...
await ai`Query D` // Cache: [B, C, D] - A evicted (oldest)

// Accessing moves to most recent (delete + re-add)
await ai`Query B` // Cache: [C, D, B] - B moved to end
await ai`Query E` // Cache: [D, B, E] - C evicted (now oldest)
```

The LRU implementation details:
- Uses JavaScript `Map` which maintains insertion order
- `get()` removes and re-adds entries to update recency
- `set()` checks capacity before adding new entries
- TTL is checked on `get()`, expired entries are removed

## Clearing Cache

Manually clear all cached entries. Returns the number of entries removed:
...
// Read current TTL (read-only)
console.log(`Cache TTL: ${ai.cache.ttl}ms`) // e.g., 300000
```
...
- [Budget Tracking](./budget) - Control costs alongside caching
- [Batch Processing](./batch) - Efficient multi-item processing
