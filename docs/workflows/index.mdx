...
import { Callout, Tabs, Tab } from 'fumadocs-ui/components/callout'
...
The `$` context is the heart of dotdo workflows. It provides a unified API for durable execution, event handling, scheduling, and cross-DO communication.
...
on.Customer.signup(async (event) => {
  ...
  await $.do('Email.sendWelcome', event.data)
})
```
...
const inventory = await $.try('Inventory.check', { sku: 'ABC123' })
...
async function processCheckout(cart: Cart) {
  try {
    ...
    const available = await $.try('Inventory.checkAll', cart.items)
    ...
    await $.do('Checkout.process', cart)
  } catch (error) {
...
const receipt = await $.do('Payment.charge', {
  customerId,
  amount: 9999,
  currency: 'usd',
})
...
on.Order.placed(async (event) => {
  ...
  await $.do('Payment.charge', order.payment)
  await $.do('Shipping.createLabel', order.address)
  await $.do('Notification.send', {
...
| Validate email format | `$.try` | Pure function, no side effects |
| Charge credit card | `$.do` | Critical, must retry on failure |
| Send welcome email | `$.do` | Important, should retry if mail server is down |
| Update search index | `$.send` | Eventually consistent is fine |
| Check inventory | `$.try` | Fast lookup, can show "check failed" message |
| Create invoice | `$.do` | Business-critical, must persist |
...
on.Customer.signup(async (event) => {
  ...
  await $.do('CRM.createContact', { id, email, name })
  await $.do('Email.sendWelcome', { email, name })
  await $.do('Analytics.identify', { userId: id })
})

on.Payment.failed(async (event) => {
  await $.do('Notification.sendPaymentFailed', event.data)
  await $.do('Dunning.startSequence', event.data.customerId)
})

on.Subscription.cancelled(async (event) => {
  await $.do('Billing.stopRecurring', event.data)
  await $.do('Email.sendCancellation', event.data)
  await $.do('Retention.startWinback', event.data.customerId)
})
```
