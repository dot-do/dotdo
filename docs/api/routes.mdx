...
|-----------|-------------|
| `doClass` | DO namespace binding name (e.g., `DO`, `BROWSER_DO`) |
| `id` | Instance identifier for `idFromName()` |
...
This routes to the `DO` namespace, gets the instance with `idFromName('user-123')`, and forwards the request with path `/profile`.
