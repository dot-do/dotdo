...
```typescript
import { thing, forward, backward } from 'dotdo/semantic'

const alice = thing(Customer, { name: 'Alice' })
const order = thing(Order, { total: 99.99 })

// After action(alice, purchase, order), relationships are stored
...
1. `thing()` creates and stores the Thing
...
- [Nouns & Verbs](/docs/semantic/nouns-verbs) - Defining entity types
- [Actions](/docs/semantic/actions) - Recording operations on Things
- [Relationships](/docs/semantic/relationships) - Connecting Things
- [Concepts: Things](/docs/concepts/things) - URL-based Things in dotdo
