...
// result.success - Always true (reserved for future validation)
```
...
When using the `DOSemantic` Durable Object, actions are created via method calls:

```typescript
// Create an action (persists edge to SQLite)
const result = this.createAction(
  'alice',           // subject $id
  'purchase',        // verb name (must be defined)
  'order-123',       // object $id (optional)
  { price: 99.99 }   // metadata (optional)
)

// The result has the same structure:
// result.event  - { type: 'purchased', subject: 'alice', object: 'order-123', timestamp, metadata }
// result.edge   - { from: 'alice', to: 'order-123', verb: 'purchased' }
// result.audit  - { actor: 'alice', verb: 'purchased', target: 'order-123', timestamp }
```

<Callout type="info">
DOSemantic validates that the subject Thing and Verb exist before creating the action. Missing subjects or verbs will throw an error.
</Callout>

## Next Steps

- [Nouns & Verbs](/docs/semantic/nouns-verbs) - Define entity types and action verbs
- [Things](/docs/semantic/things) - Create entity instances
- [Relationships](/docs/semantic/relationships) - Traverse the graph created by Actions
