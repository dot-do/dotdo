...
Fuzzy operators require Things to be indexed first with `indexThing()` or `indexThings()`. Without indexing, fuzzy operators fall back to exact graph matches.
</Callout>
...
interface FuzzyOptions {
  /** Similarity threshold (0-1). Default: 0 for fuzzy operators */
  threshold?: number
...
<Callout type="info">
When fuzzy operators find no vector search results, they fall back to exact graph matches with a simulated score of 0.95.
</Callout>
...
```typescript
import { configureVectorStore, VectorStore } from '@dotdo/semantic'
...
// Index multiple Things (batched for efficiency)
...
<Callout type="info">
Without configuration, the vector store uses `InMemoryVectorIndex` and `MockEmbeddingProvider` - suitable for testing but not production. In production, configure Cloudflare Vectorize and Workers AI.
</Callout>

### Testing with Mock Providers

For unit tests, the semantic module provides mock implementations:
...
// Create test store with mocks (this is the default)
const store = new VectorStore()

// Or explicitly configure mocks
const store = new VectorStore({
  vectorize: new InMemoryVectorIndex(),
...
When using the `DOSemantic` Durable Object, relationship traversal uses ID-based methods:

```typescript
// Forward traversal (returns Things)
const orders = this.forward('alice', 'Order')

// Backward traversal
const buyers = this.backward('order-123', 'Customer')

// Fuzzy forward (returns Promise)
const similar = await this.forwardFuzzy('alice', 'Customer', {
  threshold: 0.7,
  withScores: true
})

// Fuzzy backward
const related = await this.backwardFuzzy('order-123', 'Product', {
  threshold: 0.5
})
```

<Callout type="info">
DOSemantic stores edges in SQLite and loads them into memory on initialization for fast traversal. Fuzzy methods currently simulate scores using exact matches.
</Callout>

## Next Steps

- [Things](/docs/semantic/things) - Create entity instances
- [Actions](/docs/semantic/actions) - Create edges through Actions
- [Nouns & Verbs](/docs/semantic/nouns-verbs) - Define entity types and Verbs
