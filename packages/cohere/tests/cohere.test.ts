/**
 * @dotdo/cohere - Cohere Package Tests
 *
 * RED-GREEN-REFACTOR: Tests for the Cohere API compatibility layer including:
 ...
 * - In-memory mock backend for testing without API calls
 */
...
describe('@dotdo/cohere - MockCohereBackend', () => {
  let backend: MockCohereBackend

  beforeEach(() => {
    backend = new MockCohereBackend()
  })

  describe('generate', () => {
    it('should generate mock text', async () => {
      const client = new CohereClient({ token: 'mock-token', fetch: backend.fetch })
      const response = await client.generate({
        prompt: 'Hello world',
      })
    ...
    it('should generate multiple outputs when numGenerations is set', async () => {
      const client = new CohereClient({ token: 'mock-token', fetch: backend.fetch })
      const response = await client.generate({
        prompt: 'Hello',
        numGenerations: 3,
  ...
  describe('chat', () => {
    it('should return mock chat response', async () => {
      ...
      expect(response.id).toMatch(/^chat_/)
      expect(response.finish_reason).toBe('COMPLETE')
      expect(response.message.role).toBe('assistant')
  ...
  describe('embed', () => {
    it('should return mock embeddings', async () => {
      ...
      expect(response.id).toMatch(/^embed_/)
      expect(response.embeddings.float).toHaveLength(2)
      // Each embedding should have 1024 dimensions by default
      expect(response.embeddings.float![0].length).toBeGreaterThan(0)
    })

    it('should respect outputDimension', async () => {
      const client = new CohereClient({ token: 'mock-token', fetch: backend.fetch })
      const response = await client.embed({
        texts: ['Hello'],
        model: 'embed-v4.0',
      ...
      expect(response.embeddings.float![0]).toHaveLength(256)
    })
  })

  describe('rerank', () => {
    it('should return mock rerank scores', async () => {
      const client = new CohereClient({ token: 'mock-token', fetch: backend.fetch })
      const response = await client.rerank({
        query: 'capital of France',
        documents: ['Paris is in France', 'London is in UK', 'Berlin is in Germany'],
        model: 'rerank-english-v2.0',
      })

      expect(response.id).toMatch(/^rerank_/)
      expect(response.results).toHaveLength(3)
      // Results should be sorted by relevance score descending
      expect(response.results[0].relevance_score).toBeGreaterThanOrEqual(response.results[1].relevance_score)
    })
  ...
  describe('classify', () => {
    it('should return mock classifications', async () => {
      const client = new CohereClient({ token: 'mock-token', fetch: backend.fetch })
      const response = await client.classify({
      ...
      expect(response.id).toMatch(/^classify_/)
      expect(response.classifications).toHaveLength(2)
      expect(response.classifications[0].labels).toBeDefined()
    })
  })

  describe('setResponse', () => {
    it('should allow setting custom responses', async () => {
      backend.setResponse('generate', {
        id: 'custom_gen',
        generations: [{ id: 'custom_0', text: 'Custom response!', index: 0 }],
      })

      const client = new CohereClient({ token: 'mock-token', fetch: backend.fetch })
      const response = await client.generate({ prompt: 'test' })

      expect(response.id).toBe('custom_gen')
      expect(response.generations[0].text).toBe('Custom response!')
    })
  })

  describe('setError', () => {
    it('should allow setting error responses', async () => {
      backend.setError('chat', 429, 'Rate limit exceeded')
  ...
  describe('getCallHistory', () => {
    it('should track API calls', async () => {
      const client = new CohereClient({ token: 'mock-token', fetch: backend.fetch })
      await client.generate({ prompt: 'test1' })
      await client.generate({ prompt: 'test2' })

      const history = backend.getCallHistory('generate')
      expect(history).toHaveLength(2)
      expect(history[0].prompt).toBe('test1')
      expect(history[1].prompt).toBe('test2')
    })
  })

  describe('reset', () => {
    it('should clear call history and custom responses', async () => {
      const client = new CohereClient({ token: 'mock-token', fetch: backend.fetch })
      await client.generate({ prompt: 'test' })
      backend.setResponse('generate', { id: 'custom', generations: [] })

      backend.reset()

      expect(backend.getCallHistory('generate')).toHaveLength(0)
    })
  })
})
