/**
 * @dotdo/path-utils - Comprehensive Path Utility Tests
 *
 * TDD RED Phase: These tests define the expected POSIX-compliant behavior
 * for path manipulation utilities shared across fsx, gitx, and bashx.
 *
 * Functions under test:
 * - normalize(path) - Normalize path segments (., .., //)
 * - join(...paths) - Join path segments
 * - resolve(...paths) - Resolve to absolute path
 * - dirname(path) - Get directory portion
 * - basename(path, ext?) - Get filename portion
 * - extname(path) - Get file extension
 * - parse(path) - Parse into components
 * - format(obj) - Format components to path
 * - isAbsolute(path) - Check if absolute
 * - relative(from, to) - Compute relative path
 * - sep, delimiter - Path constants
 */
