...
// =============================================================================
// Configurable Retryable Status Codes Tests
// =============================================================================

describe('Configurable Retryable Status Codes', () => {
  test('uses default retryable statuses when not configured', () => {
    // Default statuses: 429, 500, 502, 503, 504
    expect(isRetryableStatus(429)).toBe(true)
    expect(isRetryableStatus(500)).toBe(true)
    expect(isRetryableStatus(502)).toBe(true)
    expect(isRetryableStatus(503)).toBe(true)
  ...
  test('isRetryableStatus accepts custom status codes array', () => {
    // Custom statuses: only retry on 408 (Request Timeout) and 503 (Service Unavailable)
    const customStatuses = [408, 503]

    expect(isRetryableStatus(408, customStatuses)).toBe(true)
    expect(isRetryableStatus(503, customStatuses)).toBe(true)
    // Default retryable statuses should NOT be retryable with custom config
    expect(isRetryableStatus(429, customStatuses)).toBe(false)
    expect(isRetryableStatus(500, customStatuses)).toBe(false)
    expect(isRetryableStatus(502, customStatuses)).toBe(false)
    expect(isRetryableStatus(504, customStatuses)).toBe(false)
  })

  test('RetryConfig includes retryableStatuses option', () => {
    const handler = createRetryHandler({
      retryableStatuses: [408, 503, 522], // Custom: Request Timeout, Service Unavailable, Connection Timed Out
    })

    // The handler should use custom retryable statuses
    const response408 = new Response('', { status: 408 })
    const response503 = new Response('', { status: 503 })
    const response522 = new Response('', { status: 522 })
    const response429 = new Response('', { status: 429 })
    const response500 = new Response('', { status: 500 })

    expect(handler.shouldRetry(response408)).toBe(true)
    expect(handler.shouldRetry(response503)).toBe(true)
    expect(handler.shouldRetry(response522)).toBe(true)
    // Default retryable statuses should NOT be retryable with custom config
    expect(handler.shouldRetry(response429)).toBe(false)
    expect(handler.shouldRetry(response500)).toBe(false)
  })

  test('DEFAULT_RETRY_CONFIG includes default retryableStatuses', () => {
    expect(DEFAULT_RETRY_CONFIG.retryableStatuses).toEqual([429, 500, 502, 503, 504])
  })

  test('handler.fetch retries only on configured statuses', async () => {
    vi.useFakeTimers({ shouldAdvanceTime: true })
    ...
    const handler = createRetryHandler({
      ...
      jitter: 0,
      timeoutBudget: 5000,
      retryableStatuses: [503], // Only retry on 503
    })
    ...
    const fetchMock = vi.fn().mockImplementation(async () => {
      callCount++
      if (callCount === 1) {
        return new Response('', { status: 503 }) // Should retry
      }
      return new Response('OK', { status: 200 })
    })
    vi.stubGlobal('fetch', fetchMock)

    const result = await handler.fetch('https://api.example.com')
  ...
  test('handler.fetch does NOT retry on non-configured statuses', async () => {
    vi.useFakeTimers({ shouldAdvanceTime: true })
    ...
    const fetchMock = vi.fn().mockImplementation(async () => {
      return new Response('', { status: 429 }) // Would normally be retried, but not with custom config
    })
    vi.stubGlobal('fetch', fetchMock)

    const result = await handler.fetch('https://api.example.com')
    expect(result.status).toBe(429) // Should return without retry
  ...
  test('empty retryableStatuses means no retries on any status', () => {
    const handler = createRetryHandler({
      retryableStatuses: [], // No retryable statuses
    })
    ...
    expect(handler.shouldRetry(response429)).toBe(false)
    expect(handler.shouldRetry(response500)).toBe(false)
    expect(handler.shouldRetry(response503)).toBe(false)
  })

  test('custom retryableStatuses can include non-standard status codes', () => {
    // Some CDNs/proxies use non-standard status codes
    const handler = createRetryHandler({
      retryableStatuses: [520, 521, 522, 523, 524, 525, 526, 527], // Cloudflare-specific errors
    })

    const response520 = new Response('', { status: 520 }) // Unknown Error
    const response522 = new Response('', { status: 522 }) // Connection Timed Out
    const response524 = new Response('', { status: 524 }) // A Timeout Occurred

    expect(handler.shouldRetry(response520)).toBe(true)
    expect(handler.shouldRetry(response522)).toBe(true)
    expect(handler.shouldRetry(response524)).toBe(true)
  })
})
