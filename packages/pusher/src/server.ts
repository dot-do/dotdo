 ...
 *
 * Server-side Pusher API for Cloudflare Workers.
...
export class PusherServer {
   ...
   * ```
   */
  async trigger(channels: string | string[], event: string, data: unknown, options?: TriggerOptions): Promise<TriggerResult> {
   ...
   * ```
   */
  authorizeChannel(socketId: string, channel: string, presenceData?: PresenceChannelData): AuthSignature {
  ...
  authenticateUser(
    socketId: string,
    userData: { id: string; user_info?: Record<string, unknown> }
  ): {
    auth: string
    user_data: string
  } {
  ...
  private async triggerViaDO(channels: string[], event: string, data: unknown, options?: TriggerOptions): Promise<TriggerResult> {
    try {
  ...
  private async triggerViaHTTP(channels: string[], event: string, data: unknown, options?: TriggerOptions): Promise<TriggerResult> {
    const body = {
      name: event,
      channels,
    ...
    const response = await this.makeRequest('POST', `/apps/${this.appId}/events`, JSON.stringify(body))
  ...
  private async triggerBatchViaHTTP(batch: BatchEvent[]): Promise<TriggerResult> {
    ...
    const response = await this.makeRequest('POST', `/apps/${this.appId}/batch_events`, JSON.stringify(body))
