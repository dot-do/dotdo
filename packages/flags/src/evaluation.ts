...
// Import evaluateClause - will be defined in targeting.ts
// We need to handle circular dependency
let evaluateClauseFn: (clause: Clause, context: LDContext) => boolean

/**
 * Set the clause evaluator function (breaks circular dependency)
 */
export function setClauseEvaluator(fn: (clause: Clause, context: LDContext) => boolean): void {
  evaluateClauseFn = fn
}
...
function evaluateRule(rule: FlagRule, context: LDContext): boolean {
  ...
  for (const clause of rule.clauses) {
    if (!evaluateClauseFn(clause, context)) {
      return false
    }
  }
