/**
 * Redis Set Commands
 * SADD, SREM, SMEMBERS, SISMEMBER, SCARD, SDIFF, SINTER, SUNION
 */
...
export class SetCommands {
  ...
  private async setSet(key: string, set: Set<string>, expiresAt?: number): Promise<void> {
    if (set.size === 0) {
      ...
      await this.backend.set(key, 'set', Array.from(set), expiresAt)
    }
  }
  ...
  async smove(source: string, destination: string, member: string): Promise<number> {
    const sourceEntry = await this.backend.get(source)
