/**
 * @dotdo/sendgrid - Email Providers
 *
...
export class SendGridNativeProvider implements EmailProviderAdapter {
      ...
      switch (providerConfig.provider) {
        ...
        case 'sendgrid':
          if (providerConfig.apiKey) {
            this.providers.set(
              'sendgrid',
              new SendGridNativeProvider({ apiKey: providerConfig.apiKey })
            )
          }
          break
      }
...
export function createProvider(config: EmailProviderConfig): EmailProviderAdapter | null {
  switch (config.provider) {
    ...
    case 'sendgrid':
      if (!config.apiKey) return null
      return new SendGridNativeProvider({ apiKey: config.apiKey })
    default:
      return null
  }
}
