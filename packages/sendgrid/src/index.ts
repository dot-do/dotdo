  ...
  async send(
    ...
    try {
      ...
      for (const mailData of allRequests) {
        ...
        // Send via client
        const result = await this.client.mail.send({
          to: mailData.to,
          from: mailData.from,
          subject: mailData.subject,
          text: mailData.text,
          html: mailData.html,
          templateId: mailData.templateId || mailData.template_id,
          dynamicTemplateData: mailData.dynamicTemplateData || mailData.dynamic_template_data,
          personalizations: mailData.personalizations?.map(p => ({
            to: p.to,
            cc: p.cc,
            bcc: p.bcc,
            subject: p.subject,
            dynamicTemplateData: p.dynamicTemplateData || p.dynamic_template_data,
          })),
          attachments: mailData.attachments?.map(a => ({
            content: a.content,
            filename: a.filename,
            type: a.type,
            disposition: a.disposition,
          ...
          customArgs: mailData.customArgs || mailData.custom_args,
          sendAt: mailData.sendAt || mailData.send_at,
        })
...
export { SendGridClient, SendGridClientConfig, validateSendGridRequest }
