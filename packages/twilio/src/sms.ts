...
export class TwilioSMS {
  ...
  async send(request: SMSSendRequest): Promise<SMSSendResponse> {
    ...
    return this._sendWithRetry(normalizedRequest)
  }

  /**
   * Send with retry logic (inlined from Pipe)
   */
  private async _sendWithRetry(request: SMSSendRequest): Promise<SMSSendResponse> {
