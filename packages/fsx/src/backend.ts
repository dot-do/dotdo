...
export class MemoryBackend implements FsBackend {
  ...
  /**
   * Lock for serializing rename operations.
   *
   * Rename operations must be atomic - the collect-then-modify pattern
   * is not safe if async I/O is interleaved between phases. This lock
   * ensures that concurrent renames are serialized.
   *
   * Thread-safety guarantee: All rename operations are serialized.
   * Concurrent renames will queue and execute one at a time.
   */
  private renameLock: Promise<void> = Promise.resolve()

  /**
   * Acquire the rename lock. Returns a release function.
   * Uses promise chaining to implement a simple async mutex.
   */
  private acquireRenameLock(): Promise<() => void> {
    let release: () => void
    const previousLock = this.renameLock
    this.renameLock = new Promise<void>((resolve) => {
      release = resolve
    })
  ...
  private normalizePath(path: string): string {
    if (!path) {
      throw new ENOENT('normalizePath')
    }
  ...
  async readFile(path: string): Promise<Uint8Array> {
    ...
    if (this.directories.has(normalized)) {
      throw new EISDIR('read', path)
    }

    const file = this.files.get(normalized)
    if (!file) {
      throw new ENOENT('read', path)
    }
    return file.data
  }
  ...
  async writeFile(
    ...
    if (this.directories.has(normalized)) {
      throw new EISDIR('write', path)
    }

    // Validate parent path
    const pathError = this.validatePath(normalized)
    if (pathError === 'ENOTDIR') {
      throw new ENOTDIR('write', path)
    }
    if (pathError === 'ENOENT') {
      throw new ENOENT('write', path)
    }
  ...
  async rename(oldPath: string, newPath: string): Promise<void> {
    // Acquire lock to ensure atomic rename operation.
    // This prevents race conditions when multiple renames run concurrently.
    const release = await this.acquireRenameLock()
    try {
      await this.renameImpl(oldPath, newPath)
    } finally {
      release()
    }
  }
  ...
  /**
   * Internal rename implementation. Must be called with lock held.
   */
  private async renameImpl(oldPath: string, newPath: string): Promise<void> {
    ...
    if (this.directories.has(normalizedOld)) {
      ...
      // Apply renames atomically - with lock held, no concurrent modifications possible
  ...
  async copyFile(src: string, dest: string): Promise<void> {
    ...
    if (this.directories.has(normalizedSrc)) {
      throw new EISDIR('copyFile', src)
    }

    const file = this.files.get(normalizedSrc)
    if (!file) {
      throw new ENOENT('copyFile', src)
    }

    // Check destination parent exists
    const destParent = this.getParentDir(normalizedDest)
    if (destParent !== '/' && !this.directories.has(destParent)) {
      throw new ENOENT('copyFile', dest)
    }
  ...
  async mkdir(path: string, options?: MkdirOptions): Promise<void> {
    ...
    if (options?.recursive) {
      ...
      for (const part of parts) {
        ...
        if (this.files.has(current)) {
          throw new ENOTDIR('mkdir', current)
        }
      ...
      const pathError = this.validatePath(normalized)
      if (pathError === 'ENOTDIR') {
        throw new ENOTDIR('mkdir', path)
      }
      if (pathError === 'ENOENT') {
        throw new ENOENT('mkdir', path)
      }
  ...
  async rmdir(path: string, options?: RmdirOptions): Promise<void> {
    ...
    // Cannot remove root
    if (normalized === '/') {
      throw new EPERM('rmdir', '/')
    }

    // Check if it's a file
    if (this.files.has(normalized)) {
      throw new ENOTDIR('rmdir', path)
    }

    if (!this.directories.has(normalized)) {
      throw new ENOENT('rmdir', path)
    }
    ...
    if (hasChildren && !options?.recursive) {
      throw new ENOTEMPTY('rmdir', path)
    }
    if (options?.recursive) {
  ...
  async readdir(path: string, options?: ReaddirOptions): Promise<string[] | Dirent[]> {
    ...
    // Check if it's a file
    if (this.files.has(normalized)) {
      throw new ENOTDIR('readdir', path)
    }

    // Check if directory exists
    if (!this.directories.has(normalized)) {
      throw new ENOENT('readdir', path)
    }
  ...
  async utimes(path: string, atime: Date | number, mtime: Date | number): Promise<void> {
    ...
    throw new ENOENT('utimes', path)
  }

  async symlink(_target: string, path: string): Promise<void> {
    throw new ENOSYS('symlink', path)
  }

  async link(_existingPath: string, newPath: string): Promise<void> {
    throw new ENOSYS('link', newPath)
  }

  async readlink(path: string): Promise<string> {
    throw new ENOSYS('readlink', path)
  }

  async realpath(path: string): Promise<string> {
    const normalized = this.normalizePath(path)
    if (!this.files.has(normalized) && !this.directories.has(normalized)) {
      throw new ENOENT('realpath', path)
    }
    return normalized
  }
