 ...
 *
 * Tests for @supabase/supabase-js API compatibility backed by in-memory storage:
...
describe('from().update()', () => {
  ...
  it('should update rows with filter', async () => {
    // First insert a row
    await client.from<TestUser>('users').insert({ id: 1, name: 'Alice', active: true })
  ...
  it('should update multiple fields', async () => {
    await client.from<TestUser>('users').insert({ id: 2, name: 'Bob', age: 25 })
...
describe('from().delete()', () => {
  ...
  it('should delete rows with filter', async () => {
    // First insert a row
    await client.from<TestUser>('users').insert({ id: 1, name: 'ToDelete' })
  ...
  it('should delete with count option', async () => {
    await client.from<TestUser>('users').insert({ id: 2, name: 'ToDelete2', active: false })
