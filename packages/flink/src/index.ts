/**
 * @dotdo/flink - Apache Flink DataStream API Compat Layer
 *
 * Stateful stream processing with Flink-compatible API and in-memory backend.
...
export class WindowedStream<T, K, W extends TimeWindow> {
  ...
  private processWindows<R>(processor: (elements: T[]) => R[]): DataStream<R> {
    ...
    const isSessionWindow = this.windowAssigner instanceof SessionWindows

    for (const [key, elements] of keyGroups) {
      // Sort elements by timestamp for proper window assignment
      const sortedElements = [...elements].sort((a, b) => {
        const tsA = this.timestampAssigner ? this.timestampAssigner(a) : 0
        const tsB = this.timestampAssigner ? this.timestampAssigner(b) : 0
        return tsA - tsB
      })

      // For session windows, we need to merge overlapping sessions
      if (isSessionWindow) {
        const sessions: { start: number; end: number; elements: T[] }[] = []

        for (const element of sortedElements) {
          const timestamp = this.timestampAssigner ? this.timestampAssigner(element) : Date.now()
          const [window] = this.windowAssigner.assignWindows(element, timestamp)

          // Try to merge with existing session
          let merged = false
          for (const session of sessions) {
            // Check if element's window overlaps with or is adjacent to existing session
            if (timestamp <= session.end) {
              // Extends or within existing session
              session.end = Math.max(session.end, window!.getEnd())
        ...
        for (const session of sessions) {
          const windowResults = processor(session.elements)
          results.push(...windowResults)
        }
      } else {
        // Non-session windows: standard processing
