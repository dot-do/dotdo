 ...
 */
export const DEFAULT_RPC_TIERS = {
    free: {
        name: 'free',
...
export class RPCRateLimiter {
    ...
    constructor(config = {}) {
        this.config = {
            ...
            defaultTier: config.defaultTier ?? DEFAULT_RPC_TIERS['free'],
        };
    }
    /**
     ...
     */
    checkTenantLimit(request, method, now) {
     ...
     */
    checkUserLimit(request, method, now) {
     ...
     */
    getLimitsForMethod(request, method) {
     ...
     */
    checkSlidingWindow(key, method, limits, now, scope) {
     ...
     */
    createAllowedResult(request, method, limit, windowMs) {
        const now = Date.now();
     ...
     */
    buildHeaders(limit, remaining, resetAt, retryAfterSec, scope) {
 ...
 * @issue do-dp5q.5 - Add rate limiting to RPC endpoints
 */
export function rpcRateLimitMiddleware(config = {}) {
 ...
 */
export function createRPCRateLimiter(config = {}) {
