...
import { NotFoundError, ValidationError, InternalError, AuthorizationError, serializeError, isRPCError, } from './errors';
import { validateArgs, getMethodSchema, validateZodArgs, getZodMethodSchema, isZodMethodSchema, } from './validation';
 ...
 */
export function validateMethodPath(method) {
    // Must be a string
 ...
 * Case-sensitive matching
 */
function matchesWhitelist(methodPath, whitelist) {
 ...
 */
function hasPrivateSegment(methodPath) {
...
export function createServer(options) {
    ...
    let currentWhitelist = options.whitelist;
    let currentSchemas = options.schemas;
    const app = new Hono();
    // Add updateWhitelist method to the app
    app.updateWhitelist = (newWhitelist) => {
        currentWhitelist = newWhitelist;
    };
    // Add updateSchemas method to the app
    app.updateSchemas = (newSchemas) => {
    ...
    if (enablePipeline) {
        app.post('/rpc/pipeline', async (c) => {
            ...
            try {
                ...
                if (response.error) {
                    const error = new InternalError(`Pipeline step ${response.error.stepIndex} failed: ${response.error.message}`, { stepIndex: response.error.stepIndex, code: response.error.code });
                ...
                return c.json({ ...serializeError(wrappedError, { includeStack: false }), correlationId }, wrappedError.httpStatus);
            }
        });
    }
    // RPC endpoint
    app.post('/rpc', async (c) => {
        ...
        try {
            ...
            if (currentSchemas) {
                ...
                const zodSchema = getZodMethodSchema(currentSchemas, method);
            ...
            let result = await fn.apply(current, invokeArgs);
 ...
 * @since 1.0.0
 */
export function createWorkerFromTarget(target) {
 ...
 * ```
 */
export function getExecutionContext(arg) {
    if (arg && typeof arg === 'object' && RPC_CONTEXT in arg) {
        return arg[RPC_CONTEXT];
    }
    return undefined;
}
/**
 ...
 */
export function getCorrelationIdFromArgs(args) {
    if (args.length === 0)
        return undefined;
