...
import { DotdoError, ErrorCode, ERROR_CODE_TO_HTTP_STATUS, getHttpStatusForCode, isRetryableCode, serializeDotdoError, deserializeDotdoError, isSerializedDotdoError, getErrorMessage, isRetryableError as dbIsRetryableError, registerErrorClass, } from '@dotdo/db';
// =============================================================================
// Re-export base types from @dotdo/db for backward compatibility
// =============================================================================
export { ErrorCode, ERROR_CODE_TO_HTTP_STATUS, getHttpStatusForCode, isRetryableCode, getErrorMessage, DotdoError, };
/**
...
export class NotFoundError extends RPCError {
    constructor(message = 'Resource not found', details, options) {
    ...
    static forResource(resourceType, resourceId) {
        return new NotFoundError(`${resourceType} with id ${resourceId} not found`, { resourceType, resourceId });
    }
}
/**
...
export class ValidationError extends RPCError {
     ...
     */
    static forField(field, constraint, value) {
        return new ValidationError(`Validation failed: ${field} ${constraint}`, { field, constraint, value });
    }
}
/**
...
export class AuthenticationError extends RPCError {
    constructor(message = 'Authentication required', details, options) {
...
export class AuthorizationError extends RPCError {
    constructor(message = 'Access denied', details, options) {
        super(ErrorCode.AUTHORIZATION_ERROR, message, details, options);
        this.name = 'AuthorizationError';
    }
    static insufficientPermissions(action, resource, requiredRoles) {
        return new AuthorizationError(`Insufficient permissions to ${action} ${resource}`, { action, resource, requiredRoles });
    }
}
/**
...
export class ConflictError extends RPCError {
    constructor(message = 'Resource conflict', details, options) {
        super(ErrorCode.CONFLICT, message, details, options);
        this.name = 'ConflictError';
    }
    static resourceExists(resourceType, conflictField, conflictValue) {
        return new ConflictError(`${resourceType} with ${conflictField} "${conflictValue}" already exists`, { resourceType, conflictField, conflictValue });
    }
    static versionMismatch(resourceType, resourceId, expectedVersion, actualVersion) {
        return new ConflictError(`Version mismatch for ${resourceType} ${resourceId}: expected ${expectedVersion}, got ${actualVersion}`, { resourceType, resourceId, expectedVersion, actualVersion });
    }
}
/**
...
export class RateLimitError extends RPCError {
    constructor(message = 'Rate limit exceeded', details, options) {
        super(ErrorCode.RATE_LIMIT, message, details, options);
        this.name = 'RateLimitError';
    }
    static exceeded(info) {
        const retryMsg = info.retryAfter ? ` Retry after ${info.retryAfter}s.` : '';
        return new RateLimitError(`Rate limit of ${info.limit} requests per ${info.window} exceeded.${retryMsg}`, info);
    }
}
/**
...
export class PayloadTooLargeError extends RPCError {
    constructor(message = 'Request payload too large', details, options) {
    ...
    static exceeded(info) {
        ...
        return new PayloadTooLargeError(`Request payload size (${info.size} ${unit}) exceeds maximum allowed size (${info.maxSize} ${unit})`, { size: info.size, maxSize: info.maxSize, unit });
    }
    /**
     * Create a PayloadTooLargeError with human-readable size information
     */
    static exceededReadable(info) {
        ...
        return new PayloadTooLargeError(`Request payload size (${formatSize(info.sizeBytes)}) exceeds maximum allowed size (${formatSize(info.maxSizeBytes)})`, { sizeBytes: info.sizeBytes, maxSizeBytes: info.maxSizeBytes });
    }
}
/**
 * Timeout error (504)
...
export class TimeoutError extends RPCError {
    constructor(message = 'Request timed out', details, options) {
        super(ErrorCode.TIMEOUT, message, details, options);
        this.name = 'TimeoutError';
    }
    static afterMs(timeout) {
...
export class NetworkError extends RPCError {
    constructor(message = 'Network error', details, options) {
        super(ErrorCode.NETWORK_ERROR, message, details, options);
        this.name = 'NetworkError';
    }
    static connectionRefused(host, port) {
    ...
    static dnsResolutionFailed(host) {
...
export class InternalError extends RPCError {
    constructor(message = 'Internal error', details, options) {
...
export class ServiceUnavailableError extends RPCError {
    constructor(message = 'Service temporarily unavailable', details, options) {
        super(ErrorCode.SERVICE_UNAVAILABLE, message, details, options);
        this.name = 'ServiceUnavailableError';
    }
    static maintenance(estimatedRecovery) {
...
export class TransportError extends RPCError {
    constructor(message = 'Transport error', details, options) {
        super(ErrorCode.NETWORK_ERROR, message, details, options);
     ...
     */
    static fromError(error, transport) {
        if (error instanceof Error) {
            // Check for abort/timeout errors
            if (error.name === 'AbortError' || error.name === 'TimeoutError') {
                return new TransportError(`Transport timeout: ${error.message}`, { transport, reason: 'timeout', originalError: error.name }, { cause: error });
            }
            // Check for network-related errors
            if (error.name === 'TypeError' && error.message.includes('fetch')) {
                return new TransportError(`Network error: ${error.message}`, { transport, reason: 'network_failure', originalError: error.name }, { cause: error });
            }
            return new TransportError(`Transport error: ${error.message}`, { transport, reason: 'unknown', originalError: error.name }, { cause: error });
        }
        return new TransportError(`Transport error: ${String(error)}`, { transport, reason: 'unknown' });
    }
    /**
     * Create a TransportError for a fetch failure
     */
    static fetchFailed(url, error) {
        return TransportError.fromError(error, `fetch:${url}`);
    }
    /**
     * Create a TransportError for a WebSocket failure
     */
    static webSocketFailed(url, error) {
        return TransportError.fromError(error, `websocket:${url}`);
    }
    /**
     * Create a TransportError for a DO stub failure
     */
    static stubFailed(error) {
        return TransportError.fromError(error, 'do-stub');
    }
    /**
     * Create a TransportError when a circuit breaker is open
     */
    static circuitOpen(circuitName) {
        return new TransportError(`Circuit breaker "${circuitName}" is open - requests are being rejected`, {
...
export class CircuitOpenError extends RPCError {
    constructor(message = 'Circuit breaker is open', details, options) {
    ...
    static withMetrics(metrics) {
        const retryAfter = metrics.lastFailureTime && metrics.resetTimeMs
...
for (const [name, ErrorClass] of Object.entries(RPC_ERROR_REGISTRY)) {
    registerErrorClass(name, ErrorClass);
}
/**
 ...
 */
export function serializeUnknownError(error, options = {}) {
    if (error instanceof RPCError) {
...
export function deserializeError(serialized) {
    ...
    if (errorType === 'RPCError') {
        const error = new RPCError(serialized.code ?? ErrorCode.INTERNAL_ERROR, serialized.message, serialized.details);
        if (serialized.stack) {
            error.stack = serialized.stack;
        }
        return error;
 ...
 */
export async function handleHTTPError(jsonPromise, options) {
 ...
 */
export function serializeErrorResponse(error, options = {}) {
 ...
 */
export function createErrorResponse(error, correlationId, includeStack = false) {
