...
export class WebSocketEventServer {
     ...
     * ```
     */
    broadcast(eventType, payload, eventId) {
        const message = {
            type: 'event',
            event: eventType,
            payload,
     ...
     */
    matchesPattern(eventType, noun, verb, pattern) {
        // Exact match
     ...
     */
    getSubscriptionCount(eventPattern) {
        let count = 0;
        for (const client of this.clients.values()) {
            for (const sub of client.subscriptions.values()) {
 ...
 */
export function createWebSocketEventServer(options) {
